...
ALLOWED_PACKAGE_FEEDS = [
    ...
    "https://o365exchange.pkgs.visualstudio.com/_packaging/O365PythonPackages/pypi/simple/"
]
...
class Prepare(Command):
    ...
    def get_target_file_contents(
        ...
        if Path(component_target_file_path).exists():
            ...
            if chardet.detect(rbfile).get("encoding").lower() not in ["utf-8", "ascii"]:
                raise ValueError(
    ...
    def _extract_dependencies_and_channels(self, component, component_name) -> List[list]:
        ...
        conda_dependencies = []
        conda_channels = []
        if "type" in spec and spec["type"] == "spark":
            if "conda_dependencies" in spec:
                conda_dependencies = spec["conda_dependencies"]
            ...
            if "environment" in spec and "conda_file" in spec["environment"]:
                conda_dependencies_file = spec["environment"]["conda_file"]
                try:
                    with open(
                        os.path.join(
                            component_repo, conda_dependencies_file
                        )
                    ) as file:
            ...
            for dependency in package_dependencies:
                if re.match("^--index-url", dependency) or re.match(
                    ...
                    if dependency.split(" ")[1] not in ALLOWED_PACKAGE_FEEDS:
                        ...
                        return False
            found_package_feed = False
            for feed in ALLOWED_PACKAGE_FEEDS:
                if (
                    f"--index-url {feed}" in package_dependencies
                    or f"--extra-index-url {feed}"
                    in package_dependencies
                ):
                    found_package_feed = True
            if not found_package_feed:
                log.error(
                    f"Neither Polymer package feed nor O365 feed is found in environment of {component}"
                )
                return False
    ...
    def add_conda_to_path(
        ...
        if operating_system == OperatingSystem.WINDOWS:
            if runtime_environment == RuntimeEnvironment.GITHUB_ACTION:
                Path(os.environ["GITHUB_PATH"]).write_text(
                    os.path.join(os.environ["CONDA"], "condabin")
                )
                success = True
            elif runtime_environment == RuntimeEnvironment.AZURE_DEVOPS_BUILD:
                self.azure_devops_append_to_path(
                    os.path.join(os.environ["CONDA"], "Scripts")
                )
                success = True
            else:
            ...
            if runtime_environment == RuntimeEnvironment.GITHUB_ACTION:
                Path(os.environ["GITHUB_PATH"]).write_text(
                    os.path.join(os.environ["CONDA"], "bin")
                )
                success = True
            elif runtime_environment == RuntimeEnvironment.AZURE_DEVOPS_BUILD:
                self.azure_devops_append_to_path(
        ...
        if success:
            log.info(
                f"Successfully added conda to the system path for runtime environment '{runtime_environment}' and operating system '{operating_system}'."
            )
        elif runtime_environment == RuntimeEnvironment.OTHER:
            log.info(f"Assuming local run for runtime environment {runtime_environment} and operating system {operating_system}. "
                     "Skipping adding conda to the system path."
            )
        else:
            log.error(
                f"Failed to add conda to the system path for runtime environment '{runtime_environment}' and operating system '{operating_system}'."
            )
    ...
    def determine_runtime_environment(self) -> RuntimeEnvironment:
        ...
        runtime_environment = (
            self.determine_runtime_environment_based_on_environment_variables(
                github_ev, azure_ev
            )
        )
    ...
    @staticmethod
    def determine_runtime_environment_based_on_environment_variables(
        github_ev: Union[str, None], azure_ev: Union[str, None]
    ) -> RuntimeEnvironment:
        """
        ...
        if github_ev and azure_ev:
            raise ValueError(
                "It seems like both the GitHub- and Azure-specific environment variables are non-empty. This shouldn't be possible and implies shrike should be using alternate environment variables to determine the runtime environment. Please open an issue in the GitHub shrike repository: https://github.com/ai-platform-ml-platform/shrike/issues."
            )
        elif github_ev:
