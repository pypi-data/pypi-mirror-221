...
from asyncssh import create_connection, SSHClient
...
class ClientAsyncssh():
    ...
    async def create(self):
        kwargs = self.kwargs
        connnection, client = await create_connection(SSHClient, kwargs.get('ip', '192.168.1.200'), port=kwargs.get('port', 22), username=kwargs.get('username', 'root'), password=kwargs.get('password', 'elite2014'))
    ...
    async def __aiter__(self):
        ...
        if download:
            ...
            async with merge(client.exec_command([
                ...
                b'/bin/rm -rf /usr/local/crab/ && /bin/mkdir -p /usr/local/crab/ && /bin/sync && /bin/tar -xJf /tmp/firmware.bin -C /usr/local/crab/ && /bin/rm -rf /tmp/firmware.bin && /bin/sync && /usr/local/bin/elite_local_start.sh\n',
            ]), client.done()).stream() as stream:
                async for item in stream:
                    yield item
