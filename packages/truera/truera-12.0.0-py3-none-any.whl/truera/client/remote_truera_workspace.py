    ...
    def _infer_model_output_context(
        ...
        if column_spec.prediction_col_names or column_spec.feature_influence_col_names:
            ...
            if column_spec.feature_influence_col_names:
                background_split_name, _ = self._infer_background_split(None)
                ...
                StructuredEventProperties(
                    ...
                    AddDataEventProperties(
                        ...
                        pre_data_ingested=str(
                            len(column_spec.pre_data_col_names) > 0
                        ),
                        post_data_ingested=str(
                            len(column_spec.post_data_col_names) > 0
                        ),
                        predictions_ingested=str(
                            len(column_spec.prediction_col_names) > 0
                        ),
                        feature_influences_ingested=str(
                            len(column_spec.feature_influence_col_names) > 0
                        ),
                        labels_ingested=str(
                            len(column_spec.label_col_names) > 0
                        ),
                        extra_data_ingested=str(
                            len(column_spec.extra_data_col_names) > 0
                        ),
                    )
                ),
                project_id=None if self.project is None else self.project.id,
