...
class OpenAICompletionParameters(ProcessorParameters):
    ...
    candidate_labels: Dict[str, str] = Field(
        ...
        extra="advanced,key:label,inject",
    )
...
def add_annotations(annotations: List[Dict]):
    ...
    return [Annotation(label=a['label'], start=a['start'], end=a['end']) for a in annotations]
        ...
        try:
            ...
            for document in documents:
                ...
                if params.model.value.startswith("gpt-"):
                    ...
                    response = openai_chat_completion(openai, **kwargs)
                    contents = []
                    for r in response["choices"]:
                        if r.message['content']:
                            contents.append(r.message['content'])
                        elif r.message['function_call']:
                ...
                if result:
                    if isinstance(result, str):
                        ...
                        if function_name == "add_annotations":
                            document.annotations = function_response
