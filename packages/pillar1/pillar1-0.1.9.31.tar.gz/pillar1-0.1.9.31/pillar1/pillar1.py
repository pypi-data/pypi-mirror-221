...
class Model:
    def __init__(self, user_name: str = '', password: str = '', end_point: str = '', max_new_tokens: int = 100, verbose: bool = False, api_type: str = 'free',
        ...
        elif self.end_point == MODELS.GPT35:
    ...
    def query_list_ui(self, list_of_questions):
        dropdown = widgets.Dropdown(
            ...
            layout=widgets.Layout(width='80%'),  # Add this line to make the dropdown
        )
    ...
    def query(self, prompt: str = 'Find the ten first rows in the table', print_sql: bool = False, execute_code=False):
        def submit_request():
            ...
            if self.end_point == MODELS.GPT35:
                ...
                self.response = response['choices'][0]['message']['content'].replace('```sql', '').replace('```', '')
                end_time = datetime.datetime.now()
                duration = end_time - start_time
                if self.verbose:
                    print(f"Took {duration.total_seconds():.2f} seconds to complete the operation.")

            if self.end_point == MODELS.GPT4:
                pass

            if self.end_point == MODELS.CODEY:
                if self.verbose:
                    print(f"Input: {self.full_prompt}.\n\nTemperature: {self.temperature}.\nMax output tokens: {self.max_new_tokens}")
                response = self.curr_model.predict(prefix=self.full_prompt,
                                                   temperature=self.temperature,
                                                   max_output_tokens=self.max_new_tokens)
                self.response = response.text.replace('```sql', '').replace('```', '')
