    ...
    try:
        ...
        if response.status_code == 404:
            raise llama.error.ModelNameError(
                f"Error 404: Model name not found. Model name must match the model name in Hugging Face."
            )
        if response.status_code == 429:
            raise llama.error.RateLimitError(f"Error 429: Rate limit error")
        if response.status_code == 401:
            raise llama.error.AuthenticationError(f"Error 401:Check your api key")
        if response.status_code == 400:
            raise llama.error.UserError(f"Error 400: Bad Request")
