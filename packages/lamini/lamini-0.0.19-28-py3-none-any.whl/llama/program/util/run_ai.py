    ...
    try:
        ...
        if response.status_code == 404:
            raise llama.error.ModelNameError(
                response.json().get("detail", "ModelNameError")
            )
        if response.status_code == 429:
            raise llama.error.RateLimitError(
                response.json().get("detail", "RateLimitError")
            )
        if response.status_code == 401:
            raise llama.error.AuthenticationError(
                response.json().get("detail", "AuthenticationError")
            )
        if response.status_code == 400:
            raise llama.error.UserError(response.json().get("detail", "UserError"))
        if response.status_code == 503:
            raise llama.error.UnavailableResourceError(
                response.json().get("detail", "UnavailableResourceError")
            )
