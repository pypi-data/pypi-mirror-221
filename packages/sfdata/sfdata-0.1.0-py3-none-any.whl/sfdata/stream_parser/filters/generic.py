...
from sfdata.stream_parser.checks import EventCheck, type_check
from sfdata.stream_parser import events
from sfdata.stream_parser.function_helpers import filter_caller
from sfdata.stream_parser.functions import *
from sfdata.stream_parser.stream import first_then_rest
from sfdata.stream_parser.types import EventFilter, FilteredValue, FilterErrorHandler
...
def __event_generator(func, default_args=None, **genargs):
    ...
    def wrapper(stream, *_, **kwargs):
        ...
        if isinstance(stream, events.ParseEvent):
            stream = (stream,)
...
def event_attribute_stream_filter(
    ...
    yield from extractor_stream_filter(
        iterable, lambda x: x.get(event_attribute), mapping_functions
    )
...
def check_stream_filter(
    ...
    yield from permissive_filter_stream(
        iterable, filter_function, type_check(events.Cell)
    )


def until_match(
    iterable: Iterable[events.ParseEvent], check: EventCheck, yield_final=False
) -> FilteredValue:
    """
