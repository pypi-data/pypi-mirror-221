...
class ModelPerformanceBarGraphReport(Report):
    ...
    def __init__(
        ...
        y_label: Optional[str] = None  # todo: Pass loss name from YAML
    ...
    def generate(self, df: pd.DataFrame) -> None:
        ...
        ax.set_title('PSI prediction error of tested models', fontsize=self.fonttitle)
        ax.set_xlabel('models', fontsize=self.fontsize)
...
class ModelPredictionsScatterPlotReport(Report):
    ...
    def generate(self, df: pd.DataFrame) -> None:
        ...
        for (model_fqn, hyper_parameters_md5), model_scores in top_models.iterrows():
            ...
            y_pred = np.clip(y_pred, a_min=0.0, a_max=1.0)  # clip predictions
            ...
            plt.plot([0.0, 1.0], [0.0, 1.0], c='gray', alpha=0.7, ls='--', label='y=x line')

            # plot fitting line
            fitted = self._fit_line_to_points(y_true, y_pred, val_min=0.0, val_max=1.0)
...
class FeatureRankingBarGraphReport(Report):
    ...
    def generate(self, df: pd.DataFrame) -> None:
        ...
        for (model_fqn, hyper_parameters_md5), feature_scores_df in score_groups:
            ...
            plt.title(f'Most-{self.top_k} relevant regulators\n({model_alias})', fontsize=self.fonttitle, y=1.01)
            ax.set_xlabel('relevance score', fontsize=self.fontsize)
            ax.set_ylabel('regulators', fontsize=self.fontsize)
