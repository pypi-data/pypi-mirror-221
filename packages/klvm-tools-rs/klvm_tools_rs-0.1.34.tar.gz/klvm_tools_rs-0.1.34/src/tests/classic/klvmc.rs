...
use crate::classic::klvm_tools::klvmc::compile_klvm;
use crate::tests::classic::run::read_json_from_file;

#[test]
fn test_compile_klvm_function_1() {
    let mut symbol_hash_table = HashMap::new();
    let bridge_hex_file = "validation_taproot.klvm2.hex";
    ...
    let read_in_hex_data =
        fs::read_to_string("resources/tests/bridgeref/validation_taproot.klvm.hex.reference")
            .expect("should exist");
...
#[test]
fn test_compile_klvm_with_previous_data() {
    let bridge_hex_file = "validation_taproot.klvm.hex";
    let bridge_hex =
        fs::read_to_string("resources/tests/bridgeref/validation_taproot.klvm.hex.reference")
