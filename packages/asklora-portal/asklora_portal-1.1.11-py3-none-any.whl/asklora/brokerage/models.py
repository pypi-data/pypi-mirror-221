...
class DAMApplicationPayload(BaseModel):
    ...
    # source of wealth
    sources_of_wealth: list[SourceOfWealth]

    # documents
    w8ben_file: str | PathLike
    proof_of_identity_file: str | PathLike
    proof_of_address_file: str | PathLike | None
    proof_of_address_type: enums.ProofOfAddressTypeEnum | None
    ...
    def attached_files(self):
        files = [self.w8ben_file, self.proof_of_identity_file]

        if self.proof_of_address_file:
            files.append(self.proof_of_address_file)

        return files

    @root_validator
    def check_employment_info(cls, values):
        employment_type = values.get("employment_type")
        occupation = values.get("occupation")
        employer = values.get("employer")
        employer_business = values.get("employer_business")
        employer_address_country = values.get("employer_address_country")
        employer_address_city = values.get("employer_address_city")
        employer_address_postal_code = values.get("employer_address_postal_code")
        employer_address_street_name = values.get("employer_address_street_name")

        IS_EMPLOYED = employment_type in [
            enums.EmploymentTypeEnum.EMPLOYED,
            enums.EmploymentTypeEnum.SELFEMPLOYED,
        ]

        DATA_IS_NOT_COMPLETE = None in [
            occupation,
            employer,
            employer_business,
...
class Process(BaseXmlModel):
    ...
    applications: list[ApplicationResponse] = wrapped(
        "Applications",
