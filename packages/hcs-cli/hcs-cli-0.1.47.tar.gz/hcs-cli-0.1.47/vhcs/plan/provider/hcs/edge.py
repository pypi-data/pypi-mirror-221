...
from vhcs.service import admin, portal
...
def deploy(data: dict, state: dict, save_state) -> dict:
    ...
    if not state:
        ...
        deployment = state
    id = deployment["id"]
    fn_pass_created_phase = lambda t: t['status'] != 'CREATE_ACCEPTED'
    try:
        deployment = admin.edge.wait_for(id, org_id, fn_pass_created_phase)
