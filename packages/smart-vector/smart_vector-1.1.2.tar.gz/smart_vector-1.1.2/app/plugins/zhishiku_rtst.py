...
def find(s, step=0, **kwargs):
    ...
    profile = UserProfile.objects.filter(fromUser__username=request.user.username)
    if profile:
        profile = profile[0]
    else:
        return []
    ...
    if collection.is_where and collection.remark:
        ...
        where = f"collection='{profile.collection.code}'"
    memory_name = profile.collection.name
    # try:
    if not where:
