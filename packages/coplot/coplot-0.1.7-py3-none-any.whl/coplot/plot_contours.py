...
class Plot_1d:
    ...
    def plot(self, bins=100,smooth=3,x_lims=None,colors=None,line_styles=None,
             ...
             show_title=False,show_allTitles=False,title_fontsize=None,title_sigma=1):
        ''' 
        ...
        if show_title:
            ...
            #title strings
            title_str = r'%s$=%.3f^{+%.3f}_{-%.3f}$'%(labels[0],best_fit,sigma_1r,sigma_1l)
            if show_allTitles and self.N>1:
                for i in range(self.N-1):
                    running_bestFit = Chains.bestFit(self.data[i+1], best_type='mode',out_sigma=1, symmetry_error=False)
                    running_best_fit = running_bestFit[:,0]
                    running_sigma_1l, running_sigma_1r = running_bestFit[:,1], running_bestFit[:,2]
                    running_str = '\n%s$=%.3f^{+%.3f}_{-%.3f}$'%(labels[0],running_best_fit,running_sigma_1r,running_sigma_1l)
                    title_str = title_str + running_str
...
#            legend_loc = 'center'
            if legend_size is None:
                ...
                # legend_size = ticks_size * 1.20833
            if fill_contours is False:
                ax.legend(bbox_to_anchor=bbox_to_anchor, loc=legend_loc, borderaxespad=0., fontsize=legend_size)
            elif fill_contours is True:
                patch = []
                for i in range(len(legend_colors)):
    ...
    def __plot_2d(self, ax,XY, bins, levels, smooth, line_style, line_width, color,
        ...
        if best_value is not None:
            if best_value[0] is not None and best_value[1] is not None:
...
#            legend_loc = 'center'
    ...
    def plot(self, bins=None,labels=None,colors=None,line_styles=None,
             ...
             layout_adjust=None,lims=None,line_width=None,show_titles=False, show_allTitles=False,
             ...
             legend_size=None,legend_loc=None,equal_1d=True,pdf_method='2',
        ...
        :param legend_loc: legend location, None or tuple or list 
        i.e. (i, j) or [i, j], where i refers to the i-th panel in x-axis and j refers to the j-th panel in y-axis
        :param equal_1d: making the maximum of the probability density function of multiple data sets equal
        :param pdf_method: '1' or '2', the method to estimate the probability density function for the given data, see the method pdf_1 and pdf_2
        '''
        if bins is None:
...
#            line_styles = pl.fiducial_line_styles[:self.n]
        ...
        if show_titles:
            ...
            title_str = []
            for i in range(self.N):
                title_str.append(r'%s$=%.3f^{+%.3f}_{-%.3f}$'%(labels[i],best_fit[i],sigma_1r[i],sigma_1l[i]))
            ...
            if show_allTitles and self.n>1:
                for i in range(self.n-1):
                    ...
                    running_sigma_1l, running_sigma_1r = running_bestFit[:,1], running_bestFit[:,2]
                    for j in range(self.N):
                        running_str = '\n%s$=%.3f^{+%.3f}_{-%.3f}$'%(labels[j],running_best_fit[j],running_sigma_1r[j],running_sigma_1l[j])
                        title_str[j] = title_str[j] + running_str
                ...
                if i == j:
                    ...
                    if i == self.N-1:
                        ...
                        if show_titles is True:
                            if title_sigma == 1:
                                ...
                                ax.set_title(title_str[j], fontsize=title_fontsize)
                            elif title_sigma == 2:
                                if title_fontsize is None:
                                    title_fontsize = ticks_size*1.5
                                ax.set_title(r'%s$=%.3f^{+%.3f+%.3f}_{-%.3f-%.3f}$'\
                    ...
                    for k in range(self.n):
                        if legend is False:
                            ...
                            if legend_loc is None:
                                bbox_to_anchor_calibrate = (1, self.N)
                            else:
                                bbox_to_anchor_calibrate = (legend_loc[0]-(self.N-1), legend_loc[1]-0)
                            if i == self.N-1:
                                if fill_contours is False:
                                    self.__plot_1d(ax,self.data[k][:,i],bins,colors[k],\
                                    ...
                                    bbox_to_anchor=bbox_to_anchor_calibrate)
                                elif fill_contours is True:
                                    self.__plot_1d(ax,self.data[k][:,i],bins,colors[k],\
                                    line_styles[k],line_width,smooth_1d, ticks_size, legend=legend,\
