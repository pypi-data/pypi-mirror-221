...
def analyze_nma_data(ch_nma_free_wave: nhf_reader.NHFDataset , ch_nma_interaction: nhf_reader.NHFDataset, ch_topography_in_um: nhf_reader.NHFDataset, deflection_sensitivity:float, spring_constant:float) -> np.ndarray:
    ...
    for i in range(num_of_curves):
        ...
        analysis_result = analyze_force_curve(distance_nm,force_nN)
    ...
    return max_force_matrix, adhesion_matrix, stiffness_matrix
...
def convert_file(source_file:pathlib.Path, target_file:pathlib.Path) -> bool:
    ...
    max_force_matrix, adhesion_matrix, stiffness_matrix = analyze_nma_data(
        ch_nma_free_wave, ch_nma_interaction, ch_topography_in_mu, deflection_sensitivity, spring_constant)
    print("Done")
    ...
    ### Export data to gwyddion file ###
                            ...
                            data_sets=[ch_topography_in_mu.dataset*1e-6, max_force_matrix*1e-9, adhesion_matrix*1e-9, stiffness_matrix],
                            data_labels=["Topography", "Max Force","Adhesion","Stiffness"],
                            data_units=['m', 'N', 'N', 'N/m2'])
    if done:
...
def process_all_files_in_folder(folder:pathlib.Path, from_suffix:str, to_suffix) -> bool:
    ...
    if len(list_of_source_files) > 0:
        for current_file in list_of_source_files:
            print(f"Processing file: {current_file.name}")  
            target_file = current_file.with_suffix(to_suffix)
            done = convert_file(current_file, target_file)
            if done == False:
