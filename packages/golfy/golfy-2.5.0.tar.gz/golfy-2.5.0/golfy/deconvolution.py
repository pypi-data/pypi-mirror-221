...
def solve_linear_system(
    ...
    for loo_idx in loo_indices:
        ...
        avg_activity += x
        curr_cutoff = np.sqrt(np.abs(c)) + min_peptide_activity
...
def em_step(
    ...
    new_beta = (X.T @ ((y + stability_factor) / (stability_factor + X @ beta))) * (
        beta / X.sum(0)
    )
...
def em_deconvolve(
    ...
    background = beta[-1]
    any_empty_wells = np.any(linear_system.A.sum(1) == 0)
    if any_empty_wells:
        cutoff = np.abs(background) + min_peptide_activity
    else:
        cutoff = 4 * np.abs(background) + min_peptide_activity
