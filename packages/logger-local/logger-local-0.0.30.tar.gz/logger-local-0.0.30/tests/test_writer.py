...
def test_check_Function():
    ...
    conn = get_connection()
    cursor = conn.cursor()
    sql = f"""SELECT component_id FROM logger.logger_table WHERE record = '{'{"payload": "check python", "component_id": 102, "component_name": "Logger Python", "a": 5, "b": 6, "severity_id": 400, "severity_name": "Start", "function": "test_check_Function", "environment": "play1", "class": "test_writer", "row": 372, "computer_language": "Python"}'}' ORDER BY timestamp DESC LIMIT 1;"""
    cursor.execute(sql)
    result = cursor.fetchone()
    assert result[0] == COMPONENT_ID
    ...
    sql = f"""SELECT component_id FROM logger.logger_table WHERE record = '{'{"component_id": 102, "payload": "check python", "return": 9, "component_name": "Logger Python", "severity_id": 400, "severity_name": "End", "function": "test_check_Function", "environment": "play1", "class": "test_writer", "row": 386, "computer_language": "Python"}'}' ORDER BY timestamp DESC LIMIT 1;"""
    cursor.execute(sql)
    result = cursor.fetchone()
    assert result[0] == COMPONENT_ID
