...
from pankmer.reg_coverage import check_for_bgzip, get_chromosome_sizes_from_ref
...
def anchormap(pk_results, output_dir, anchors, anno, threads: int = 1):
    ...
    sizes = {'.'.join(os.path.basename(a).split('.')[:-2]):
             get_chromosome_sizes_from_ref(a).set_index('name')['size']
             for a in anchors}
    ...
    for anchor, anno in zip(anchors, anno):
        ...
        regcov_dict = pk_results.get_collapsed_regional_scores(anchor,
            {c: [[1, sizes[a][c]]] for c in sizes[a].index})
        print('converting to bytes')
        scores = b''.join(int(''.join(str(i) for i in s), 2).to_bytes(
                score_bitsize, byteorder="big", signed=False)
            for c in sizes[a].index for s in regcov_dict[c][(1, sizes[a][c])])
        print('writing scoremap')
        for step in 1, LOWRES_STEP:
            with BgzfWriter(os.path.join(output_dir, 'anchors', f'{a}.{step}.bgz'), 'wb') as scores_out:
                scores_out.write(scores[::step] if step > 1 else scores)
            subprocess.run(('bgzip', '-rI',
