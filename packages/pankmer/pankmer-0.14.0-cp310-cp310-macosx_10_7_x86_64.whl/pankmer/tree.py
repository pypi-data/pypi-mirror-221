...
def adj_to_qv(adj_matrix):
    ...
    mat = adj_matrix.to_numpy()
    kmers_per_col = get_kmers_per_col(mat)
    qv = -10*np.log10(1 - (mat / kmers_per_col)**(1/31) + np.identity(len(adj_matrix.index)))
    return pd.DataFrame(qv, index=adj_matrix.index, columns=adj_matrix.columns)


def adj_to_qv_symmetric(adj_matrix):
    ...
    mat = adj_matrix.to_numpy()
    kmers_per_col = get_kmers_per_col(mat)
    qv_sym = -10*np.log10(1 - (mat / (kmers_per_col + np.transpose(kmers_per_col) - mat))**(1/31) + np.identity(len(adj_matrix.index)))
