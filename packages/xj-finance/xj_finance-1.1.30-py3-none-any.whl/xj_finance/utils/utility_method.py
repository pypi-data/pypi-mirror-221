...
def aggregate_data(data_list, group_field, agg_fields):
    ...
    # 根据 group_field 对数据进行分组，并对 agg_fields 进行求和
    grouped = df.groupby(group_field, as_index=False).agg({field: 'sum' for field in agg_fields})
    ...
    for record in grouped_data:
        group_values = record[group_field]
        merged_record = df[df[group_field] == group_values].iloc[0].to_dict()
