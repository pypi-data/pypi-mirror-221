...
class FinanceListService:
    ...
    def user_balance_list(params):
        # 子查询以获取每个account_id的最新create_time
        ...
        params = format_params_handle(
            ...
            split_list=["account_id_list"],
            alias_dict={"account_id_list": "account_id__in"
                        },
        )
        transact_obj = Transact.objects
        latest_transactions = transact_obj.filter(**params).values('account_id').annotate(
            max_date=Max('create_time')
        ).values('account_id', 'max_date')

        # 用于筛选finance_transaction对象的子查询
        latest_balance_per_account = Transact.objects.filter(
            account_id__in=Subquery(latest_transactions.values('account_id')),
            create_time__in=Subquery(latest_transactions.values('max_date'))
        ).values('account_id', 'balance')
        total = latest_balance_per_account.count()
        current_page_set = latest_balance_per_account[
                           page * size: page * size + size] if page >= 0 and size > 0 else latest_balance_per_account
        for i, it in enumerate(current_page_set):
            it['order'] = page * size + i + 1
            it['balance'] = keep_two_decimal_places(it['balance'])
            res_list.append(it)
        ...
        user_id_list = [item.get("account_id", None) for item in data]
        user_list, err = DetailInfoService.get_list_detail(user_id_list=user_id_list,
                                                           filter_fields=['user_id', 'user_name', 'nickname', 'phone',
                                                                          'register_time',
                                                                          'real_name', 'is_delete'])
        if user_list:
            data = JoinList(data, user_list, "account_id", "user_id").join()
        data = format_dates(data, ['register_time'])
