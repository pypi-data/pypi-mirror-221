...
from xj_finance.utils.utility_method import extract_values, replace_key_in_dict
...
class FinanceApi(APIView):  # 或继承(APIView)
    ...
    def list(self, *args, user_info, request_params, **kwargs, ):
        ...
        account_params = format_params_handle(
            param_dict=request_params,
            filter_filed_list=["account_name"],
            is_remove_empty=True
        )
        if account_params:
            account_ids, err = UserService.user_list(
                params=replace_key_in_dict(account_params, "account_name", "user_name"))
            if not err:
        ...
        their_account_params = format_params_handle(
            param_dict=request_params,
            filter_filed_list=["their_account_name"],
            is_remove_empty=True
        )
        if their_account_params:
            their_account_ids, err = UserService.user_list(
                params=replace_key_in_dict(their_account_params, "their_account_name", "user_name"))
            if not err:
    ...
    def balance_list(self, *args, user_info, request_params, **kwargs, ):
        ...
        account_params = format_params_handle(
            param_dict=request_params,
            filter_filed_list=["account_name", "phone"],
            is_remove_empty=True
        )
        if account_params:
            account_ids, err = UserService.user_list(
