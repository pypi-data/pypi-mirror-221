...
class CmdOps(Process):
    ...
    def toHtml(self, params={}):
        if 'exec' in params:
            ...
            return "Done."
        return '<form><input type="text" name="exec" /><input type="hidden" name="name" value="'+params['name']+'" /><input type="submit" name="submit" value="Exec" /></form>'
...
class NWebJobFetch(Ticker):
    ...
    """
    key = None
    def __init__(self, interval = 91, nwebclient=None, group=None):
...
class NWebJobResultUploader(Process):
    ...
    def cmd(self, args):
        if len(args)>=2 and args[0]=='jobresult':
            ...
            self.nweb.createDoc(name, s, self.group, kind='json')
...
class NwFlaskRoutes(b.Base):
    def __init__(self):
        super().__init__()
    def addTo(self, app):
        app.add_url_rule('/nw/<path:p>', 'nw', lambda p: self.nw(p))
    ...
    def nw(self, path):
        ...
        params = {**request.args.to_dict(), **{'path': path}}
        if self.hasName(path):
            return self.getChildByName(path).page(params)
        plugin = b.Plugins('nweb_web')[path]
        if plugin is not None:
            obj = util.load_class(plugin.value, create=True)  
            w = self.addChild(b.WebObject(obj, params))
            w.name = path
...
class WebProcess(Process):
    web = None
    def __init__(self, port = 9080, host=None):
    ...
    def startServer(self):
        ...
        self.thead = threading.Thread(target=f)
        self.thead.start()
    def index(self):
    ...
    def createApp(self):
        ...
        self.addChild(NwFlaskRoutes()).addTo(app)
        return app
    def startAsync(self):
        app = self.createApp()
