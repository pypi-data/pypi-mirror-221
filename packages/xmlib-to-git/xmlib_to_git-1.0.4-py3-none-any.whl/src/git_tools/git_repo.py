...
def check_git_exists():
    if not os.path.exists(settings.APP_DIR + "/.git"):
        ...
        git_repo_obj = Repo.init(settings.APP_DIR)
        git_origin = git_repo_obj.create_remote(
        ...
        git_repo_obj = Repo(settings.APP_DIR)
        git_origin = git_repo_obj.remote("origin")

    git_repo_obj.remotes.origin.push("--all")
