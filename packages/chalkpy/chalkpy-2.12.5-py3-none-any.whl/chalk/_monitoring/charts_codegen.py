...
from typing import Any, Dict, List, Literal, Optional, Union, Tuple, TypeVar
from chalk.features.resolver import ResolverProtocol
from chalk._monitoring.charts_enums_codegen import GroupByKind
from chalk._monitoring.charts_series_base import SeriesBase, ResolverType
...
class Series(SeriesBase):
    ...
    @classmethod
    def feature_request_count_metric(
        cls,
    ...
    @classmethod
    def resolver_request_count_metric(
        cls,
        name: Optional[str] = None
    ...
    def resolver_latency_metric(
        ...
        if window_function not in {"mean", "max", "99%", "95%", "75%", "50%", "25%", "5%", "min", "all"}:
            raise ValueError(f"window_function value '{window_function}' is not valid")
        return ResolverLatencySeries(
            name=name,
            metric="RESOLVER_LATENCY",
            window_function=window_function
        )

    @classmethod
    def resolver_success_ratio_metric(
        cls,
        name: Optional[str] = None
    ...
    @classmethod
    def query_success_ratio_metric(
        cls,
        name: Optional[str] = None
    ...
    def stream_message_latency_metric(
        ...
        if window_function not in {"mean", "max", "99%", "95%", "75%", "50%", "25%", "5%", "min", "all"}:
            raise ValueError(f"window_function value '{window_function}' is not valid")
        return StreamMessageLatencySeries(
            name=name,
            metric="STREAM_MESSAGE_LATENCY",
            window_function=window_function
        )
    ...
    def stream_messages_processed_metric(
        cls,
        window_function: Literal["sum", "mean", "max", "99%", "95%", "75%", "50%", "25%", "5%", "min", "all"],
        name: Optional[str] = None,
        ...
        """
        if window_function not in {"sum", "mean", "max", "99%", "95%", "75%", "50%", "25%", "5%", "min", "all"}:
            raise ValueError(f"window_function value '{window_function}' is not valid")
    ...
    def stream_window_latency_metric(
        ...
        if window_function not in {"mean", "max", "99%", "95%", "75%", "50%", "25%", "5%", "min", "all"}:
            raise ValueError(f"window_function value '{window_function}' is not valid")
        return StreamWindowLatencySeries(
            name=name,
            metric="STREAM_WINDOW_LATENCY",
            window_function=window_function
        )
