...
class Chart:
    ...
    def with_series(self, series: SeriesBase) -> "Chart":
        ...
        if series._entity_id and self._entity_id and series._entity_id != self._entity_id:
            raise ValueError(
                ...
                f"'{series._name or series._default_name}' "
    ...
    def with_trigger(
        ...
        if trigger_name is None:
            trigger_name = (
                f"{expression.lhs._name or expression.lhs._default_name}{expression.operation}{expression.rhs}"
            )
    ...
    def __getitem__(self, key: str) -> Union[SeriesBase, _Formula]:
        ...
        """
        for series in self._series:
            if series._name == key or series._default_name == key:
                return series
