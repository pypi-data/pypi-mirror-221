...
from ..Types import target_type_is_numeric, convert_types
...
class WriterCDC(GenericWriter, WriterInterface):
    ...
    def save_upsert_data_on_aux_s3(self, bucket, df, path_file, file_name):
        def save_data_on_aux(op, df, path_file):
            df = self.get_upsert_rows(df, op=op)
            df = convert_types(self.struct, df)
        ...
        save_data_on_aux("I", df, f"{path_file}/insert/{file_name}")
        save_data_on_aux("U", df, f"{path_file}/update/{file_name}")
    ...
    def save_to_redshift_from_s3(self, path_file, bucket):
        if self.check_s3_path_exists(bucket, path_file) is True:
            print(f"Saving data to Redshift: s3://{bucket}/{path_file}")
    ...
    def delete_on_aux_s3(self, bucket, path_file):
        if self.check_s3_path_exists(bucket, path_file) is True:
            bucket_client = self.migrator_redshift_connector.s3_session.Bucket(bucket)
    ...
    def save_data(self, operations):
        ...
        path_file = f"raw/aux/{self.env}/{self.struct.target_schema}/{self.struct.target_table}/{time}"
        bucket = None
        ...
        try:
            for operation in operations:
                ...
                print(f"Downloading file from S3: {url}")
                ...
                df = self.equalize_number_columns(df)

                self.delete_data(df)
                self.save_upsert_data_on_aux_s3(bucket, df, path_file, file_name)
            ...
            self.delete_on_aux_s3(bucket, path_file)
