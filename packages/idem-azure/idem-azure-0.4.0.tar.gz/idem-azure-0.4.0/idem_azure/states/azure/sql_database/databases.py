    ...
    if not existing_resource:
        ...
        if ctx.get("test", False):
            ...
            if response_put["status"] == 202:
                ...
                get_response = await hub.tool.azure.polling.await_operation(
                    ctx,
                    initial_response=response_put,
...
async def absent(
    ...
    response_get = await hub.exec.azure.sql_database.databases.get(
        ctx,
        resource_id=resource_id,
        raw=True,
    )
    if response_get["result"]:
        if response_get["ret"]:
            ...
            if ctx.get("test", False):
                result["comment"].append(
                    f"Would delete azure.sql_database.databases '{name}'"
                )
                return result
            ...
            if not response_delete["result"]:
                hub.log.debug(
                    f"Could not delete azure.sql_database.databases '{name}' {response_delete['comment']} {response_delete['ret']}"
                )
                result["result"] = False
                result["comment"] = [response_delete["comment"], response_delete["ret"]]
                return result

            result["comment"].append(f"Deleted azure.sql_database.databases '{name}'")
            return result
        else:
            # If Azure returns 'Not Found' error, it means the resource has been absent.
            result["comment"].append(
                f"azure.sql_database.databases '{name}' already absent"
            )
            return result
    else:
        hub.log.debug(
            f"Could not get azure.sql_database.databases '{name}' {response_get['comment']} {response_get['ret']}"
        )
        result["result"] = False
