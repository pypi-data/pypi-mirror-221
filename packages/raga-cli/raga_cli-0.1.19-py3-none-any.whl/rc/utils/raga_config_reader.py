import os
import configparser
from rc.utils import RAGA_CONFIG_FILE
import sys

DEFAULT_CONFIG_VALUES = {
    "rc_base_url": "https://example.com",
    "web_backend_url": "https://example.com",
    "influx_backend_url": "https://example.com",
}
...
def validate_default_section(config):
    ...
    for option, default_value in DEFAULT_CONFIG_VALUES.items():
        if option not in default_section or default_section[option] == default_value:
            raise ValueError(f"Please update the value of '{option}' in the [default] section of the config file. You can update using this command `sudo vim {config_file_path}`")
