...
class Initializer:
    ...
    def __init__(self, obj, axis: Optional[int] = None, *, data_manager = None) -> None:
        ...
        if len(list_obj) == 0:
            assert axis is None, ValueError("`axis` must be None since nothing to concatenate")
            self._concat_axis = None
            self._dtype = None
            self._shape = (0,)
            self._metadata = None
