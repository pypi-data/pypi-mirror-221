        ...
        try:
            with cupy.cuda.Device(index) as self._d:
                with cupy.cuda.Stream(non_blocking=True) as self._s:
                    if os.getenv('CUPY_INIT_MEMPOOL', '1') != '0':
                        # cupy.cuda.alloc(2 ** min(30, int(log2(self._d.mem_info[0]))))  # TODO config prealloc
                        cupy.cuda.alloc(self._d.mem_info[0] - (self._d.mem_info[0] % 2**30))  # maximize prealloc to GB
