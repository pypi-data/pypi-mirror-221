...
def start(base_folder = "", project_name = "", save_files = False):
    ...
    if save_files == True:
      ...
      data = {'run_id' : run_id_global, 'config_dict': str({"Python Files":run_dir_global+'/files/'}), 'node_name': "Datasets", 'username': username, 'key': key}
...
def torch_model(model=None, metric_name=None, metric_value=None, goal='maximize', run_id=None):
    try:
        ...
        print("Please install Pytorch before using this function.")
        return
    ...
    if model:
      ...
      if goal == 'maximize':
        if metric_value > prev_max:
          ...
          if metric_name:
            torch.save(model.state_dict(), run_dir_global+'/model/best_'+str(metric_name)+'_'+str(metric_value)+'_chkpt.pt')
            config_dict.update({'best_weights': run_dir_global+'/model/best_'+str(metric_name)+'_'+str(metric_value)+'_chkpt.pt'})
          else:
            torch.save(model.state_dict(), run_dir_global+'/model/best_metric_chkpt.pt')
            config_dict.update({'best_weights': run_dir_global+'/model/best_metric_chkpt.pt'})
        ...
        if metric_value < prev_min:
          ...
          if metric_name:
            torch.save(model.state_dict(), run_dir_global+'/model/best_'+str(metric_name)+'_'+str(metric_value)+'_chkpt.pt')
            config_dict.update({'best_weights_path': run_dir_global+'/model/best_'+str(metric_name)+'_'+str(metric_value)+'_chkpt.pt'})
          else:
            torch.save(model.state_dict(), run_dir_global+'/model/best_metric_chkpt.pt')
            config_dict.update({'best_weights_path': run_dir_global+'/model/best_metric_chkpt.pt'})
    else:
      print("Please specify a Pytorch model.")
      return

    config(config_dict = config_dict, node_name="Datasets", run_id = None)
    

def data(train_set=[], train_labels=[], test_set=[], test_labels=[], val_set=[], val_labels=[],
  ...
  if not os.path.exists(run_dir_global+'/data'):
    os.mkdir(run_dir_global+'/data')

  if len(train_set) > 0:
    with open(run_dir_global+'/data/train_set.pkl', 'wb') as f:
        dill.dump(train_set, f)
    config_dict.update({'train_set': run_dir_global+'/data/train_set.pkl'})
    '''
    train_mean = np.mean(train_set)
    train_min = np.min(train_set)
    train_max = np.max(train_set)
    config_dict.update({'train_mean': train_mean, 'train_min': train_min, 'train_max': train_max})
    '''

  if len(val_set) > 0:
    with open(run_dir_global+'/data/val_set.pkl', 'wb') as f:
        dill.dump(val_set, f)
    config_dict.update({'val_set': run_dir_global+'/data/val_set.pkl'})
    '''
    val_mean = np.mean(val_set)
    val_min = np.min(val_set)
    val_max = np.max(val_set)
    config_dict.update({'val_mean': val_mean, 'val_min': val_min, 'val_max': val_max})
    '''

  if len(test_set) > 0:
    with open(run_dir_global+'/data/test_set.pkl', 'wb') as f:
        dill.dump(test_set, f)
    config_dict.update({'test_set': run_dir_global+'/data/test_set.pkl'})
    '''
    test_mean = np.mean(test_set)
    test_min = np.min(test_set)
    test_max = np.max(test_set)
    config_dict.update({'test_mean':test_mean, 'test_min':test_min, 'test_max':test_max})
    '''
  ...
  if len(train_labels) > 0:
    ...
    with open(run_dir_global+'/data/train_labels.pkl', 'wb') as f:
  ...
  if len(val_labels) > 0:
    ...
    with open(run_dir_global+'/data/val_labels.pkl', 'wb') as f:
  ...
  if len(test_labels) > 0:
    ...
    with open(run_dir_global+'/data/test_labels.pkl', 'wb') as f:
...
def torch_data(train_dataloader=None, val_dataloader=None, test_dataloader=None, label_index=None, data_index=None,
         problem_type = 'binary classification',  node_name="Datasets", run_id = None):
  ...
  if train_dataloader:
    if label_index:
      ...
      with open(run_dir_global+'/data/train_dataloader.pkl', 'wb') as f:
          dill.dump(train_dataloader, f)
      config_dict.update({'train_dataloader': run_dir_global+'/data/train_dataloader.pkl'})

  if val_dataloader:
    if label_index:
      val_labels = [np.array(val_dataloader.dataset[i][label_index]) for i in range(len(val_dataloader.dataset))]
      with open(run_dir_global+'/data/val_dataloader.pkl', 'wb') as f:
          dill.dump(val_dataloader, f)
      config_dict.update({'val_dataloader': run_dir_global+'/data/val_dataloader.pkl'})

  if test_dataloader:
    if label_index:
      test_labels = [np.array(test_dataloader.dataset[i][label_index]) for i in range(len(test_dataloader.dataset))]
      with open(run_dir_global+'/data/test_dataloader.pkl', 'wb') as f:
          dill.dump(test_dataloader, f)
      config_dict.update({'test_dataloader': run_dir_global+'/data/test_dataloader.pkl'})
