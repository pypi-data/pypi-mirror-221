...
def create_run(project_id = None):
    ...
    response = requests.post(protocol+'://'+IP+'/api/create_run', data=data)
...
def start(base_folder = "", project_name = ""):
    ...
    r_id = create_run(project_id)

    project_id = p_id
    run_id = r_id

    os.mkdir(base_folder+'/mynacode/'+project_name+'/'+str(r_id)+'_'+str(datetime.now())[:11])
