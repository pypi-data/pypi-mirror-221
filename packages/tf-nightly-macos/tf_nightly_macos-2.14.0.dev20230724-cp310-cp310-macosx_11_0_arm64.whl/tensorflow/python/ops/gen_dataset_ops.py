...
def dataset_cardinality(input_dataset, cardinality_options="", name=None):
...
def dataset_cardinality_eager_fallback(input_dataset, cardinality_options, name, ctx):
  if cardinality_options is None:
    cardinality_options = ""
  cardinality_options = _execute.make_str(cardinality_options, "cardinality_options")
  input_dataset = _ops.convert_to_tensor(input_dataset, _dtypes.variant)
...
def tf_record_dataset_v2(filenames, compression_type, buffer_size, byte_offsets, metadata="", name=None):
  r"""Creates a dataset that emits the records from one or more TFRecord files.
...
def tf_record_dataset_v2_eager_fallback(filenames, compression_type, buffer_size, byte_offsets, metadata, name, ctx):
  if metadata is None:
    metadata = ""
  metadata = _execute.make_str(metadata, "metadata")
  filenames = _ops.convert_to_tensor(filenames, _dtypes.string)
