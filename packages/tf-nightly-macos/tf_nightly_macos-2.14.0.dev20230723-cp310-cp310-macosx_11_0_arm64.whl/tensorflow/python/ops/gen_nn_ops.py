...
@_dispatch.add_type_based_api_dispatcher
@tf_export('conv')
def conv(input, filter, strides, padding, explicit_paddings=[], data_format="CHANNELS_LAST", dilations=[], batch_dims=1, groups=1, name=None):
  r"""Computes a N-D convolution given (N+2)-D `input` and `filter` tensors.
...
def conv_eager_fallback(input, filter, strides, padding, explicit_paddings, data_format, dilations, batch_dims, groups, name, ctx):
  if not isinstance(strides, (list, tuple)):
    raise TypeError(
        "Expected list for 'strides' argument to "
        "'conv' Op, not %r." % strides)
