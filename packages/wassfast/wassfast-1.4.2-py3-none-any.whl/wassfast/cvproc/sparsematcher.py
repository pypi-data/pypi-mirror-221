...
class SparseMatcher:
    ...
    def extract_features(self, I0P, I1P, mask0, mask1, fb_threshold=0.8, winsize=(13,13), maxlevel=3, optflo_method= 'PyrLK' ):
        ...
        if optflo_method == 'PyrLK':
            ...
            flow = None
            flow = cv.calcOpticalFlowFarneback(I0P,I1P,flow,pyr_scale=0.5,levels=3,winsize=15, iterations=10, poly_n=5, poly_sigma=1.5,flags = 0 )
            ...
            flow = None
            flow = cv.calcOpticalFlowFarneback(I1P,I0P,flow,pyr_scale=0.5,levels=3,winsize=15, iterations=10, poly_n=5, poly_sigma=1.5,flags = 0 )
            fu = np.squeeze( flow[:,:,0] )
            fv = np.squeeze( flow[:,:,1] )
