...
def load_library(path: Optional[str] = None, cuda: bool = False) -> Any:
    ...
    lib.ctransformers_llm_get_total_state_size.argtypes = [
        llm_p
    ]
    lib.ctransformers_llm_get_total_state_size.restype = c_size_t

    lib.ctransformers_llm_clone_state.argtypes = [
        llm_p,
        c_void_p
    ]
    lib.ctransformers_llm_clone_state.restype = None

    lib.ctransformers_llm_set_state.argtypes = [
        llm_p,
        c_void_p
    ]
    lib.ctransformers_llm_set_state.restype = None
...
class LLMState:
    def __init__(self, llm: "LLM"):
        state_size = llm.state_size()
        assert state_size > 0
        self._ptr = create_string_buffer(state_size)
...
class LLM:
    ...
    def token_id_to_str(self, token: int) -> str:
        return self.ctransformers_llm_detokenize(token)
    ...
    def state_size(self) -> int:
        return self.ctransformers_llm_get_total_state_size()

    def clone_state(self) -> LLMState:
        state = LLMState(self)
        self.ctransformers_llm_clone_state(state._ptr)
        return state
    
    def set_state(self, state: LLMState):
        self.ctransformers_llm_set_state(state._ptr)
