...
class BaseWarmup(_LRScheduler):
    ...
    def scale(self):
        ...
        if isinstance(scale_value, (int, float)):
            ...
            assert isinstance(scale_value, (list, tuple)), 'Only support list or tuple type!'
            assert len(scale_value) == len(self.optimizer.param_groups), ('Size mismatch {} != {}'.format(en(scale_value), len(self.optimizer.param_groups)))
