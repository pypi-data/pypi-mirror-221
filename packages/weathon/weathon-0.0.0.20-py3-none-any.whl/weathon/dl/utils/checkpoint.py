...
def save_pretrained(model,
    ...
    if target_folder is None or os.path.isfile(target_folder):
        raise ValueError(f'Provided path ({target_folder}) should be a directory, not a file')

    if save_checkpoint_name is None:
        raise Exception('At least pass in one checkpoint name for saving method')
    ...
    ignore_file_set.add('.*')
    if hasattr(model,'model_dir') and model.model_dir is not None and is_master():
        copytree(model.model_dir,target_folder,ignore=ignore_patterns(*ignore_file_set))
