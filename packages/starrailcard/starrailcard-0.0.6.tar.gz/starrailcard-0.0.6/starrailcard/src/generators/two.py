...
class Creat:
    ...
    async def main_skills(self):
        ...
        resus = treePaths.map_icon[PATH]

        num = 1
        id_stats = f"{self.character.id}00{num}"

        
        for key in self.character.skills:
            data = await treePaths.get_data(id_stats)
            if data is None:
                num += 2
                id_stats = f"{self.character.id}00{num}"
                continue
            if str(data["baseSkill"][0]) == key.id:
                ...
                bg.alpha_composite(icon,resus[data["pos"]]["icon"]) 
                if "count" in resus[data["pos"]]:
                    bg.alpha_composite(count,resus[data["pos"]]["count"]) 
        ...
        num = 1
        id_stats = f"{self.character.id}10{num}"

        dop_path_closed = _of.path_closed

        while num != 4:
            data = await treePaths.get_data(id_stats)
            icon = await icon_tree(data["type"], data["icon"])
            icon = await pill.recolor_image(icon, (0, 0, 0))
            
            pos_data = resus[data["pos"]]
            bg.alpha_composite(icon, pos_data["icon"])

            if not int(id_stats) in dats:
                bg.alpha_composite(dop_path_closed, pos_data["icon"])
        ...
        i = 1
        position_num = 5
        id_stats = f"{self.character.id}20{num}"
        position = f"Point0{position_num}"
        while True:
            data = await treePaths.get_data(id_stats)
            if data is None:
                break
            ...
            if num == 10:
                if f"{num}" in resus[position]["dop_point"]:
                    bg.alpha_composite(icon,resus[position]["dop_point"][f"{num}"])
                else:
                    for key in resus:
                        if "dop_point" in resus[key]: 
                            if  f"{num}" in resus[key]["dop_point"]: 
                                bg.alpha_composite(icon,resus[key]["dop_point"][f"{num}"])
            else:
                if f"0{num}" in resus[position]["dop_point"]:
                    bg.alpha_composite(icon,resus[position]["dop_point"][f"0{num}"])
                else:
                    for key in resus:
                        if "dop_point" in resus[key]: 
                            if  f"0{num}" in resus[key]["dop_point"]: 
                                bg.alpha_composite(icon,resus[key]["dop_point"][f"0{num}"]) 
            
            if not int(id_stats) in dats:
                if num == 10:
                    if f"{num}" in resus[position]["dop_point"]:
                        bg.alpha_composite(_of.path_closed_dop.resize((36,35)),resus[position]["dop_point"][f"{num}"])
                    else:
                        for key in resus:
                            if "dop_point" in resus[key]: 
                                if  f"{num}" in resus[key]["dop_point"]: 
                                    bg.alpha_composite(_of.path_closed_dop.resize((36,35)),resus[key]["dop_point"][f"{num}"]) 
                else:
                    if not f"0{num}" in resus[position]["dop_point"]:
                        for key in resus:
                            if "dop_point" in resus[key]: 
                                if  f"0{num}" in resus[key]["dop_point"]: 
                                    bg.alpha_composite(_of.path_closed_dop.resize((36,35)),resus[key]["dop_point"][f"0{num}"])                        
                    else:
                        bg.alpha_composite(_of.path_closed_dop.resize((36,35)),resus[position]["dop_point"][f"0{num}"])

            if i == len(resus[position]["dop_point"]):
                position_num += 1
                position = f"Point0{position_num}"
                i = 0
            i += 1
            num += 1
            if num == 10:
                id_stats = f"{self.character.id}2{num}"
            else:
                id_stats = f"{self.character.id}20{num}"
        return bg      
