...
async def connection_create_snowflake(
    ...
    if integration_name is None:
        integration_name = click.prompt("Custom integration name (optional)", default='', show_default=False)

    if stage_name is None:
        stage_name = click.prompt("Custom stage name (optional)", default='', show_default=False)
...
async def connection_create_gcs(
    ...
    if not connection_name:
        connection_name = click.prompt(f"Connection name (optional, current: {key})", default=key)
        await validate_connection_name(client, connection_name, 'gcs')
    ...
    params = ConnectionReplacements.map_api_params_from_prompt_params("gcs", key=key, secret=secret, region=region, connection_name=connection_name)
