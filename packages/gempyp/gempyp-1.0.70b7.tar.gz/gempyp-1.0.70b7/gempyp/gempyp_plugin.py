...
# def pytest_reporter_object(request):
#     """
#     Fixture to access the reporter object.
#     This fixture will store the reporter object created in pytest_runtest_makereport hook.
#     """
#     # Get the reporter object created in pytest_runtest_makereport hook
#     reporter_instance = request.config.pluginmanager.get_plugin("reporter_object")
#     return reporter_instance.reporter if reporter_instance else None
...
def send_testcase_data(item,data,DATA,rep,s_run_id):
    # reporter_instance = gempyp_plugin()
    # reporter = reporter_instance.reporter
    ...
    # item.config.pluginmanager.register(pytest_reporter_object, "reporter_object")
    ...
    if(rep.outcome=="passed"):
            ...
            description=str(assertion_statements)
    reporter.addRow( rep.nodeid, description, testcaseStatus)
...
def pytest_runtest_makereport(item):
    ...
    if rep.when == 'call':
        ...
        # assertion_statements = self.extract_assertions(source_code)
        send_testcase_data(item,data,DATA,rep,s_run_id)
