...
def cv_split_prep(df, n_splits=12, stratify_on=None, group_on=None, ts_split_on=None):
    if ts_split_on:
...
def cv_split(df, n_splits=12, stratify_on=None, group_on=None, ts_split_on=None, val_size=0.0):
    df, groups, y, kfold, n_splits = cv_split_prep(df, n_splits=n_splits, stratify_on=stratify_on, group_on=group_on, ts_split_on=ts_split_on)
...
def train_cv(df, target_col, clf, n_splits=12, stratify_on=None, group_on=None, ordered_split=False, ts_split_on=None, del_cols=tuple(), feature_cols=tuple(), uid_col=None, sample_weight_col=None, eval_size=0.0, framework='auto', fit_params=None, post_fit=None, target_manipulation=None, with_confidence=False, with_tqdm=True, as_numpy=False, n_jobs=1):
    # fix feature names...
    ...
    _, _, _, _, total = cv_split_prep(df, n_splits=n_splits, stratify_on=stratify_on, group_on=group_on, ts_split_on=ts_split_on)
    splits = cv_split(df, n_splits=n_splits, stratify_on=stratify_on, group_on=group_on, ts_split_on=ts_split_on, val_size=eval_size)
    ...
    def predict_confidence(self, X, n_sim=1000, as_list=True):
        ...
        for clf in self.clfs:
            ...
            try:
                ...
                logging.warning("predict_confidence: can't generate clfs, only using original")
                new_clfs = [clf]
        ...
        all_y = [y.reshape(y.shape + (1,)) for y in all_y]
        if len(all_y) == 0:
            raise ValueError("model couldn't predict well for everyone")
