...
def chunked_get_historical_features(
    ...
    **get_historical_features_kwargs,
    ...
    """
    Computes historical features for a given spine. This function breaks the computation into chunks by day based
    on split_size_days.

    This persists any results in a day-partitioned Parquet table at output_path. If this function is interrupted,
    re-running it will pick off from where it left off.

    :param fv_or_fs: the feature view or feature service to compute historical features for
    :param spine: the spine to compute historical features for, as a Spark DataFrame
    :param timestamp_key: the name of the timestamp column in the spine, passed to get_historical_features
    :param output_path: the path to write the historical features to
    :param partition_col: the name of the daily partition column in the output table. Must not overlap with any existing columns
    :param repartition: The number of partitions to repartition the output table to. Affects the numbers of file per partition. If None, no repartitioning is done.
    :param split_size_days: The number of days to compute historical features for at a time.
    :param get_historical_features_kwargs: Additional keyword arguments to pass to get_historical_features
    :return: pyspark.sql.DataFrame: The spine with the historical features joined, partitioned by partition_col.
    """
    spark = SparkSession.getActiveSession()
    row = spine.select(F.expr(f"max({timestamp_key}) as max_ts"), F.expr(f"min({timestamp_key}) as min_ts")).collect()[
        0
    ]
