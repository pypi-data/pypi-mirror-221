...
class Model(ABC):
    ...
    class _Stat:
        ...
        def __str__(self):
            """Convert to string"""
            return f"{self.desc}: [{self.min:.2f}..{self.avg:.2f}..{self.max:.2f}]/{self.cnt}"
    ...
    def __init__(self, model_name: str, model_params: ModelParams):
        ...
        # by default, set format to RAW for all image-type inputs to reduce computations
        self._patch_img_fmt("RAW")
    ...
    def image_backend(self) -> str:
        ...
        `'auto'` means try PIL first, if not installed, try `'opencv'`."""
...
class _CloudServerModel(Model):
    ...
    def __init__(
        ...
        self._patch_img_fmt(
            "JPEG"
        )  # set format to JPEG for all image-type inputs to reduce traffic
