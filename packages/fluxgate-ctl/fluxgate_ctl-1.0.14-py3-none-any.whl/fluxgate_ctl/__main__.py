...
CFG_DEFAULTS = {
    ...
    "settle_delay": 0,
    "; Allowed enhanced_filter values: [ 0, 2, 3, 5, 6 ]": None,
    "enhanced_filter": 0,
...
def main():
    ...
    data_parser.add_argument("dcount", type=int, nargs="?", default=0, help="Number of samples to capture")
    ...
    if args.ip.lower() == "generator":
        args.func = arg_generator
...
def set_config(args):
    ...
    if args.enhanced_filter != 0:
        args.use_sinc3 = False
        args.rate = args.rate & 0x7FFFFFFF
        enhanced_filter = 0x80000000 | (args.enhanced_filter & 0x7)
    else:
        enhanced_filter = 0x00000000
...
def arg_generator(args):
    context = zmq.Context()
    try:
        data_socket = context.socket(zmq.PUB)
        data_socket.bind("tcp://0.0.0.0:5555")
    except zmq.ZMQError:
    ...
    try:
        ...
        ctrl_socket.bind("tcp://0.0.0.0:5556")
    except zmq.ZMQError:
        # No message received, keep looping
        ctrl_socket = None
    ...
    ctrl_reg = [0,0,0,0,0,0xFFFF]
    status_reg = [0, 0, datetime.datetime.now().timestamp()]

    while(not flag.exit()):
        tx_counter = tx_counter + 1
        current_time = time.time_ns()
        status_flag = 0
        mag_id = mag_id + 1 if mag_id < 15  else 0
        x_axis = int(pow(2, 24) / 2) + int((random.random()) * 65536)
        y_axis = int(pow(2, 24) / 2) + int((random.random()) * 65536)
        z_axis = int(pow(2, 24) / 2) + int((random.random()) * 65536)

        msg = struct.pack("<IIIIIIII", tx_counter, int(current_time / 1000000000), int(current_time % 1000000000), status_flag, mag_id, x_axis, y_axis, z_axis)
        data_socket.send(msg, zmq.NOBLOCK)

        # Process messages, we always receive then reply
        try:
            msg = ctrl_socket.recv(zmq.NOBLOCK)
            msg_type, addr, data = struct.unpack("<III", msg)
            resp_code = ord('r')
            if(msg_type == ord('w')):
                print('write')
                if(addr == 0): ctrl_reg[addr] = data
                elif(addr == 1): ctrl_reg[addr] = data
                elif(addr == 2): ctrl_reg[addr] = data
                elif(addr == 3): ctrl_reg[addr] = data
                elif(addr == 4): ctrl_reg[addr] = data
                elif(addr == 5): ctrl_reg[addr] = data
                else: resp_code = ord('e')
            elif(msg_type == ord('r')):
                if(addr == 0): data = ctrl_reg[addr]
                elif(addr == 1): data = ctrl_reg[addr]
                elif(addr == 2): data = ctrl_reg[addr]
                elif(addr == 3): data = ctrl_reg[addr]
                elif(addr == 4): data = ctrl_reg[addr]
                elif(addr == 5): data = ctrl_reg[addr]
                else: resp_code = ord('e')
            elif(msg_type == ord('s')):
                print('status')
                if(addr == 0): data = status_reg[addr]
                elif(addr == 1): data = status_reg[addr]
                elif(addr == 2): data = status_reg[addr]
                else: resp_code = ord('e')
            else:
                resp_code = ord('e')
            resp = struct.pack("<II", resp_code, data)
            ctrl_socket.send(resp)
...
def arg_plot(args):
    ...
    if args.magnetometer == "all":
        ...
        fig.legend(fig.axes[0].lines, ["X", "Y", "Z"], loc="upper center", ncol=3)
...
def arg_data(args):
    ...
    with open(args.file, "wt") as run_file:
        ...
        while not flag.exit() and ((args.dcount == 0) or (args.dcount > 0 and msg_count < args.dcount)):
            try:
                message = socket.recv_multipart(flags=zmq.NOBLOCK)
