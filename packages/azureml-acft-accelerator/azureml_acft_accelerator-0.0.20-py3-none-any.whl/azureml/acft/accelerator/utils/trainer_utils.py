...
class FinetuneCallback(TrainerCallback):
    ...
    def on_train_end(self, args, state, control, **kwargs):
        ...
        if self.azureml_run is None:
            logger.info(f"Local run. Not setting best metric properties")
            return
        ...
        logger.info(f"Best metric properties set on run")
        parent_run = self._should_log_to_parent()
        if parent_run:
            parent_run.add_properties(metric_properties)
            logger.info(f"Best metric properties set on root pipeline run")
        ...
        if parent_run:
            ...
            logger.info(f"automl_child run template set on run")
            parent_run.add_properties(automl_properties)
            logger.info(f"automl_child run template set on root pipeline run")
