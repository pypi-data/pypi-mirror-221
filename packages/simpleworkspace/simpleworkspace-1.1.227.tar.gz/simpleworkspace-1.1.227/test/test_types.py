...
class TimeTests(BaseTestCase):
    ...
    def test_TimeUnit_KnownEqualityChecks(self):
        self.assertEqual(TimeUnit(2, TimeEnum.Second),
                         TimeUnit(1000 * 1000 * 1000 * 2, TimeEnum.NanoSecond))
    ...
    def test_StrictTypeChecks(self):
        TimeUnit(5, TimeEnum.Second) #ok
        with self.assertRaises(TypeError):
            TimeUnit(5, ByteEnum.MegaByte) #bad

        with self.assertRaises(TypeError):
            TimeUnit(1, TimeEnum.Second).To(ByteEnum.MegaByte) #bad
        
        with self.assertRaises(TypeError):
            t1 = TimeUnit(1, TimeEnum.Second)
            t1 += ByteUnit(1, ByteEnum.MegaByte) #adding a non timeunit or float should be bad aswell
        pass
