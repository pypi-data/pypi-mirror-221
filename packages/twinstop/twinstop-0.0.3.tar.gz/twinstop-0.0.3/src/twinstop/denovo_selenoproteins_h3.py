...
def run_tblastx(path_query_file, path_db_file, n_cpu,
...
def chunking(filename, n_chunks, n_lines, func, n_cpu, timeout, path_postchunking_df,
             n, pairwise=False, overlapping=False, ext_orfs=False, chunk_by_key=False):
    '''
            ...
            with mp.Pool(processes=n_cpu) as pool:
                ...
                if pairwise:
                    ...
                    if chunkdf.shape[0] != df_chunk.shape[0]:
                        mis_align = chunkdf[~chunkdf['ID'].isin(df_chunk['ID'])]
                        results = multiprocessing(mis_align, mafft, n_cpu, timeout)
                        df_chunk = pd.concat([df_chunk, results], axis=0, ignore_index=True)
                else:
            ...
            while not iterator.finished:
                ...
                if debugging:
                    if overlapping:
                        ...
                        filtered_out_subset = filtered_out_overlapping[['Chromosome', 'Q_ID', 'Q_fr', 'Score', 'Subj_fr']]
                        filtered_out_subset = filtered_out_subset\
                            .groupby(by=['Chromosome', 'Q_ID', 'Q_fr', 'Subj_fr'], as_index=False).sum()
                        filtered_out_subset.rename(columns={'Score': 'Tot_Score'}, inplace=True)
                        filtered_out_overlapping = filtered_out_overlapping\
                            .merge(filtered_out_subset, on=["Chromosome", "Q_ID", "Q_fr", "Subj_fr"], how='inner')
                        filtered_out_overlapping.sort_values('Tot_Score', ascending=False,
                        ...
                        filtered_out_overlapping = pretty_output(filtered_out_overlapping)
...
def run_extend(chunk, path_query_file, path_subj_file):
    ...
    # print(f'joined_df_1:\n{joined_df}')
    joined_df.drop_duplicates(subset=['Chromosome', 'Q_ID', 'Start', 'Q_align_s', 
                                      'End', 'Q_align_e'], ignore_index=True,
...
def query_subject_dfs(df):
    ...
    subj_df = df.drop(['Q_ID', 'Q_align_s', 'Q_align_e',
                       'Q_fr', 'Q_align_prot_seq'], axis=1)
...
def overlapping_filter(fragments_df, n_cpu):
    ...
    '''
    # write(f'Overlapping')
    fragments_df_subset = fragments_df[['Chromosome', 'Subj_fr', 'Q_ID', 'Q_fr', 'Score']]
    qsf_score = fragments_df_subset.groupby(by=['Chromosome', 'Q_ID', 'Q_fr', 'Subj_fr'], as_index=False)\
        .sum().sort_values('Score', ascending=False, ignore_index=True)
    qsf_score.drop_duplicates(subset=["Chromosome"], inplace=True, ignore_index=True, keep='first')
    qsf_score.drop(labels="Score", axis=1, inplace=True)
    # keeping all alignments of best Q/S
    fragments_df = fragments_df.merge(qsf_score, on=["Chromosome", "Q_ID", "Q_fr", "Subj_fr"], how='inner')
    del qsf_score
...
def pairwise_alignment(extended_hits_df):
    ...
    for i in extended_hits_df.index:
        ...
        alignment = pairwise2.align.globalds(extended_hits_df.at[i, 'Q_align_prot_seq'],
                                             ...
                                             blosum_matrix, -7, -1, one_alignment_only=True)
...
def UGA_alignments(aligned_hits_df):
    ...
    for i, row in aligned_hits_df.iterrows():
        ...
        if not index_u is None:
            ...
            if n_stops_subj >= 2 or n_stops_query >= 2:
                ...
                (list_up_query, list_down_query,
                 list_up_subj, list_down_subj) = list_UGAs(row['Q_align_prot_seq'],
                                                           row['Subj_align_prot_seq'], index_u)
                ...
                if len(list_down_query) != 0:
                    query_cd = len(row['Q_align_prot_seq'][list_down_query[0] + 1:].replace('-', '')) * 3
                ...
                if len(list_up_query) != 0:
                    query_cu = (len(row['Q_align_prot_seq'][:list_up_query[-1]].replace('-', '')) + 1) * 3
                    ...
                    # print(f"row Q_align_prot_seq:\n {row['Q_align_prot_seq']}")
                    # print(f"row Query_CDS:\n {row['Query_CDS']}")
                    row['Query_CDS'] = row['Query_CDS'][(list_up_query[-1] + 1 - gaps_up) * 3:]
                    # print(f"row Query_CDS:\n {row['Query_CDS']}")
                if len(list_down_subj) != 0:
                    subj_cd = len(row['Subj_align_prot_seq'][list_down_subj[0] + 1:].replace('-', '')) * 3
                ...
                if len(list_up_subj) != 0:
                    subj_cu = (len(row['Subj_align_prot_seq'][:list_up_subj[-1]].replace('-', '')) + 1) * 3
                    ...
                    # print(f"row Subj_align_prot_seq:\n {row['Subj_align_prot_seq']}")
                    # print(f"row Subj_CDS:\n {row['Subj_CDS']}")
                    row['Subj_CDS'] = row['Subj_CDS'][(list_up_subj[-1] + 1 - gaps_up) * 3:]
                    # print(f"row Subj_CDS:\n {row['Subj_CDS']}")
            ...
            if not ((row['Subj_CDS'][index_3t_nucl_subj:index_3t_nucl_subj + 3] == 'TGA') and (
    ...
    candidates = candidates.reindex(
        columns=['Density_Score', 'ID', 'Chromosome', 'Start', 'End', 'Subj_fr',
                 ...
                 'Score', 'Evalue', 'Subj_CDS', 'Query_CDS', 'Subj_align_prot_seq',
                 'Q_align_prot_seq'])
...
def find_sec_pos(row):
    ...
    index_u = UGA(row['Q_align_prot_seq'], row['Subj_align_prot_seq'])
    if row['Strand'] == '+':
        sec_start = (index_u - row['Subj_align_prot_seq'][:index_u].count('-')) * 3 + row['Start']
        sec_stop = sec_start + 3
    else:
        sec_start = row['End'] - 3 - (index_u - row['Subj_align_prot_seq'][:index_u].count('-')) * 3
...
def dN_dS(row):
    ...
    #     print(row['Subj_CDS'])
    #     print(row['Query_CDS'])
    changes_dN_dS_u = count_coding_changes(query_aligned_cds[:index_u * 3],
                                           subj_aligned_cds[:index_u * 3])
    changes_dN_dS_d = count_coding_changes(query_aligned_cds[(index_u + 1) * 3:],
                                           subj_aligned_cds[(index_u + 1) * 3:])
    ...
    u_dN_dS = 'NA' if (udN == 0 or udN == 'NA' or udS == 0 or udS == 'NA') else round(udN/udS, 4)
    d_dN_dS = 'NA' if (ddN == 0 or ddN == 'NA' or ddS == 0 or ddS == 'NA') else round(ddN/ddS, 4)
...
def pretty_output(df):
    ...
    for i, row in df.iterrows():
        ...
        pretty_outfile += '\n'
        pretty_outfile += f" ID = {row['ID']},  Score = {row['Score']},  Evalue = {row['Evalue']}\n"
        ...
        if 'dN_dS_up' in df:
            pretty_outfile += f" udN/dS = {row['dN_dS_up']}, ddN/dS = {row['dN_dS_down']}\n"
...
def regression_filter(candidates, lr_filepath, ncpus):
    ...
    if candidates.isnull().values.any():
        ...
        candidates['dN_dS_down'] = candidates['dN_dS_down'].where(~candidates['dN_dS_down'].isna(), maxdnds)
        candidates['dN_dS_down'] = candidates['dN_dS_down'].where(candidates['dN_dS_down'] <= maxdnds, maxdnds)
        # print(candidates['dN_dS_up'].isnull().values.any())
        # print(candidates['dN_dS_down'].isnull().values.any())

    test_ml = candidates[['Density_Score', 'Conservation_up', 'Conservation_down',
                          'dN_dS_up', 'dN_dS_down', 'sec_start', 'sec_end', 'Q_length',
                          'S_length', 'index_u', 'changes_dN_dS_u', 'changes_dN_dS_down',
                          'U+1','U+2', 'U+3', 'U+4', 'U+5','U-1', 'U-2', 'U-3', 'U-4', 'U-5']]

    replaced_col = test_ml[['U+1','U+2', 'U+3', 'U+4', 'U+5','U-1','U-2', 'U-3', 'U-4', 'U-5']].replace(
        {'A': 'nonpolar', 'L': 'nonpolar', 'I': 'nonpolar', 'V': 'nonpolar', 'M': 'nonpolar',
         'C': 'cysteine', 'S': 'polar', 'N': 'polar', 'T': 'polar', 'Q': 'polar', 'D': 'negative',
         'E': 'negative', 'G': 'distort', 'P': 'distort', 'F': 'aromatic', 'Y': 'aromatic', 'W':'aromatic',
         'H': 'positive', 'R': 'positive', 'K': 'positive', 'U': 'stop', '*': 'stop', 'Z': 'outofbounds'})

    test_ml[['U+1','U+2', 'U+3', 'U+4', 'U+5','U-1','U-2', 'U-3', 'U-4', 'U-5']] = replaced_col
    # test_ml.rename(columns={'dN_dS_up': 'dN/dS_up', 'dN_dS_down': 'dN/dS_down'}, inplace=True)

    # X = test_ml.drop(['true_positive'], axis=1)
    X = pd.get_dummies(test_ml, columns=['U+1', 'U+2', 'U+3', 'U+4', 'U+5',
                                         'U-1', 'U-2', 'U-3', 'U-4', 'U-5'], dtype='int')
    if benchmark:
...
def preprocessing_candidates(candidates_df, q_file, subj_file, sp4_subj_gff=None):
    ...
    candidates_df['Subj_prot_seq'] = candidates_df['Subj_align_prot_seq'].apply(
        lambda x: x.replace('-', ''))
    candidates_df[['Subj_aligned_CDS', 'Query_aligned_CDS']] = \
        candidates_df[['Subj_CDS', 'Query_CDS', 'Subj_align_prot_seq', 'Q_align_prot_seq']].apply(
            lambda row: make_aligned_cds(row['Subj_CDS'], row['Query_CDS'], row['Subj_align_prot_seq'],
                                         row['Q_align_prot_seq']), axis=1, result_type='expand')
    candidates_df['index_u'] = candidates_df[['Subj_align_prot_seq', 'Q_align_prot_seq']] \
        .apply(lambda x: UGA(x['Subj_align_prot_seq'], x['Q_align_prot_seq']),
               axis=1, result_type='expand').astype(int)
    candidates_df['changes_dN_dS_u'] = candidates_df[['Query_aligned_CDS', 'Subj_aligned_CDS', 'index_u']] \
        .apply(lambda x: count_coding_changes(x['Query_aligned_CDS'][:x['index_u'] * 3],
                                              x['Subj_aligned_CDS'][:x['index_u'] * 3]),
               axis=1, result_type='expand').agg('sum', axis=1)
    candidates_df['changes_dN_dS_down'] = candidates_df[['Query_aligned_CDS', 'Subj_aligned_CDS', 'index_u']] \
        .apply(lambda x: count_coding_changes(x['Query_aligned_CDS'][(x['index_u'] + 1) * 3:],
    ...
    candidates_df[['Conservation_up', 'Conservation_down']] = candidates_df[
        ['Q_align_prot_seq', 'Subj_align_prot_seq', 'index_u']].apply(
    ...
    def get_element(row, n):
        try:
            return row['Subj_prot_seq'][row['index_u'] +
                                        n - (row['Subj_align_prot_seq']
                                             [:row['index_u']].count('-'))]
        except IndexError:
            return 'Z'

    candidates_df['U+1'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
        .apply(lambda x: get_element(x, 1), axis=1, result_type='expand')
    candidates_df['U+2'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
        .apply(lambda x: get_element(x, 2), axis=1, result_type='expand')
    candidates_df['U+3'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
        .apply(lambda x: get_element(x, 3), axis=1, result_type='expand')
    candidates_df['U+4'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
        .apply(lambda x: get_element(x, 4), axis=1, result_type='expand')
    candidates_df['U+5'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
        .apply(lambda x: get_element(x, 5), axis=1, result_type='expand')
    candidates_df['U+6'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
        .apply(lambda x: get_element(x, 6), axis=1, result_type='expand')
    candidates_df['U+7'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
        .apply(lambda x: get_element(x, 7), axis=1, result_type='expand')
    candidates_df['U+8'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
        .apply(lambda x: get_element(x, 8), axis=1, result_type='expand')
    candidates_df['U+9'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
        .apply(lambda x: get_element(x, 9), axis=1, result_type='expand')
    candidates_df['U+10'] = candidates_df[['Subj_prot_seq', 'index_u', 'Subj_align_prot_seq']] \
...
def main():
    ...
    if benchmark:
        ...
        selenos_annot = sp4_prediction_subj[sp4_prediction_subj.Sel_family.isin(sp4_prediction_query.Sel_family)]
