...
def fake_repo_instance():
    ...
    fake_repo_instance = FakeRepo(TEST_REPO_ID, TEST_HIL_SDK_PATH)
...
def test_run_command_hil_sdk_path_not_found(fake_repo_instance):
    ...
    cli_result = get_run_result()
    assert "hil sdk path not found" in cli_result.stderr
    assert cli_result.exit_code == 2


def test_run_command_hil_sdk_path_not_found_file_exists(fake_repo_instance):

    """Test the result of the run command when the hil/config.yml file exists but the hil_sdk_path key doesn't exist"""

    fake_repo_instance.remove_hil_sdk_path_attr()
