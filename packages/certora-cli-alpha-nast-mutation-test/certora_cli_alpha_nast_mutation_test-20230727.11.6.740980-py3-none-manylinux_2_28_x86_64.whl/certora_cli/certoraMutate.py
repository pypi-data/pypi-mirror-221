...
def range_number_mutants(arg):
    try:
        num = int(arg)
    except ValueError:
        raise argparse.ArgumentTypeError("Must be a number")
    if num < MutantsNumber.MIN_VAL or num > MutantsNumber.MAX_VAL:
        raise argparse.ArgumentTypeError(f"Argument must be < {MutantsNumber.MAX_VAL} and > {MutantsNumber.MIN_VAL}")
    return num
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--gambit_conf", help="The Gambit .conf file to use for Gambit runs.")
    parser.add_argument("--manual_mutations", help="The file containing manual mutations information.")
    parser.add_argument("--num_generated_mutants", type=range_number_mutants,
    ...
    parser.add_argument("--request_timeout", default=10, type=int, help="The timeout in seconds for a web request")
    parser.add_argument("--remote", action='store_true', help="Temporary flag used for testing the new flow")
...
class WebUtils:
    ...
    def put_response_with_timeout(self, url: str, data: Any) -> Optional[requests.Response]:
        """
        ...
        ----
            ...
            data (str | Dict): the data to send
        ...
        for i in range(self.max_timeout_attempts_count):
            try:
                return requests.put(url, data=data, timeout=self.request_timeout) #, headers=headers)
            except Exception:
...
def submit(args: argparse.Namespace) -> None:
    ...
    # call gambit
    generated_mutants = run_gambit(Path(args.gambit_conf), args)
    # todo generate manual mutants
    ...
    if not args.sync:
        ...
        if args.remote:
            ...
            # upload the collect_data to the cloud
            response = send_get_request(web_utils, type=FlowType.ASYNC)
            upload_file_to_cloud_storage(web_utils, response.presigned_url, collect_data)
            generate_report_url(web_utils.ui_url, response.mutation_id, response.anonymous_key)
    else:
...
def collect(args: argparse.Namespace) -> bool:
    ...
    original_results_as_map = {res.name: res.status for res in original_results}
    ...
    # in the new flow this step is redundant
    if not args.remote:
        submit_to_ui(results, web_utils)

    if args.ui_out is not None:
        store_in_a_file(results, args.ui_out)
    else:
        store_in_a_file(results)
...
def send_get_request(web_utils: WebUtils, type: str = "") -> ServerResponse:
    url = web_utils.ui_submit_url + default_cookies[Constants.CERTORA_KEY]
    if type == FlowType.ASYNC:
        url += f"&type={type}"
    resp = web_utils.get_response_with_timeout(url)
    if resp is None:
    ...
    return ServerResponse(resp_obj[Constants.ID], resp_obj[Constants.ANONYMOUS_KEY], resp_obj[Constants.PRE_SIGNED_URL])


def upload_file_to_cloud_storage(web_utils: WebUtils, presigned_url: str,
                                 data: Any) -> None:
    put_resp = web_utils.put_response_with_timeout(presigned_url, json.dumps(data))
    if put_resp is None:
...
def generate_report_url(ui_url: str, mutation_id: str, anonymous_key: str, type: str = "") -> None:
    final_url = f"{ui_url}{mutation_id}&{Constants.ANONYMOUS_KEY}={anonymous_key}"
    message = "Mutation testing report"
    if type == FlowType.ASYNC:
        message += " will be "
    else:
        message += " is "
    message += f"available at {final_url}"
    print(message)


def store_in_a_file(results: List[Any], ui_out: str = "results.json") -> None:
    try:
        with open(ui_out, 'w+') as ui_out_json:
            json.dump(results, ui_out_json)
    except Exception:
        print(f"Failed to output to {ui_out}")


def submit_to_ui(results: List[Any], web_utils: WebUtils) -> None:
    response = send_get_request(web_utils)
    upload_file_to_cloud_storage(web_utils, response.presigned_url, json.dumps(results))
    generate_report_url(web_utils.ui_url, response.mutation_id, response.anonymous_key)
...
def convert_to_mutation_testing_status(treeview_status: TreeViewStatus) -> str:
    if treeview_status is TreeViewStatus.VERIFIED or treeview_status is TreeViewStatus.SKIPPED:
        return MutationTestRuleStatus.SUCCESS
    elif treeview_status in TreeViewStatus.VIOLATED:
        return MutationTestRuleStatus.FAIL
    elif treeview_status in TreeViewStatus.TIMEOUT:
        return MutationTestRuleStatus.TIMEOUT
    elif treeview_status in TreeViewStatus.SANITY_FAILED:
...
def get_results(link: str, fetcher: ReportFetcher) -> Optional[List[RuleResult]]:
    if type(fetcher) == FileFetcher:
        ...
        if job_status != FinalJobStatus.SUCCEEDED and job_status != FinalJobStatus.FAILED:
...
class WebFetcher(ReportFetcher):
    ...
    def get_output_json(self, url: str) -> Optional[Dict[str, Any]]:
        resp = self.web_utils.get_response_with_timeout(get_file_url_from_orig_url(url, "output.json"))
        if resp is None:
            return None
        if resp.status_code != 200:
            print(f"Got bad response code {resp.status_code}")
    ...
    def get_job_status(self, url: str) -> Optional[Dict[str, Any]]:
        resp = self.web_utils.get_response_with_timeout(url.replace(Constants.JOB_STATUS, Constants.JOB_DATA))
        if resp is None:
            return None
        if resp.status_code != 200:
            print(f"Got bad response code {resp.status_code}")
