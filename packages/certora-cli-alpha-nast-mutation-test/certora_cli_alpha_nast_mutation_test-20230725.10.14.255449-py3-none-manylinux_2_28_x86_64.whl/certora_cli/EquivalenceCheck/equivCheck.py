...
from EVMVerifier.certoraBuild import InputConfig, CertoraBuildGenerator, CertoraVerifyGenerator, FunctionSig
...
class EquivalenceChecker:
    ...
    def build_fun_signatures(self) -> None:
        ...
        for i in range(2):
            ...
            logger.debug(f'created CertoraBuildGenerator successfully for {file_i}:{contract_i}')
            cfb.build()
            self.funcs.append(cfb.collect_func_source_code_signatures_source(file_i, contract_i, solc_i))
            logger.debug(f'grabbed functions from {contract_i}: {self.funcs[i]}')
    ...
    def read_input_conf_file(self) -> None:
        ...
        with self.conf_path.open() as conf_file:
            ...
            if len(files) != 2:
                logger.error("INPUT ERROR: must provide two files for reference")
                sys.exit(1)
            self.files = []
            for file in files:
            ...
            if not self.contracts:
                logger.error('INPUT ERROR: Must provide at least two contracts via conf or CLI. Please use "contracts" \
                in the conf file')
                sys.exit(1)
            if not self.functions:
                logger.error('INPUT ERROR: Must provide functions via conf or CLI. Please use "functions" \
                in the conf file')
                sys.exit(1)
            if len(self.contracts) != 2:
                logger.error("INPUT ERROR: not enough contracts provided. Must supply at least 2 contracts")
                sys.exit(1)
            if len(self.functions) != 2:
                logger.error("INPUT ERROR: Must provide at least 2 seperate functions")
                sys.exit(1)
    ...
    def validate_signatures(self) -> None:
        ...
        if sig_a[3] == "view" and sig_b[3] == "view" and \
            ...
            logger.error("cannot yet compare two view functions from two separate contracts")
            sys.exit(1)

        if (sig_a[3] == "pure" and sig_b[3] == "view") or (sig_a[3] == "view" and sig_b[3] == "pure"):
            logger.error("cannot compare a view function to a pure function")
            sys.exit(1)

        if (sig_a[3] not in valid_identifiers) or (sig_b[3] not in valid_identifiers):
            logger.error("functions must both be pure or view functions for now")
            sys.exit(1)
    ...
    def generate_conf_file(self, spec: Path) -> Path:
        ...
        if self.conf_mode:
            ...
            with (self.path / DEFAULT_CONF_PATH).open() as conf_file:
                ...
                for file in self.files:
                    contents[FILES].append(str(file))
                if self.args.bitvector is not None:
                    contents['prover_args'] = ["-smt_bitVectorTheory true"]
    ...
    if args.conf_mode:
        if args.functionA and args.functionB:
            if ':' in args.functionA and ':' in args.functionB:
                ...
                logger.error("INPUT ERROR: Both arguments to CertoraEqCheck should have the same number of components"
            ...
            logger.error('INPUT ERROR: Must give arguments for both contract/functions')
            sys.exit(1)
        else:
            args.contracts = None
            args.functions = None
        ...
        if len(split_a) < 3:
            logger.error(f'wrong input format given, please use file:function:solc for each function \
                            \n Input: {args.arg_a}')
            sys.exit(1)
        if len(split_b) < 3:
            logger.error(f'wrong input format given, please use file:function:solc for each function \
