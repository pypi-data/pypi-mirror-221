...
from Shared.certoraUtils import get_certora_config_dir, get_certora_build_file, get_certora_verify_file, \
    SolcCompilationException, abs_posix_path_relative_to_root_file, convert_path_for_solc_import, \
...
class CertoraBuildGenerator:
    ...
    def collect_func_source_code_signatures_source(self, contract_file: Path,
        ...
        if file_abs_path.suffix != SOL:
            raise CertoraUserInputError(f'May only collect functions from Solidity files: {file_abs_path} received.')
    ...
    try:
        ...
        if mode_has_spec_file(context.mode) and not context.build_only and not ignore_spec_syntax_check:
            ...
            if attr:
                ...
                spec_check_exit_code = run_local_spec_check(with_typechecking=False, rules_to_check=context.rule)
