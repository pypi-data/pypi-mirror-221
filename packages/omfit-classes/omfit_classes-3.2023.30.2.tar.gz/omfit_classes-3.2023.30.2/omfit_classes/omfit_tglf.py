...
class OMFITtglf_QL_flux_spectrum(OMFITdataset, OMFITascii):
    ...
    def load(self):
        ...
        self.update({'QL_data': QL_data_array})
...
class OMFITtglf(OMFITdir):
    ...
    def _parse_QL_flux_spectrum(self):
        ...
        '''
        self['QL_weight_spectrum'] = OMFITtglf_QL_flux_spectrum(
...
def get_sat_params(sat_rule_in, ky, gammas, mts=5.0, ms=128, small=0.00000001, **kw):
    ...
    wE = kx0_factor * array([min(x / 0.3, 1.0) for x in kyi]) * vexb_shear_kx0 / gamma_reference_kx0
    ...
    return kx0_e, SAT_geo1_out, SAT_geo2_out, R_unit, Bt0_out, B_geo0_out, grad_r0_out, theta_out, Bt_out, grad_r_out, B_unit_out
...
def intensity_sat(
    ...
    **kw,
    ...
    if return_phi_params:
        ...
        out = phinorm, QLA_P, QLA_E, QLA_O  # SAT1 or SAT2 intensity

    return out


def reconstruct_kxky_phi(ky_grid, kx_grid, gammas, imode=0, theta_grid=None, make_plots=False, **kw):
    '''reconstruct the TGLF spectral shift model for the saturated kxky_phi spectrum.
    This function can be used for SAT1 or SAT2.
    The SAT2 SS-model also contains the \theta dim, the kxky_phi output may be kxkytheta_phi (if len(theta_grid) > 1)
    ...
    :kwarg mode_num: int, index of mode to use for reconstruction.
    ...
    nmodes = kw["NMODES"]
    if sat_rule_in == 0:
        printe("cannot reconstruct kxky_phi for SAT0")
        return
    ...
    # call intensity_sat,
    # NOTE: the output is totally different when return_phi_params = True, will need to pass actual QL_data for SAT3 parameters
    phi_params = intensity_sat(
        sat_rule_in,
        ky_grid,
        gammas,
        kx0_e,
        ...
        return_phi_params=True,
        **sat_params,
        **kw,
    )

    # What we do from here depends on the sat rule...
    # --- --- --- SAT1 --- --- ---
    # eq. 13 of Staebler et al. "New Paradigm..." (2013)
    # We need <k_x>, cx, cy, gamma_eff, and \alpha_x
    #   - <k_x> is given around eq. 14 of that paper but it should be kx0_e
    #   - gamma_eff is calculated by intensity_sat
    #   - ax (="\alpha_x") constant = 1.15
    #   - ay (="cx/cy") constant = 0.56... (cy is arb.)

    if sat_rule_in == 1:
        # for broadcasting,
        kx_grid = np.atleast_2d(kx_grid).T
        # Eq. 8
        phi00ky = phi_params['gammaeff'][:, imode] / (phi_params["ay"] * ky_grid**2)
        # Eq. 13
        numerator = phi_params['gammaeff'][:, imode] / (1 + (phi_params["ax"] * kx0_e / ky_grid) ** 4)
        denominator = ky_grid**2 + phi_params["ay"] * kx0_e**2 + phi_params["ay"] * (kx_grid - kx0_e) ** 2
        # final,
        kxky_phi = numerator / denominator  # [nkx, nky]

    # --- --- --- SAT2 --- --- ---
    # eq. 34 of Staebler et al. "Verification..." (2021).
    # Therefore we need G(\theta), k_x0, k_x^model, \alpha_x, and \sigma_x
    #   - kx0_e (="k_x0") calculated by get_sat_params.
    #   - kx_width (="k_x^model") calculated by intensity_sat.
    #   - G(theta) is calculated below
    #   - ax (="\alpha_x") constant = 1.21 (in phi_params)
    #   - exp_ax (="\sigma_x") constant = 2.0 (in phi_params)

    if sat_rule_in == 2:
        # eq. 13 of Staebler et al. "Verification..." (2021).
        # peak of the model spectrum theta = 0, kx = 0
        phi00ky = phi_params['gammaeff'][:, imode] / (phi_params['kx_width'] * ky_grid)

        # eqs. 6-20 of Staebler et al. "Verification..." (2021).
        # geometry factor,
        ...
        # apply eq. 19, 20 of Staebler et al. "Verification..." (2021).
        G1 = (B_geo0_out / Bt_out) ** 4  # = (B(0)/B(theta))**4
        Gq = Bt_out / (
            grad_r_out * B_unit_out
        )  # = B(theta)/(|grad r|*B_unit) (Staebler, PPCF, 2021, Eq. 7; NOTE: typo in Staebler, NF, 2021, in text below Eq. 18 should be 1/Gq not Gq)
        ...
        for i, ky0 in enumerate(ky_grid):
            if ky0 < phi_params["kycut"]:
                G_theta[:, i] = sqrt(phi_params["d1"] * G1)
            else:
                term1 = phi_params["d1"] * G1 * phi_params["kycut"]
                term2 = phi_params["b3"] * phi_params["d2"] * G2 * (ky0 - phi_params["kycut"])
                G_theta[:, i] = sqrt((term1 + term2) / ky0)

        # Following broadcasting rules we have,
        # G_theta: [ntheta,nky]
        # phi00ky: [nky,]
        # when multiplied together they yield an [ntheta, nky] array

        # Everything but the kx_grid in the denominator has shape: [nky,]
        # --> make kx_grid 2d with: [nkx,1]
        # when multiplied this will be broadcast to [nkx,nky]
        kx_grid = np.atleast_2d(kx_grid).T

        numerator = (G_theta * phi00ky).T  # [nky, ntheta]
        denominator = (1 + (kx0_e / phi_params['kx_width']) ** 2 + ((kx_grid - kx0_e) / phi_params['kx_width']) ** 2) * (
            1 + (phi_params['ax'] * kx0_e / phi_params['kx_width']) ** phi_params['exp_ax']
        )

        kxky_phi = np.squeeze(numerator / denominator[:, :, np.newaxis])  # [nkx, nky, ntheta]

    # NORMALIZATION: this kxky_phi matrix has the GB-norm.
    # to obtain "real" units it should be multiplied by: (rho_s,unit * Te) / (a * e)

    if make_plots:
        # first plot: parameters for Phi vs. ky,
        fig, ax = plt.subplots(1, 1, num="reconstruct_kxky_phi: phi_params")
        ...
        if sat_rule_in == 1:
            ...
            txt = r"$|_{\theta=0}$"
            ax.semilogx(ky_grid, G_theta[0, :], color='orange', label=r'G($\theta=0$)')

        ax.semilogx(ky_grid, phi00ky, 'c-', label=r'$\Phi_{model}(0,k_y)$' + txt)
        ikx0 = np.argmin(abs(np.squeeze(kx_grid)))
        if kxky_phi.ndim == 3:
            ax.semilogx(ky_grid, kxky_phi[ikx0, :, 0], 'b-', label=r'$\Phi_{ss}(0,k_y)$' + txt)
        else:
            ax.semilogx(ky_grid, kxky_phi[ikx0, :], 'b-', label=r'$\Phi_{ss}(0,k_y)$' + txt)
...
def reconstruct_kxky_n(
    ...
    '''Reconstruct the 2D fluctuating density spectrum: kxky_n using,
        - the TGLF spectral shift model for the saturated potential spectrum (kxky_phi)
        - the QL phase shifts of the density field
    ...
    :kwarg species: int, used to select the proper field for QL weights.
    '''
    ...
    n_intensity = intensity_spectrum["density"].data[imode, :, ispecies]  # (nky,)
    ...
    # TGLF does not write N_weight to a file... but it does write,
    ...
    # ASSUME: QL weights are independant of kx, theta
    # Take sqrt() to get "n" instead of n^2.
    kxky_n = np.sqrt(n_weights) * kxky_phi  # (nky,) * (nkx, nky, [ntheta]) --> (nkx, nky, [ntheta])

    # NORMALIZATION: this kxky_n matrix has the GB-norm.
    # to obtain "real" units it should be multiplied by: (rho_s,unit * ne) / a

    if make_plots:
        fig, ax = plt.subplots(1, 1, num="reconstruct_kxky_n")
        ax.semilogx(ky_grid, phi_bar_out, label='potential_spectrum')
        ax.semilogx(ky_grid, n_intensity, label='intensity_spectrum')
        ax.semilogx(ky_grid, n_weights, 'k-', lw=2, label='QL N-weights')
