...
from src.hirmeos_access_logs_local_driver.logdata import LogStream, Request
from src.hirmeos_access_logs_local_driver.process_download_logs import make_filters
...
class TestLogData(TestCase):
    ...
    def test_apache_log_stream_wrong_request_and_wrong_structure_raises_error(
        ...
        with self.assertRaises(ValueError) as err:
            ...
            result = result.return_output()
        self.assertEqual(
            str(err.exception), "There wasn't any match with the url"
        )
    ...
    def test_apache_log_stream_the_ip_address_is_wrong_will_raise_error(
        self,
    ) -> None:
        """Test the class LogStream fails because there is no match.
        The file structure is wrong and the request is wrong."""
        ...
        self.assertEqual(
            str(err.exception),
            (
                "('the line:', 'cjs.testing-url.com:123 99.999.999.999 - - ["
                '02/Jun/2023:04:55:37 +0000] "GET /testing-url/10.3456768/cjs.'
                'abcde.1234/image/1234/download/ HTTP/1.1" 200 123456 "https://'
                'www.google.com/" "Mozilla/5.0 (Linux; Android 13; testPhone'
                ' ABCDEF) TestPhone/123.45 (KHTML, like Gecko) TestBrowser/'
                '21.0 Chrome/120.3.4567.890 Mobile Safari/537.36"'
                "', 'does not hold a valid ip addrees')"
            )
        )
    ...
    def test_nginx_log_stream_raises_error_no_matches_timestamp(self) -> None:
        """Test the nginx logs are processed as well
        the four lines will be matched."""
        content = str.encode(
            '25.123.123.3 - - [06/Jul/2023:20:15:19 +0000] "GET '
    ...
    def test_nginx_log_stream_the_ip_address_is_wrong_will_raise_error(
        ...
        self.assertEqual(
            str(err.exception),
            (
                "('the line:', '999.999.999.999 - - [06/Jul/2023:"
                '15:04:34 +0000] "GET /test.txt HTTP/1.0" 302 555 "'
                '-" "Mozitest (http://Moziddler.io/about)"'
                "', 'does not hold a valid ip addrees')"
            )
        )
