...
class ThreadCategoryTreeServices:
    ...
    def get_category_classify_tree(params=None):
        ...
        params = format_params_handle(
            ...
            alias_dict={"name": "name__contains", "id_list": "id__in", "category_value": "value"}
        )

        # 第一步，把类别列表全部读出来
        category_set = ThreadCategory.objects.filter(**params).annotate(category_value=F('value')).order_by('sort')
        ...
        # 第三步，获取分类并简历映射
        classify_set_list = list(ThreadClassify.objects.filter(**params).annotate(classify_value=F('value')).order_by('sort').values(
            "id",
            "name",
            "show",
            "classify_value",
