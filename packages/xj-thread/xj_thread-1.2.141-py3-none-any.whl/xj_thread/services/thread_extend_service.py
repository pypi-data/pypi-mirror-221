...
from ..models import ThreadExtendField, Thread, ThreadExtendData, ThreadMainExtendField
...
class ThreadMainExtendService:
    ...
    def format_params_beforehand(self):
        ...
        for i in self.extend_fields:
            if not i["field_index"] in ThreadMainExtendService.all_main_extend_fields:
                continue
            filter_filed_key = i["field"] + ("|int" if i["field_index"] in ThreadMainExtendService.int_extend_fields else "|str")
    ...
    def validate(self, params: dict):
        ...
        field_map = {i["field_index"]: i for i in self.extend_fields if i["field_index"] in ThreadMainExtendService.all_main_extend_fields}
        for field_index, map in field_map.items():
            # 判断字符串字段是否超出长度
            if params.get(field_index) and field_index in self.char_extend_fields and len(params.get(field_index)) > 255:
