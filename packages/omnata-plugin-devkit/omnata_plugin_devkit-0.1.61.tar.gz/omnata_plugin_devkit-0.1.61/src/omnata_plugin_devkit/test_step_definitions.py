...
def step_impl_outbound_sync(context):
    ...
    outbound_sync_request._prebaked_record_state = context.source_records # pylint: disable=protected-access
    context.error = None
    try:
...
def step_impl_inbound_sync(context):
    ...
    try:
        ...
        if len(inbound_sync_request._apply_results) == 0: # pylint: disable=protected-access
            raise ValueError("No results returned from sync_inbound")
        context.sync_inbound_result = inbound_sync_request._apply_results # pylint: disable=protected-access
    except Exception as exception: # pylint: disable=broad-except
        context.error = exception
        logging.exception(exception)
...
def step_impl_use_plugin_class(context, plugin_class, plugin_module):
    ...
    sys.path.insert(
        0, os.path.abspath(os.path.join(context._root["config"].base_dir, "..")) # pylint: disable=protected-access
    )
...
def step_impl_use_http_recordings(context, filename):
    ...
    file_name = os.path.join(
        context._root["config"].base_dir, "vcr_cassettes", filename # pylint: disable=protected-access
    )
    ...
    if "VCRPY_LOG_LEVEL" in os.environ:
        ...
        vcr_log.setLevel(logging._nameToLevel[os.environ["VCRPY_LOG_LEVEL"]]) # pylint: disable=protected-access
