...
from omnata_plugin_runtime.omnata_plugin import (OmnataPlugin, PluginInfo,
                                                 PluginManifest)
  ...
  ;
    ...
    def upload_plugin(
        ...
        if os.path.exists(os.path.join(plugin_directory, icon_file_name)):
            ...
            with open(os.path.join(plugin_directory, icon_file_name),encoding="utf-8") as f:
                icon_source = f.read()
        else:
        ...
        with open(install_script, "w", encoding="utf-8") as setup_script:
            ...
            if os.path.exists(function_handlers_path):
                ...
                snowpark_package(pypi_download='yes',check_anaconda_for_pypi_deps=True,package_native_libraries='yes')
            ...
            if os.path.exists("setup_script.sql"):
                ...
                with open("setup_script.sql", "r", encoding="utf-8") as custom_setup_script:
        ...
        if os.environ.get('OMNATA_LATEST_PLUGIN_INFO_DATABASE',None) is not None:
            if os.environ.get('OMNATA_LATEST_PLUGIN_INFO_SCHEMA',None) is None:
                raise ValueError('OMNATA_LATEST_PLUGIN_INFO_SCHEMA must be set if OMNATA_LATEST_PLUGIN_INFO_DATABASE is set')
            ...
            print(f"Persisting plugin info to {os.environ['OMNATA_LATEST_PLUGIN_INFO_DATABASE']}.{os.environ['OMNATA_LATEST_PLUGIN_INFO_SCHEMA']}.OMNATA_PLUGIN_INFO")
            self.session.sql(f"""create database if not exists {os.environ['OMNATA_LATEST_PLUGIN_INFO_DATABASE']}""").collect()
            self.session.sql(f"""create schema if not exists {os.environ['OMNATA_LATEST_PLUGIN_INFO_DATABASE']}.{os.environ['OMNATA_LATEST_PLUGIN_INFO_SCHEMA']}""").collect()
            self.session.sql(self.plugin_info_udf_definition(
                    manifest=manifest,
                    anaconda_packages=anaconda_packages,
                    bundled_packages=other_packages,
                    icon_source=icon_source,
