...
Track = Tuple[int, str, str, Tuple[List[float], List[float]], List[int], int]
Ground = Tuple[Tuple[float, float, float, float, float, float], float, float]
Shape = Tuple[int, int]
...
def st_track_analysis(
    ...
    """
    Create and return a Streamlit component for track analysis.

    Parameters:
        date (datetime or str): The date of the track analysis.
...
if not _RELEASE:
    ...
    def process_tracks(tracks) -> List[Track]:
        ...
        for group_id, group_data in grouped:
            # Extract required information for each group
            ...
            track_type = group_data["Type"].iloc[0]
