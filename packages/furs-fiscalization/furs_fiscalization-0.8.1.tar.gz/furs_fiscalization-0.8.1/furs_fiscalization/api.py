...
import uuid
import datetime
from furs_fiscalization.base_api import FURSBaseAPI
...
class FURSEchoAPI(FURSBaseAPI):
    ...
    def echo(self,
        ...
        self.is_server_accessible(message)
...
class FURSBusinessPremiseAPI(FURSBaseAPI):
    ...
    def register_immovable_business_premise(self,
        ...
        response_message = self._send_request(path=REGISTER_BUSINESS_UNIT_PATH, data=message)
        response = {'Response': response_message['BusinessPremiseResponse'],
                    'DateTime': response_message['BusinessPremiseResponse']['Header']['DateTime'],
                    'MessageID': response_message['BusinessPremiseResponse']['Header']['MessageID'],
                    'Status': True}
        return response
...
class FURSInvoiceAPI(FURSBaseAPI):
    ...
    def __init__(self, low_tax_rate=9.5, high_tax_rate=22, lowered_tax_rate=5, *args, **kwargs):
        """
        ...
        :param lowered_tax_rate:  (float) - Defaults to 5 for 5%
        :param args:
        :param kwargs:
        :return:
        """
        ...
        self.lowered_tax_rate = lowered_tax_rate
    ...
    def get_invoice_eor(self,
        ...
        :param lowered_tax_rate_base:
        :param lowered_tax_rate_amount:
        ...
        if low_tax_rate_base or high_tax_rate_base or lowered_tax_rate_base:
            tax_spec['VAT'] = self._build_tax_specification(low_tax_rate_base=low_tax_rate_base,
                                                            ...
                                                            lowered_tax_rate_base=lowered_tax_rate_base,
                                                            lowered_tax_rate_amount=lowered_tax_rate_amount)
        ...
        response_message = self._send_request(path=INVOICE_ISSUE_PATH, data=message)
        response = {'Response': response_message['InvoiceResponse'],
                    'DateTime': response_message['InvoiceResponse']['Header']['DateTime'],
                    'MessageID': response_message['InvoiceResponse']['Header']['MessageID'],
                    'UniqueInvoiceID': response_message['InvoiceResponse']['UniqueInvoiceID'],
                    'Status': True}
        return response
    ...
    def _build_tax_specification(self,
                                 ...
                                 lowered_tax_rate_base,
                                 lowered_tax_rate_amount):
        """
        ...
        lowered_tax_spec = {
            'TaxRate': self.lowered_tax_rate,
            'TaxableAmount': lowered_tax_rate_base,
            'TaxAmount': lowered_tax_rate_amount
        }

        return list(filter(lambda x: x['TaxableAmount'] is not None, [low_tax_spec, high_tax_spec, lowered_tax_spec]))
