    ...
    def set_name(self, name):
        ...
        if name in self.graph.operations:
            raise QctrlArgumentsValueError(
                f"There is already a node named '{name}' in the graph.", {"name": name}
            )
...
def _check_node_name(name: str):
    ...
    if not isinstance(name, str):
        raise QctrlArgumentsValueError("The node name must be a str.", {"name": name})
    if "#" in name:
        raise QctrlArgumentsValueError(
