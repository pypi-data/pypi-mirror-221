...
from qctrlcommons.node.namespace import create_namespace_doc
...
class RandomNamespace:
    """
    Operations to create random quantities.
    """
    ...
    @wraps(RandomChoices.create_graph_method())
    def choices(self, *args, **kwargs):
        return RandomChoices.create_graph_method()(self._graph, *args, **kwargs)

    @wraps(RandomColoredNoiseStfSignal.create_graph_method())
    def colored_noise_stf_signal(self, *args, **kwargs):
        return RandomColoredNoiseStfSignal.create_graph_method()(
            self._graph, *args, **kwargs
        )

    @wraps(RandomNormal.create_graph_method())
    def normal(self, *args, **kwargs):
        return RandomNormal.create_graph_method()(self._graph, *args, **kwargs)

    @wraps(RandomUniform.create_graph_method())
    def uniform(self, *args, **kwargs):
        return RandomUniform.create_graph_method()(self._graph, *args, **kwargs)


# Note that the names of all legacy random nodes all start with `random_`
# and we want to drop it in the new client.
RandomNamespaceDoc = create_namespace_doc(
    "random", __name__, [Category.RANDOM_OPERATIONS], lambda x: x[len("random_") :]
)
