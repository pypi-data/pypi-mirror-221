...
global true_blank_thres_peak_fold
global true_blank_hole_thres
global true_blank_objective_thres
pL = 0
saved_img_shape = 0
saved_pL_type = 'blank'
temp_directory = 'D:/MATLAB_temp'
true_blank_thres_peak_fold = 3.5
true_blank_hole_thres = 'default'
true_blank_objective_thres = 4
...
def cell_2D_area(img, global_thres_fold = 'place_holder', hole_thres = 'place_holder', objective_thres = 'place_holder', output_fig = False):
    '''
    ...
    global_thres_fold : positive number.
        Fold of NE_peak to be used as global theshold. The default is 3.5 (set as library global varible). 
    hole_thres : None or positive number.
        Size by pixel of maximum holes you want to close. If input is None, then will use 1/16 of x-resolution multiplied with 1/16 of y-resolution. The default is None. None is recommended. (set as library global varible)
    objective_thres : positive number.
        The minimum size by pixel of non-connected element regarded as cell component. The default is 4. (set as library global varible)
    ...
    '''
    global_thres_fold = true_blank_thres_peak_fold
    hole_thres = true_blank_hole_thres
    objective_thres = true_blank_objective_thres
    if (img.ndim==2):
        img = img-img.min()
        img = img.astype('float')
        thres = NE_peak(img)*true_blank_thres_peak_fold
        img_c = img>thres
...
def test_and_call_areas_of_folder (folder_path, obj_channel):
    ...
    for i in range(len(samples_path)):
        ...
        area_test = cell_2D_area(img, output_fig = True)
