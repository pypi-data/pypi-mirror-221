...
def cell_2D_area(img, global_thres_fold = true_blank_thres_peak_fold, hole_thres = true_blank_hole_thres, objective_thres = true_blank_objective_thres, output_fig = False):
    '''
...
def test_and_call_areas_of_folder (folder_path, obj_channel):
    ...
    for i in range(len(samples_path)):
        ...
        img = ILEE_CSK.rearrange_image_dimension(img, target_channel = obj_channel)
        img = np.amax(img, axis=0)
        img = img.astype('float')
        img = img-img.min()
        img = (img/img.max())*4095
        area_test = ILEE_CSK.cell_2D_area(img, output_fig = True)
