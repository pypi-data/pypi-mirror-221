...
class JSONLogger(PromptLogger):

    def __init__(self, name, level=logging.NOTSET, formatter=None):
        formatter = jsonlogger.JsonFormatter()
        super().__init__(name, level=level, formatter=formatter)
...
class Session:
    ...
    def prompt(self, instructions=None, input=None, output=None, context=None, template=None, llm=None, examples=None, num_examples=1, history=None, tools=None, dryrun=False, retries=3, debug=False, silent=False, **kwargs):
        p = Prompt(
            output=output,
...
def prompt(instructions=None, input=None, output=None, context=None, template=None, llm=None, examples=None, num_examples=1, history=None, tools=None, dryrun=False, retries=3, debug=False, silent=False, **kwargs):
    kwargs = dict(
        instructions=instructions,
        input=input,
        output=output,
...
def load_dotenv_from_multiple_locations(filename='.env'):
    dotenv_paths = []
    ...
    while True:
        ...
        # Stop if we've reached the root or the home directory
        if current_dir == os.path.dirname(current_dir) or current_dir == os.path.expanduser('~'):
            break

        # Go up one level
        current_dir = os.path.dirname(current_dir)

    # Load dotenv files in reverse order
    for dotenv_path in reversed(dotenv_paths):
        load_dotenv(dotenv_path)


if __name__ == '__main__':
    fire.Fire(Session)
    load_dotenv_from_multiple_locations()

    from chromadb.utils import embedding_functions
    DEFAULT_EMBED_FUNCTION = embedding_functions.DefaultEmbeddingFunction()
    llm = MockLLM()
    logger = NotebookLogger('default')
    init(llm=llm, ef=DEFAULT_EMBED_FUNCTION, logger=logger)