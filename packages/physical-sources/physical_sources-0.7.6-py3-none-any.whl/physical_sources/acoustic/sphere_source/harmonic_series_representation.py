...
from physical_sources.src.physical_sources.acoustic.sphere_source.isphere_source import ISphereSource
...
def plot_irregular_surface(locations, desired_x=None, desired_y=None, ax=None, vmin=None, vmax=None, colormap=cm.jet,
    ...
    if vmax is None:
        vmax = true_data['z'].min()
    if vmin is None:
        vmin = true_data['z'].max()
...
class HarmonicSeries(ISphereSource):
    ...
    @staticmethod
    def _build_even_order_matrix(order: int, polar, azimuthal):
        """
    ...
    def make_xml(self, root):
        ...
        for c in self.coefficients:
            ...
            imag_node = xml.etree.ElementTree.SubElement(node, "real")
    ...
    def fit(x, y, order: int):
        ...
        lvl = b.dot(c).real
        r = pearsonr(y, lvl)[0]
        mse = mean_squared_error(y, lvl)
        return HarmonicSeries(c, "", r, mse)


class HarmonicSpectrumNoiseSource:
    """
