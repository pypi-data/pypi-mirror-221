...
class DegreeOfFreedom(Connector):
    def __init__(self, name, *node_amplitude_pairs, DC=0):
        ...
        for node in self.drives:
            if not isinstance(node, LocalDegreeOfFreedom):
                raise Exception(
                    f"Degree of freedom ({name}) input `{node}` should be a {LocalDegreeOfFreedom.__name__}"
                )
            if not (
                node.AC_type == NodeType.ELECTRICAL
                or node.AC_type == NodeType.MECHANICAL
            ):
                raise Exception(
                    f"Degree of freedom ({name}) input `{node}` should be an electrical or mechanical node"
                )
            if AC_type and AC_type != node.AC_type:
                raise Exception(
                    f"Degree of freedom ({name}) input `{node}` should be the same type as other nodes, {AC_type}"
                )
            else:
                AC_type = node.AC_type
    ...
    def _on_add(self, model):
        for dof in self.drives:
            if model is not dof.AC_IN._model and model is not dof.AC_OUT._model:
                raise Exception(
