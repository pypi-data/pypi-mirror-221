...
class DeprecationHelper(object):
    ...
    def __init__(self, old_name, new_name, new_target, until_version):
        ...
        self.msg = f"{self.old_name} is depreciated as of version {self.until_version}"
        self.msg += f": use instead {self.new_name}"
    ...
    def _warn(self):
        ...
        if version.parse(finesse.__version__) >= version.parse(self.until_version):
...
def depreciation_warning(msg, until_version):
    """Function that warns a user about a depreciation. If the current version is past
    when this feature is depreciated it will raise `DeprecationWarning` instead.
    ...
    if version.parse(finesse.__version__) >= version.parse(until_version):
