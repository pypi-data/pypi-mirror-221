...
def mpiRun(command, directory: Path, output="vasp.out"):
    d1 = datetime.now()
    _commands = [
    ...
    cmds = "".join(_commands)
    popen = Popen(cmds, cwd=directory, stdout=PIPE, shell=True)
    popen.wait()
    out = popen.stdout.read().decode("utf-8")
    (directory / output).write_text(out)
    d2 = datetime.now()
    return (d1 - d2).__str__()
