...
from pyimagej.ui.RectangleRoi import RectangleRoi
from pyimagej.ui.CircleRoi import CircleRoi
from pyimagej.ui.LineRoi import LineRoi
from pyimagej.ui.AngleRoi import AngleRoi
from pyimagej.ui.EllipseRoi import EllipseRoi
...
class ImageViewer(QGraphicsView):
    ...
    def mousePressAngleEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            self.roi.setROI(pos.x(), pos.y(), len(self.points))
    ...
    def mouseMoveAngleEvent(self, event: QMouseEvent):
        ...
        if self.handle_move_status:
            self.setCursor(Qt.CursorShape.PointingHandCursor)
            self.roi.move_handle(current_pos, self.handle_index)
            return
        ...
        if self.roi_select_status:
            self.setCursor(Qt.CursorShape.CrossCursor)
            self.roi.setROI(current_pos.x(), current_pos.y(), len(self.points) + 1)
