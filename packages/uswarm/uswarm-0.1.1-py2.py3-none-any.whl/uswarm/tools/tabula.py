...
def isnan(x):
    return str(x) == "nan"
    # return id(x) == id(np.NaN)


def extract_desc(line, remove_ids=True, ignore_bad=False):
    if not isnan(line):
        d = parse_descriptor(line)
        if d:
            if remove_ids:
                return d["text"]
            return d["descriptor"]
...
def merge_strategy_descritores(src, tar=None, remove_ids=True, ignore_bad=False):

    if isinstance(src, str) and (isinstance(tar, str) or tar is None):
        # remove
        foo = 1
        src = [extract_desc(d, remove_ids) for d in src.split("||")]
        if tar is None:
            tar = []
        else:
            tar = [extract_desc(d, remove_ids) for d in tar.split("||")]
        # add only when is missing and preserve 'src' order
        foo = 1
        for s in src:
            if s not in tar:
                tar.append(s)
...
def merge_strategy_join(src, tar):
    return f"{src}||{tar}"


def merge_strategy_min(src, tar):
    return min(str(src), str(tar))


def merge_strategy_keep(src, tar):
    return tar


def merge_strategy_default(src, tar):
    return tar


def merge_columns(df, **ctx):
    """Build a MAP for column renaming.

    - xxx -> xxx[es_ES]

    """
    handlers = {
        "dc.description.abstract": merge_strategy_join,
        "dc.coverage": merge_strategy_descritores,
        "dc.coverage.spatial": merge_strategy_descritores,
        "dc.coverage.temporal": merge_strategy_min,
        "dc.contributor.author": merge_strategy_descritores,
        "dc.identifier": merge_strategy_keep,
        "ridpha.clasificacion": merge_strategy_keep,
    }
    forced_conversion = {
        "dc.contributor.author[es_ES]": merge_strategy_descritores,
        'ridpha.direccion[es_ES]': merge_strategy_descritores,
    }
    mapping = {}
    ...
    #rows_changed = set()
    for col in columns:
        m = re.search(r"(?P<bad>.*?)\[es_ES\]$", col)
        if m:
            bad = m.groupdict()["bad"]
            if bad in columns:
                # set the mapping
                # print(f"mapping: {bad} ---> {col}")
                mapping[bad] = col
                for i, row in df[[bad, col]].iterrows():
                    if isnan(row[bad]):
                        continue
                    if isnan(row[col]):
                        value = row[bad]
                        df.loc[i, col] = row[bad]
                        continue
                    elif row[bad] != row[col]:
                        # try to merge values
                        # print(f"-- Merge -----------------------------------------")
                        # print(row)
                        hnd = handlers.get(bad, merge_strategy_default)
                        src, tar = row[bad], row[col]
                        if hnd == merge_strategy_default:
                            foo = 1
                        df.loc[i, col] = hnd(src, tar)

                        foo = 1

    # force conversion
    for col, hnd in forced_conversion.items():
        #tmp = df[~df.index.isin(rows_changed)][[col]]
        tmp = df[[col]]
        for i, row in tmp.iterrows():
            # print(f"-- Force -----------------------------------------")
            # print(row)
            df.loc[i, col] = hnd(row[col])
