...
# from uswarm.tools.system import lsof, pid_of
# from uswarm.tools import round_robin, reversed_lines, direct_lines, sorted_lines
# from lswarm.loop import Query, Worker, NOP
...
# TODO: Move Repository Base to gutools
...
class iKeyStorage(dict):  # dict subclaass?
    """Interface of Key: Value storage.

    TODO: copy from another uswarm or atlas example
    (that uses __slices__ as well.)
    """
...
# ------------------------------------------------
class KeySpecs:
    def __init__(self, range_="1d", type_="date", pos=0):
        self.range = range_
        self.type = type_
        self.pos = pos

        self.cluster_range = None
        self.BLOCK_SPEC = getattr(self, self.BLOCK_SPEC, {})
        self.BLOCK_SPEC.update(
            {
                "date": self._date_range_info,
            }
        )
        self.info = self.BLOCK_SPEC[type_]
        self.info()

    def _date_range_info(self, current=None):
        if current is None:
            current = datetime.now()

        self.cluster_range = timeframe2delta(self.range)
        delta = self.cluster_range.total_seconds()

        x0 = datetime.fromtimestamp((current // delta + 0) * delta)
        x1 = datetime.fromtimestamp((current // delta + 1) * delta)

        return x0, x1, self.cluster_range


class SparseFile:

    # CLUSTER_SIZE = 3600  # each subclass may define a different value

    """
    Sparse file uses a folder to store all data blocks.

    It also uses some private sub-folders for temporal operations
    or showing some warnings about miss-uses.
    ...
    *opener* must return an open file descriptor (passing os.open as *opener*
    ...
    def __init__(
        ...
        # will be reset in _shift_cluster() but it's better
        # to be declared in __init__()
        self._min_key = self._max_key = self._last_key = None

        # boundaries
        self._bound_left = self._bound_right = None
        self._bound_xleft = self._bound_xright = None
        self._delta = None
        self._set_boundaries(**self.query)

        # file and underlying file from repository (only for writing mode)
        self._file_path = None
        self._file = None
        self._underlaying_path = None
        self._underlaying = None
        self._underlaying_record = None
        self._underlaying_line = None
        self._closed = False

        # set the write record method to use for faster processing
        if self.writable():
            self.write = self._write_record_mixing_cluster_1st_time
        else:
            # TODO: if is not writable, maybe define a new writting
            # TODO: method that raise an exception
            self.write = self._write_record_new_cluster

        # do not create any file until some register is read or written
        # moving to current cluster (steps=0)
        # self._shift_cluster(steps=0)
    ...
    def write(self, record):
        """Dummy function to be replaced by needed one."""
        assert (
            self.writable()
        ), f"This method need to be replaced using mode: {self._mode}"
        raise RuntimeError("You can not write records")

    def _write_record_mixing_cluster_1st_time(self, record):
        """*Write record just when is used by 1st time.*

        - [ ] open both clusters
        - [ ] while record if higher than underlying:
            - [ ] copy registes from underlying to journal
        - [ ] append record in underlying.

        """
        if self._file is None:
            # assert self.bound_left is None
            key = record[self.KEY]
            self._set_boundaries(begin=key, **self.query)

            # assert self.bound_left
            # assert self.bound_right
            # assert self.bound_xleft
            # assert self.bound_xright
            self._shift_cluster(steps=0)

        self.write = self._write_record_mixing_cluster
        self.write(record)

    def _write_record_new_cluster(self, record):
        """Write a record into cluster

        - [x] get the key from record
        - [ ] check if we need to create a new cluster or can write in current one
            - [ ] close and create a new underlying cluster
        - [ ] encode record into writable form
        - [ ] append to the end of cluster

        """
        key = record[self.key]
        if key >= self._bound_right:
            # close and open next boundary
            self._shift_cluster()
        else:
            pass

        if not self._min_key:
            self._min_key = key

        if self._last_key:
            assert key > self._last_key  # monotone crescend
            foo = 1

        self._last_key = key
        self._file.write(self._encode(record))

    def _write_record_mixing_cluster(self, record):
        """*Write a record into cluster.*
        ...
        key = record[self.KEY]
        if key >= self._bound_right:
            # close and open next cluster
            self._shift_cluster()

        # Fast-Fwd: copy all records from underlying to journal
        # until reach the same point
        self._fast_forward(key)

        if not self._min_key:
            self._min_key = key

        if self._last_key and key <= self._last_key:  # monotone crescend
            self._add_review_record(record)
        else:
            # add record to cluster
            # debug(f"++ WR:{record}")
            if self._underlaying and not self.allow_overwriting:
                # check that underlying record and current one match
                # otherwise it will raise an exception
                if record != self._underlaying_record:
                    raise OverwrittenError(
                        f"Overwritting mismatched records is not allowed {record} and {self._underlaying_record}"
                    )
    ...
    # xxxx methods
    # --------------------------------------------
    def _shift_cluster_org(self, steps=1):
        ...
        left, right = self._boundary_clusters()
        ...
        while self._file is None:
            ...
            for _ in range(abs(steps)):
                ...
                self._bound_right = self._shift_keys(self._bound_right, delta)

                if not self.writable() and (
                    self._bound_right > left["end"] or self._bound_left < left["begin"]
                ):
                    self._closed = True
                    return
            ...
            query, self._underlaying_path = repo._get_path(
                self.query, "rt", begin=self._bound_left, end=self._bound_right
            )
            self._underlaying_reader = self.read_underlaying_records()  # generator
            self._underlaying_line = self._underlaying_record = None

            if os.path.exists(self._underlaying_path):
                # open underlying and read first record if exists
                self._underlaying = self._open(self._underlaying_path, "rt")
            else:
                self._underlaying = None
            ...
            if self.writable():
                if self._bound_left:  # is set, then open the cluster
                    ...
                    self._file_path = self._underlaying_path.replace(
                        repo.root, repo.journal
                    )
                    ...
                    if os.path.exists(self._file_path):
                        for _ in range(20):
                            for pinfo in pid_of(self._file_path):
                                time.sleep(0.5)
                                break
                    ...
                    self._file = self._open(self._file_path, self.mode)
                else:
    ...
    def _shift_cluster(self, steps=1):
        ...
        self._min_key = self._max_key = self._last_key = None
        repo = self.repository
        left = repo.stats.get('begin')
        right = repo.stats.get('end')
        # we assume used keys has the product operation define for them.
        # otherwise we need iterate loop
        if steps > 0:
            delta = self._delta
        ...
        while self._file is None:
            ...
            for _ in range(abs(steps)):
                ...
                if not self.writable() and (
                    self._bound_right > left or self._bound_left < right
                ):
                    self._closed = True
                    return
    ...
    def _close(self):
        ...
        if self.mode not in ("r", "rt") and self._underlaying_path != self._file_path:
            # debug(f">> CL: moving journal to {self._underlaying_path}")
            os.renames(self._file_path, self._underlaying_path)
            # self._file_path = self._underlaying_path

    # -------------------------------------------
    def _set_boundaries(self, **query):
        """*Try to set the limits for the file.*"""
        (
            self._bound_left,
            self._bound_right,
            self._delta,
        ) = self._get_block_info(**query)

        # current cluster boundaries and used keys
        self._bound_xleft = self._bound_xleft or self._bound_left
        self._bound_xright = self._bound_xright or self._bound_right

    def _get_block_info(self, **query):
        raise NotImplementedError()

        # timeframe = query.get("timeframe") or self.DEFAULT_TIMEFRAME
        # cluster_size = self.CLUSTER_SIZE.get(timeframe, self.DEFAULT_CLUSTER_SIZE)
        # timeframe = timeframe2delta(timeframe)
        # cluster_range = timeframe * cluster_size
        # delta = cluster_range.total_seconds()

        # query = Query(query)
        # query.update(kw)

        # t0 = query.get("begin") and datetime.fromtimestamp(
        # query["begin"].timestamp() // delta * delta
        # )

        # t1 = query.get("end") and datetime.fromtimestamp(
        # (query["end"].timestamp() // delta + 1) * delta
        # )

        # if t0:
        # t1 = t1 or t0 + cluster_range

        # return t0, t1, cluster_range
    ...
    def _get_cluster_params(self, query):
        """Get the custers that holds the records specified in query"""
        raise NotImplementedError()

    def _get_cluster_info(self, query):
        """Get the boundaries of the cluster that the key belongs"""
        raise NotImplementedError()
    ...
    def close(self):
        not self._closed and self._close()
        self._closed = True
    ...
    def readable(self):
        ...
        return self.mode in ("r", "rt", "ra", "rb", "r+")
    ...
    def _fast_forward(self, key):
        """*Move records from underlying to working file until key is reached.*"""
        if not self._underlaying_record:
            for self._underlaying_record in self._underlaying_reader:
                break

        if self._underlaying_record:
            if self._underlaying_record[self.KEY] < key:
                for self._underlaying_record in self._underlaying_reader:
                    if self._underlaying_record[self.KEY] >= key:
                        break
                    # print(f">> FF: {self._underlaying_line}", end='')
                    self._file.write(self._underlaying_line)
            # print(f">> FF: {key} Done!")
    ...
    def read_records(self):
        if self._file is None:
            ...
            assert self._bound_xleft
            assert self._bound_xright
    ...
    def read_underlaying_records(self):
        while self._underlaying:
            try:
                self._underlaying_line = self._underlaying.readline()
                while self._underlaying_line:
                    record = self._decode(self._underlaying_line)
                    yield record
                    self._underlaying_line = self._underlaying.readline()
                else:
                    break
                    # underlying is exhausted
                    # self._underlaying.close()
                    # self._underlaying = None
            except EOFError as why:
                print(f"ERROR in file: {why}, continue!")
...
class Repository:
    ...
    Then an user open a "file" that contains the query of the subset
    ...
    PATH = "{klass}/{key1}/{key21}.{key22}.{ext}"
    ...
    def _inspect_repository(self, clean=False):
        ...
        for path, d in fileiter(self.root, self.patterns, info="d", relative=True):
            ...
            d["dt1"] = dt1 = "{date1}-{time1}".format_map(d)
            d["dt2"] = dt2 = "{date1}-{time2}".format_map(d)
    ...
    def _convert_query(self, query):
        ...
        for label in self.CONVERTER_KEYS.intersection(result):
    ...
    def _new_stream(self, query, mode="r"):
        ...
        stream._bound_xright = stream._bound_xright or self.stats.get("begin")
        stream._bound_xleft = stream._bound_xleft or self.stats.get("end")
        return stream
    ...
    def _get_path(self, query, mode="r", build=False, root=None, **kw):
        ...
        # info.soft(**dict([(k, None) for k in self.PARAMS]))
        data = self._convert_query(data)

        # build path
        path = self.PATH.format(**data)
...
class DateSparseFile(SparseFile):
    ...
    def _get_block_info(self, **query):
        ...
        if x0:
            # get [) bounds
            x0, x1 = query.get("begin").span(unit)  # bounds [)

        _x1 = query.get("end")
        if _x1 in query:
            if _x1 > x1:
                # get the ")" bound for x1 moving back 'delta'
                # an getting the
                delta = neg_delta(delta)
                x1 = _x1.shift(**delta).span(unit)[1]
            else:
                # x1 in inside same "block", so do nothin
                x1 = _x1
        elif not x0:
            x1 = None
...
class DateRepository(Repository):
    ...
    PATH = "{klass}/{date1}/{time1}-{time2}.{timeframe}.{fmt}.{ext}"
    ...
    def __init__(self, *args, **kw):
        ...
        self._add_patterns(
            # bar/20180903/070000-080000.1s.xz
            r"""(?imux)
            #(?P<klass>[^./]+)(\.|/|-)
            (?P<klass>.*?)/
    ...
    def _convert_query(self, query):
        result = super()._convert_query(query)
        # result['date1'] = parse_arrow(result["begin"])
        # result['date2'] = parse_arrow(result["end"])
        return result


class HistoricalFile(SparseFile):
    ...
    def _get_cluster_params(self, query):
        ...
        - [ ] compute the cluster boundaries.
        - [ ]

        # USED ?

        """
        delta = timeframe2delta(query["timeframe"])
        begin = query.__class__(**query)
        begin["begin"] = None

        end = query.__class__(**query)
...
class HistoricalRepository(Repository):
    """
    ...
    def _get_cluster_info(self, query, **kw):
        timeframe = query.get("timeframe") or self.DEFAULT_TIMEFRAME
        cluster_size = self.CLUSTER_SIZE.get(timeframe, self.DEFAULT_CLUSTER_SIZE)
        timeframe = timeframe2delta(timeframe)
        cluster_range = timeframe * cluster_size
