...
class Runner(SendableMixin, LoggingMixin):
    ...
    def __init__(
        ...
        **run_args,
        ...
        try:
            ...
            self._extra_files_base["send"].append(lpath)
    ...
    def resultfile(self) -> TrackedFile:
        ...
        Result file name
        """
        ext = self.parent.serialiser.extension
        base_name = self._format_filename("result", ext)

        return TrackedFile(self.local_dir, self.run_path, base_name)
    ...
    def stage(
        ...
        **run_args,
        ...
        if not self.assess_run(self._run_args_temp):
        ...
        if extra_files_send is not None:
            self._extra_files_temp["send"] += ensure_list(extra_files_send)
        if extra_files_recv is not None:
            self._extra_files_temp["recv"] += ensure_list(extra_files_recv)
    ...
    def assess_run(self, run_args: dict) -> bool:
        """
