...
void ReInitGlobals()
{
	...
	if (strcmp(xcli_opt, "+NOMANUAL=ON") == 0)
		nomanual = true;
	else
		nomanual = false;
}
...
added key on your secret keyring.\n"));
	...
	if (!addresfork && !use_clipboard)
		if (!emit_radix_64)
			PGPSetFinfo(outputfile, 'Cryp', 'MPGP');
#endif
...
static void do_armorfile(char *armorfile)
{
	...
	for (;;)
	{
		...
		else
		{
			...
			{
				if (checksig_pass == 2)
				{ /* Sigcheck failed: try again with local charset */
					tempf = tempfile(0);
...
static int do_decrypt(char *cipherfile)
{
	...
	do
	{ /* while nested parsable info present */
		if (nested_info)
		{
	...
	else if ((i = compressSignature(header)) >= 0)
	{
		...
		*/
		...
		newname = maybe_force_extension(plainfile, compressExt[i]);
			 ...
			 (is_ctb_type(header[0], CTB_PKE_TYPE) || is_ctb_type(header[0], CTB_SKE_TYPE) || is_ctb_type(header[0], CTB_CKE_TYPE)))
	{
	...
	{ /*      Extract key from key ring
	   ...
	   */
		...
		status = extract_from_keyring(mcguffin, workfile,
									  ringfile, (filter_mode ? FALSE : emit_radix_64));
