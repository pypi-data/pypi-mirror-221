  ...
  for (i = 0; i < sizeof(info) / sizeof(info[0]); ++i)
  {
    ...
    for (j = 0; j < nstrings; ++j)
    {
      ...
      else
      {
        ...
        {
          message("%s  %s %s\n", spaces, (info[i].id[1] == 'C') ? _("Copyright (C)") : _("Produced (P)"), latin1);
        }
        else
          message("%s  %s\n", spaces, latin1);
      }
 ...
 */
static void show_status(struct stats *stats,
 ...
 */
static struct id3_tag *get_id3(struct mad_stream *stream, id3_length_t tagsize,
                               struct input *input)
{
 ...
 */
static enum mad_flow decode_error(void *data, struct mad_stream *stream,
                                  struct mad_frame *frame)
{
  struct player *player = data;
  signed long tagsize;
  ...
  default:
    if (player->verbosity >= -1 &&
        ((stream->error == MAD_ERROR_LOSTSYNC && !player->input.eof) || stream->sync) &&
    ...
    while (playlist->current < playlist->length)
    {
      ...
      case PLAYER_CONTROL_DEFAULT:
            ...
            ++i < playlist->length)
        {
          ...
          int j = (i + rand() % ((playlist->length + 1) / 2)) % playlist->length;
