...
class Functions(Parameters):
    ...
    def set_proyect(self, project_name=None):
        ...
        if Parameters.environment == "Windows":
            Functions.path_downloads = f"{Parameters.current_path}\\projects\\{str(Functions.project_name)}\\src\\downloads"
            Functions.path_files = f"{Parameters.current_path}\\projects\\{str(Functions.project_name)}\\src\\files"
            Functions.path_images = f"{Parameters.current_path}\\projects\\{str(Functions.project_name)}\\src\\images"
            Functions.path_outputs = f"{Parameters.current_path}\\projects\\{str(Functions.project_name)}\\src\\outputs"
            Functions.path_json = f"{Parameters.current_path}\\projects\\{str(Functions.project_name)}\\src\\pages"
            Functions.path_resources = f"{Parameters.current_path}\\projects\\{str(Functions.project_name)}\\src\\resources"
            Functions.path_steps = f"{Parameters.current_path}\\projects\\{str(Functions.project_name)}\\src\\steps"
            Functions.path_config = f"{Parameters.current_path}\\projects\\{str(Functions.project_name)}\\config.yml"
        ...
        if os.environ.get("env"):
            ...
            Parameters.env = "DataTest"
        Functions.full_read_excel(self, Parameters.env)
    ...
    def get_recordset_sqlserver(self, server, base, user, password, query):
        ...
        recordset = []
        cursor = None
        connection = Functions.establish_connection_sqlserver(server, base, user, password)
    ...
    def send_mail(self, receiver_email: list, title, content, file_attach=None):
        ...
        """
        Functions.usuario_pybot_email = Functions.Encryptor("CLAVES", "id", "Email de Pybot", "USER").main()
        ...
        sender_email = Functions.usuario_pybot_email
        password = ""
        if Parameters.environment != "Linux":
            smtp_server = "smtp.office365.com"
            sender_email = Functions.usuario_pybot_email
            password = Functions.Encryptor("CLAVES", "id", "Email de Pybot", "PASS").main()
    ...
    def full_read_excel(self, file_name=None, specific_sheet=None, test_cases_name_list=None):
        ...
        if file_name is None:
            ...
            resource = fr"{Functions.path_resources}\{Functions.file_name}.xlsx"
            book = openpyxl.load_workbook(resource, data_only=True)
        if specific_sheet is None:
        ...
        def main(self):
            ...
            if self.project_file_exists() is True:
                ...
                if data_returned is None:
                    ...
                    if self.backup_file_exists() is True:
                        ...
                        # Si el dato NO existe
                            ...
                            unittest.case.TestCase.skipTest(self, "Parace que ninguno de los archivos contiene la información buscada")
                    else:
                        # Si el archivo backup no existe, crea uno en base a un template vacio
                        unittest.case.TestCase.skipTest(self, "Archivo BackUp no encontrado!, Creando archivo BackUp con template base para futuras ejecuciones")
                ...
                # Si no existe el archivo del proyecto y tampoco existe el archivo backup
                    ...
                    unittest.case.TestCase.skipTest(self, "No se puede continuar sin la presencia de al menos uno de los archivos environment_access.xml")
