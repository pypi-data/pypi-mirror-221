...
@Pipe
def xThreadPoolExecutor(iterable, func, max_workers=5, desc="Processing", unit="it"):  # todo: iterable 太长怎么办
...
def xProcessPoolExecutor(iterable, func, max_workers=5, desc="Processing", unit="it"):
    iterable = list(iterable)
    if max_workers > 1:
        with ProcessPoolExecutor(max_workers) as pool, tqdm(total=len(list(iterable)), desc=desc,
                                                            unit=unit) as pbar:  # 被消费了
            for i in pool.map(func, iterable):
                yield i
                pbar.update()
