        ...
        for name, version in packages:
            ...
            result = run(
                [
                    f"{python_venv_exe}",
                    "-c",
                    f"import pkg_resources as p, packaging.version as v, sys; sys.exit(2 * int(p.require({name})[0].parsed_version._key[1]!= v.parse({version})._key[1]))", # noqa: E501
                ],
                text=True,
                capture_output=True,
            )
...
class RemoteModule(ObjectProxy):
    def __init__(self, python_venv_exe, module, conn=None):
        if conn is None:
            conn = start_remote_client(python_venv_exe, module)
...
def start_remote_client(python_venv_exe, module_name):
    port = None
    proc = None
        ...
        with subprocess.Popen(
            ...
            for l in iter(proc.stdout.readline, ""):
                ...
                if l.startswith("PORT="):
                    ...
                    continue
                print(f"{module_name}: {l}", flush=True)
