...
APPNAME = "Datalayer" if sys.platform in ("win32", "darwin") else "datalayer"
...
def use_platform_dirs() -> bool:
    ...
    We plan for this to default to False in datalayer_core version 5 and to True
    in datalayer_core version 6.
    """
...
def datalayer_data_dir() -> str:
    ...
    if sys.platform == "darwin":
        ...
        if appdata:
            return str(Path(appdata, "datalayer").resolve())
        else:
...
if use_platform_dirs():
    ...
    deprecation(
        ...
        "The use of platformdirs will be the default in `datalayer_core` v6"
    )
    if os.name == "nt":
        programdata = os.environ.get("PROGRAMDATA", None)
        if programdata:
            SYSTEM_DATALAYER_PATH = [pjoin(programdata, "datalayer")]
        else:  # PROGRAMDATA is not defined by default on XP.
            SYSTEM_DATALAYER_PATH = [os.path.join(sys.prefix, "share", "datalayer")]
    else:
...
ENV_DATALAYER_PATH: List[str] = [os.path.join(sys.prefix, "share", "datalayer")]
...
def datalayer_path(*subdirs: str) -> List[str]:
    ...
    if site.ENABLE_USER_SITE:
        ...
        if userbase:
            userdir = os.path.join(userbase, "share", "datalayer")
...
if use_platform_dirs():
    ...
    if os.name == "nt":  # noqa
        programdata = os.environ.get("PROGRAMDATA", None)
        if programdata:  # noqa
            SYSTEM_CONFIG_PATH = [os.path.join(programdata, "datalayer")]
        else:  # PROGRAMDATA is not defined by default on XP.
            SYSTEM_CONFIG_PATH = []
    else:
        SYSTEM_CONFIG_PATH = [
...
ENV_CONFIG_PATH: List[str] = [os.path.join(sys.prefix, "etc", "datalayer")]
...
def datalayer_config_path() -> List[str]:
    ...
    if site.ENABLE_USER_SITE:
        ...
        if userbase:
            userdir = os.path.join(userbase, "etc", "datalayer")
            if userdir not in user:
                user.append(userdir)
