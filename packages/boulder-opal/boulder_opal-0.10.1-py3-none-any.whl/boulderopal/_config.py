...
def get_default_router() -> ApiRouter:
    """Returns the default router that the Boulder Opal
    client uses.
    """
...
def get_config() -> CoreClientSettings:
    """Returns the global Boulder Opal settings."""
    return CoreClientSettings(router=get_default_router, product=Product.BOULDER_OPAL)


def configure(**kwargs):
    """Updates the global Boulder Opal settings. See :class:`CoreClientSettings`
...
def configure_api(api_url: str, oidc_url: str):
    """Convenience function to configure Boulder Opal for API
    routing.
...
def configure_local(resolver: "BaseResolver"):  # type: ignore
    """Convenience function to configure Boulder Opal for local
    routing.
