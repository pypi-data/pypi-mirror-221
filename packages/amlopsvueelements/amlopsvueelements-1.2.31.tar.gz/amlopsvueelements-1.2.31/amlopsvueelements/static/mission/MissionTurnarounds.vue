<template>
  <div id="mission-turnarounds" :class="$style['mission-turnarounds']">
    <div :class="$style['mission-turnarounds__table-wrapper']">
      <p :class="$style['mission-turnarounds__title']">Mission Turnarounds</p>
      <div ref="tableWrapperRef" class="overflow-x-auto !overflow-y-visible h-[calc(100vh-200px)]">
        <table
          class="relative"
        ...
        >
          <thead class="sticky top-0 left-0 z-50" :class="$style['service-table__header']">
            <tr :class="$style['service-table__header-row']">
              <th>Flightlegs</th>
              <th v-for="(item, index) in computeLegs" :key="index">{{ item }}</th>
              <th></th>
            </tr>
            <tr :class="$style['service-table__header-row']">
              <th>Service</th>
    ...
    <div :class="$style['mission-turnarounds__actions']">
      <UButton :class="[$style['mission-turnarounds__add-service']]" @click="onClickAddServiceRow">
        <span>+ Add service row</span>
      </UButton>
      <div class="flex items-center gap-x-2">
        <UButton
          :class="[$style['mission-turnarounds__add-service']]"
          @click="onUpdateGroundServicing"
        >
          <span>Update Ground Servicing</span>
        </UButton>
      </div>
    </div>
...
const { groundServicing, isFetchingTurnarounds, computeLegs, turnaroundsModel, generateServices } =
...
const onClickAddServiceRow = () => {
  turnaroundsModel.value.push({
    id: null,
    is_deleted: false,
    turnarounds: Array.from({ length: computeLegs.value }).map((_, index) => {
      return {
        ...
        booking_quantity: 0,
        note: '',
        service: groundServicing.value[index]
      }
    })
  })
...
.mission-turnarounds {
  @apply relative flex border border-grey-opacity-800/25 flex-col bg-white min-w-0 rounded-[0.5rem] mb-10 p-2;

  &__title {
    @apply text-center text-xl font-medium text-grey-900 py-2 #{!important};
  }

  &__table-wrapper {
    @apply border border-grey-opacity-800/25 rounded-md;
  }
  .service-table {
    @apply w-full;
    ...
    th,
    td {
      @apply p-3  border-r border-grey-opacity-800/25;
      &:not(:last-child) {
        @apply min-w-[8rem];
      }
    }
    tr {
      @apply border-t border-grey-opacity-800/25;
    }
  ...
  &__add-service {
    @apply flex shrink-0 focus:shadow-none text-white bg-grey-900 mb-0 mt-2 p-2 px-4 w-fit #{!important};
  }
}
