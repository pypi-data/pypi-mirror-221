...
import typer
from beni import bcolor, bfile, bfunc, bstore, btable, btask, bzip, bpath
...
async def upload(
    ...
    async with bpath.useTempFile() as f:
        await bzip.zipFile(f, file)
...
async def _getBucket():
    try:
        ...
        btask.abort('请先执行 beni bin init <ak> <sk> 进行初始化')
