...
class ServiceSubscriptionApiViewSet(mixins.CreateModelMixin,
    ...
    def subscribe(self, request, pk, *args, **kwargs):
        ...
        if service is None:
            logger.info('No service with pk: {}'.format(pk))
            raise Http404
        ...
        if is_staff(requesting_user):
            ...
            if serializer.is_valid():
                ...
                if user_sso_id is not None:
                    ...
                    profile = requesting_user.get_sso_app_profile()

        else:
            profile = requesting_user.get_sso_app_profile()

        if profile is None:
            logger.warning('User has no profile: {}'.format(requesting_user))
            raise Http404
    ...
    def get_queryset(self):
        ...
        if is_authenticated(requesting_user):
            if is_staff(requesting_user):
                ...
                return Subscription.objects.filter(profile=requesting_user.get_sso_app_profile())
        else:
