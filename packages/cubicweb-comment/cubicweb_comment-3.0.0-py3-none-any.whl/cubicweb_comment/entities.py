...
def subcomments_count(commentable):
    return sum(
        [len(commentable.reverse_comments)]
...
class Comment(AnyEntity):
    ...
    fetch_attrs, cw_fetch_order = fetch_config(
        ("creation_date",), "creation_date", order="ASC"
    )
...
class CommentFullTextView(EntityView):
    ...
    def cell_call(self, row, col, indentlevel=0, withauthor=True):
        ...
        # Use reST-like transformation for Markdown as well as it looks better
        # (for list items in particular) that plain text.
        rest = e.content_format in ("text/rest", "text/markdown")
        lines.append(normalize_text(content, 80, indentstr, rest=rest))
...
class CommentFullThreadText(CommentFullTextView):
    ...
    def cell_call(self, row, col):
        ...
        for i, ceid in enumerate(cpath):
            ...
            strings.append(
                comment.view(
                    "fulltext", indentlevel=indentlevel - i, withauthor=i != indentlevel
                ).strip()
                + "\n"
            )
        strings.append(
            "\n%s: %s"
            % (
                self._cw._("i18n_by_author_field"),
                e.dc_creator() or self._cw._("unknown author"),
            )
        )
        strings.append(f"url: {itree.root().absolute_url()}")
        self.w("\n".join(strings))
