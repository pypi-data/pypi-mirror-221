...
from libquery_extensions.british_library.online_gallery._typing import (
    MetadataEntry,
    SourceData,
)
...
def get_publish_date(source_data: SourceData) -> Union[TimePoint, None]:
    if "date" not in source_data:
        return None
    return {"year": int(source_data["date"])}
...
def process(entry: MetadataEntry, img_dir: Union[str, None]) -> ProcessedMetadataEntry:
    ...
    return {
        "uuid": entry["uuid"],
        "authors": None,
        "displayName": entry["sourceData"]["title"],
        "publishDate": get_publish_date(entry["sourceData"]),
        "viewUrl": entry["url"],
