...
class MakeSvgInstance:
    ...
    def __drawAspect(self, r, ar, aspect_dict, aspect_id: str, isAxis=False):
        degA= aspect_dict['p1_abs_pos']
        degB = aspect_dict['p2_abs_pos']
        color = self.colors_settings[f"aspect_{aspect_dict['aspect_degrees']}"] if not isAxis else '#ff0000'
        ...
        for e in range(len(keys)):
            ...
            if self.planets_retrograde[i]:
                if self.planets_settings[i]["name"] == 'Mercury':
                    offset_retrograde_x = -2.9 
                    offset_retrograde_y = 9
                elif self.planets_settings[i]["name"] == 'Venus':
                    offset_retrograde_x = 2.5
                    offset_retrograde_y = 5
                elif self.planets_settings[i]["name"] == 'Mars':
                    offset_retrograde_x = 3
                    offset_retrograde_y = 3
                elif self.planets_settings[i]["name"] == 'Jupiter':
    ...
    def makeTemplate(self):
        ...
        if self.chart_type == "Transit" or self.chart_type == "Composite":
            ...
            td['makeAxis'] = self.__makeAxis(r, (r-self.c1+30))
