    ...
    def setReactions(self):
        ...
        for r in self.core.reactions:
            ...
            if not r.hasCompartment() and len(self.core.compartments) == 1:
                print(
                    'Info: single compartment model: locating \"%s\" in default compartment'
                    % r.name
                )
                C = '@%s' % self.core.compartments[0].getName()
            elif not r.hasCompartment() and len(self.core.compartments) > 1:
                print(
                    'Info: multiple compartments defined but reaction %s is not located in any of them.'
                    % r.name
                )
                ...
                for pwd in self.pw_symbols:
                    if pwd in r.formula:
                        for pwc in self.core.__piecewises__[pwd]:
                            if pwc != 'other':
                                pwreplace += '%s,%s,' % (
                                    self.core.__piecewises__[pwd][pwc][0],
                                    self.core.__piecewises__[pwd][pwc][1],
                                )
    ...
    def setSpecies(self):
        ...
        for f in self.core.species:
            ...
            if not f.hasCompartment() and len(self.core.compartments) == 1:
                print(
                    'Info: single compartment model %s is in default compartment'
                    % f.name
                )
                C = '@%s' % self.core.compartments[0].getName()
            elif not f.hasCompartment() and len(self.core.compartments) > 1:
                print(
                    'Warning: multiple compartments defined but species %s is not located in any of them.'
                    % f.name
                )
                ...
                if len(self.pw_symbols) > 0:
                    ...
                    for pwd in self.pw_symbols:
                        if pwd in p.formula:
                            ...
                            if self.core.__piecewises__[pwd]['other'] != None:
                                pwreplace += (
                                    '%s) ' % self.core.__piecewises__[pwd]['other']
                                )
                            else:
                                pwreplace = pwreplace[:-1] + ')'
                            formula = formula.replace(pwd, pwreplace)
...
class PscToCore(object):
    ...
    def getParsedModel(self):
        ...
        self._PysMod__InitStrings = [
            s.replace('self.', '') for s in self.pscParser.InitStrings
        ]
...
class CoreToSBML(object):
    ...
    def setDescription(self, txt=None):
        ...
        notes += (
            '<p><span style="font-family: Courier New,Courier,monospace;">%s</span></p>\n'
            % txt
        )
        notes += (
            '<p><span style="font-family: Courier New,Courier,monospace;"><a href="http://pysces.sourceforge.net">PySCeS</a> (%s) generated model (%s)</span></p>'
    ...
    def setSpecies(self):
        ...
        for spe in self.core.species:
            ...
            if not spe.hasCompartment() and len(self.core.compartments) == 1:
                ...
                raise UserWarning(
                    "%s does not have a compartment and there are more than 1 to choose from!"
                    % spe.getName()
                )
            ...
            if not rxn.multistoich_enabled:
                ...
                for m in rxn.modifiers:
                    if m.name not in AllReagents:
                        ...
                        self.model.createModifier().setSpecies(m.name)
            formula = (
                rxn.code_string.split('=')[1].replace('self.', '').replace('()', '')
            )
            formula = self.infixPSC2SBML(formula)
            if self.SBML.getLibSBMLVersion() < 40000:
...
class SbmlToCore(object):
    ...
    def getUnits(self):
        ...
        for ud in self.model.getListOfUnitDefinitions():
            ...
            if ud.getNumUnits() == 1:
                ...
                self.__uDict__[ud.getId()].update(
                    {
                ...
                for u in ud.getListOfUnits():
                    self.__uDict__[ud.getId()].update(
                        {
                            self.SBML.UnitKind_toString(u.getKind()): {
                                'multiplier': u.getMultiplier(),
                                'exponent': u.getExponent(),
                                'scale': u.getScale(),
                                'kind': self.SBML.UnitKind_toString(u.getKind()),
    ...
    "event.use_values_from_trigger_time = False" not implemented.
        PySCeS evaluates event assignments at trigger time.
            ...
            if hasTimeS != None:
                ...
                print(
                    'csymbol time defined as \"%s\" in event %s'
                    % (tSymb, trigger.getId())
                )
    ...
    def getId(self, e):
        ...
        if name in self.__reserved__:
            print(
                '%s is a reserved symbol, replacing with %s'
                % (name, self.__reserved__[name])
            )
            self.__Errors__.update(
                {
                    next(self._ecount): 'Reserved symbol %s replaced with %s'
                    % (name, self.__reserved__[name])
                }
            )
            name = self.__reserved__[name]
    ...
    def getParsedModel(self):
        ...
        for comp in self.model.getListOfCompartments():
            ...
            self.__compartments__.update(
                {
                    self.getId(comp): {
                        'name': self.getId(comp),
        ...
        if len(self.model.getListOfSpecies()) < 1:
            self.__Errors__.update(
                {next(self._ecount): 'No free species!? ... help I\'m confused!'}
            )
        ...
        for i in self.model.getListOfSpecies():
            ...
            if self.__DEBUG__:
                print(
                    '%s has only substance units: %s'
                    % (specname, i.getHasOnlySubstanceUnits())
                )
            ...
            if i.getBoundaryCondition() or i.getConstant():
                if i.getConstant() and not i.getBoundaryCondition():
                    print(
                        '%s is set as constant, assuming: BoundaryCondition = True'
                        % specname
                    )
        ...
        NetworkDict = dict(
            [
                ...
                for i in [self.getId(j) for j in reactions]
            ]
        )
        ...
        if len(Punits) > 0:
            self.__Errors__.update(
                {
                    next(self._ecount): 'Parameter units ignored for parameters:\n%s'
                    % Punits
                }
            )
        ...
        if self.model.getNumFunctionDefinitions() > 0:
            for fnc in self.model.getListOfFunctionDefinitions():
                ...
                if hasTimeS != None:
                    ...
                    tSymb = tSymb.replace('>', '').replace('<', '').strip()
                    print(
                        'csymbol time defined as \"%s\" in formula %s'
                        % (tSymb, self.getId(fnc))
                    )
                    SRs = {tSymb: '_TIME_'}
                ...
                self.__functions__.update(
                    {name: {'args': args, 'formula': func, 'name': name}}
                )
        ...
        for i in reactions:
            ...
            if j != None:
                ...
                if hasTimeS != None:
                    ...
                    tSymb = tSymb.replace('>', '').replace('<', '').strip()
                    print(
                        'csymbol time defined as \"%s\" in reaction %s'
                        % (tSymb, self.getId(i))
                    )
                ...
                if len(j.getListOfParameters()) > 0:
                    ...
                    for k in j.getListOfParameters():
                        ...
                        req = req.replace(
                            '@' + self.getId(k) + '@',
                            self.getId(i) + '_' + self.getId(k),
                        )
                        par.append(self.getId(i) + '_' + self.getId(k))
                        init_par.setdefault(
                            self.getId(i) + '_' + self.getId(k), k.getValue()
                        )
                        # local parameters are hashed with reaction name
                        if self.__DEBUG__:
                            print('%s_%s' % (self.getId(i), self.getId(k)), end=' ')
                    if self.__DEBUG__:
                        print('')
                    req = req.replace('@', '')
                else:
                ...
                print(
                    'WARNING: No rate equation for reaction %s in file: %s'
                    % (self.getId(i), self.sbml_file)
                )
                req = None
                    ...
                    self.__Errors__.update(
                        {
                            next(
                                self._ecount
                            ): 'StoichiometryMath (%s) not supported and ignored'
                            % k.getSpecies()
                        }
                    )
        ...
        if len(hasFast) > 0:
            self.__Errors__.update(
                {
                    next(self._ecount): 'Fast attribute ignored for reactions:\n%s'
                    % hasFast
                }
            )
        if len(Punits) > 0:
        ...
        if len(delayignore) > 0:
            self.__Errors__.update(
                {
                    next(self._ecount): 'delay function removed in reactions:\n%s'
                    % delayignore
                }
            )
    ...
    def getRules(self):
        ...
        for rule in self.model.getListOfRules():
            ...
            if hasTimeS != None:
                ...
                tSymb = tSymb.replace('>', '').replace('<', '').strip()
                print(
                    'csymbol time defined as \"%s\" in rule %s'
                    % (tSymb, self.getId(rule))
                )
                InfixParser.setNameStr('', '')
                SRs = {tSymb: '_TIME_'}
                SRs.update(self.__reserved__)
    ...
    def removeCompartmentFromKineticLaw(self, kl):
        ...
        def idxNode(node, idx=0):
            ...
            if node.text != None and node.text.strip() in list(
        ...
        if len(self._comp_idx) > 0:
            for comp_idx in self._comp_idx:
                ...
                if (
                    len(node_idx[cParent].getchildren()) == 3
                    and node_idx[cParent].getchildren()[0].tag
                    == '{http://www.w3.org/1998/Math/MathML}times'
                ):
                    for c in node_idx[cParent].getchildren():
                        if c.tag != '{http://www.w3.org/1998/Math/MathML}times' and c.text.strip() not in list(
                            self.__compartments__.keys()
                        ):
                            ...
                            for cp in node_idx[cParentParent].getchildren():
                                if cp.attrib['idx'] == node_idx[cParent].attrib['idx']:
                                    ...
                                    c.attrib.update(
                                        {
                                            'parent': node_idx[cParentParent].attrib[
                                                'idx'
                                            ]
                                        }
                                    )
                    ...
                    len(node_idx[cParent].getchildren()) == 2
                    and node_idx[cParent].getchildren()[0].tag
                    ...
                    for c in node_idx[cParent].getchildren():
                        if c.text != None and c.text.strip() in list(
                            self.__compartments__.keys()
                        ):
