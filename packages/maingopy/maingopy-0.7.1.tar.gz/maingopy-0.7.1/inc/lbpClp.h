...
/**
* @class LbpClp
* @brief Wrapper for handling the lower bounding problems by interfacing CLP
*
* This class constructs and solves lower bounding problems using CLP (COIN-OR project).
*
*/
class LbpClp: public LowerBoundingSolver {

  public:
    /**
        * @brief Constructor, stores information on the problem and initializes the CLP problem and solver instances.
        *
  ...
  protected:
    ...
    /**
        * @brief Function for checking if a specific option has to be turned off for a given lower bounding solver. We don't have to turn anything off for CLP
        */
    void _turn_off_specific_options(){};

#ifdef LP__OPTIMALITY_CHECK
    /**
        * @brief Function for checking if the solution point returned CLP is really infeasible using Farkas' Lemma
        *
    ...
    /**
        * @brief Function for checking if the solution point returned by CLP solver is really feasible
        *
    ...
    /**
        * @brief Function for checking if the solution point returned by CLP solver is really optimal using strong duality
        *
        * @param[in] currentNode is holding the current node in the branch-and-bound tree
  ...
  private:
    /**
        * @brief Function for taking care of memory management by terminating CLP (either called from destructor or when an exception is thrown)
        */
    void _terminate_Clp();

    /**
        * @brief Function for initializing the CLP-internal matrix
        */
    ...
    ClpSimplex _clp;          /*!< CLP solver object  */
    CoinPackedMatrix _matrix; /*!< CLP internal matrix representation */

    unsigned _numrows;    /*!< number of matrix rows */
    unsigned _numcolumns; /*!< number of matrix columns */

    double *_objective;      /*!< coefficients in objective function */
    double *_lowerRowBounds; /*!< lower bounds on matrix rows (-inf) */
    double *_upperRowBounds; /*!< upper bounds on matrix rows */

    double _eta;      /*!< auxiliary objective variable  */
    double _etaCoeff; /*!< current coefficient of eta in the LP, needed for equilibration  */

#ifdef LP__OPTIMALITY_CHECK
    std::vector<std::vector<double>> _dualValsObj;                /*!< auxiliary variable for optimality check */
    std::vector<std::vector<double>> _dualValsIneq;               /*!< auxiliary variable for optimality check */
    std::vector<std::vector<double>> _dualValsEq1;                /*!< auxiliary variable for optimality check */
    std::vector<std::vector<double>> _dualValsEq2;                /*!< auxiliary variable for optimality check */
    std::vector<std::vector<double>> _dualValsIneqRelaxationOnly; /*!< auxiliary variable for optimality check */
    std::vector<std::vector<double>> _dualValsEqRelaxationOnly1;  /*!< auxiliary variable for optimality check */
    std::vector<std::vector<double>> _dualValsEqRelaxationOnly2;  /*!< auxiliary variable for optimality check */
    std::vector<std::vector<double>> _dualValsIneqSquash;         /*!< auxiliary variable for optimality check */
#endif
    /**@}*/
};
