    ...
    _LBS(LBSIn), _UBS(UBSIn), _nvar(variables.size()), _maingoSettings(settingsIn), _logger(loggerIn), _nvarWOaux(nvarWOauxIn)
{
    ...
    MAiNGO_IF_BAB_MANAGER
        ...
        _informedWorkerAboutDataset = std::vector<bool>(_nProcs - 1, true);    // Initial dataset is always build - no need to send it
...
BranchAndBound::solve(babBase::BabNode &rootNodeIn, double &solutionValue, std::vector<double> &solutionPoint, const double preprocessTime, double &timePassed)
{
    try {
        ...
        MAiNGO_IF_BAB_MANAGER
            ...
            rootNodeIn = babBase::BabNode(rootNodeIn.get_pruning_score(), rootNodeIn.get_lower_bounds(), rootNodeIn.get_upper_bounds(), 0 /*index data set*/, 1 /*ID*/, 0 /*depth*/, rootNodeIn.get_augment_data());
...
BranchAndBound::_update_incumbent_and_fathom(const double solval, const std::vector<double> sol, const unsigned int currentNodeID)
{
    ...
    if (solval < _ubd) {
        ...
        _ubd             = solval;
        _incumbent       = sol;
        _incumbentNodeId = currentNodeID;
...
BranchAndBound::_augment_dataset(babBase::BabNode &currentNode)
{
    ...
    unsigned int noData    = (*_datasets)[0].size();
    unsigned int noDataOld = (*_datasets)[newIndex].size();

    // Use next larger reduced dataset
    newIndex++;
    if (newIndex >= (*_datasets).size()) {    // If dataset doesn't exist: build it

        // Simplest rule: augment by x%*nData data points, but at least 1 point
        int noAddedData = std::round((double)_maingoSettings->growing_augmentPercentage * (double)noData);
        noAddedData     = std::max(1, noAddedData);

        if (noDataOld + noAddedData < _maingoSettings->growing_dataSizeTol * noData) {    // Add a new dataset

            // Simplest rule: new dataset extends previous dataset
            std::set<unsigned int> newDataset = (*_datasets)[newIndex - 1];

            std::set<unsigned int> unusedDataPoints;
            std::set_difference((*_datasets)[0].begin(), (*_datasets)[0].end(), (*_datasets)[newIndex - 1].begin(), (*_datasets)[newIndex - 1].end(), std::inserter(unusedDataPoints, unusedDataPoints.end()));

            // Pick (pseudo-)random data points
            std::srand(5 + _iterations);    // For reproducible results
            ...
            while (count < noAddedData) {
                ...
                unsigned int idxRand = tmpRand * unusedDataPoints.size();
                ans                  = newDataset.insert(*(std::next(unusedDataPoints.begin(), idxRand)));
                if (ans.second == true) {
                    count++;
                }
            }
...
#ifdef HAVE_MAiNGO_MPI
            // There is a new dataset for all future worker processes
            _informedWorkerAboutDataset = std::vector<bool>(_nProcs - 1, false);
#endif
        }
        else {    // Next dataset is the full dataset
            newIndex = 0;
        }
    }
