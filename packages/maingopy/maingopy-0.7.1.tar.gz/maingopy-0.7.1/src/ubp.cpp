...
// passes index of new dataset to respective DagObj routine
void
UpperBoundingSolver::pass_data_position_to_solver(const std::shared_ptr<std::vector<std::set<unsigned int>>> datasetsIn, const unsigned int indexFirstDataIn)
{
...
// needed to ensure correct order of subgraphs in storedSubgraphs in parallel version
void
UpperBoundingSolver::update_datasets_and_storedSubgraphs(std::set<unsigned int> &newDataset)
{
    size_t ndataNew = newDataset.size();

    bool datasetExists = false;
    for (int i = (*_DAGobj->datasets).size() - 1; i > -1; i--) {
        if ((*_DAGobj->datasets)[i].size() == ndataNew) {
            datasetExists = true;
            break;
        }
    }
    if (!datasetExists) {
        (*_DAGobj->datasets).push_back(newDataset);
    }

    if (_DAGobj->storedSubgraph.size() != (*_DAGobj->datasets).size()) {
        // If we do not have a subgraph for each dataset

        for (auto idx = _DAGobj->storedSubgraph.size(); idx < (*_DAGobj->datasets).size(); idx++) {
            _DAGobj->add_subgraph_for_new_dataset(idx);
        }
    }
}
