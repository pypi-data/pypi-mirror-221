...
namespace melon {
    ...
    */
	...
	public:
        ...
        */
        std::vector<double> get_normalized_observations() { return _data->Y; }
	...
	template<typename T>
	int GaussianProcess<T>::get_input_dimension() {
	...
	template<typename T>
	int GaussianProcess<T>::get_number_of_training_data_points() {
        if (!MelonModel<T>::_modelLoaded) {
            throw MelonException("  Error: No Gaussian process loaded.");
        }
	...
	template<typename T>
	double GaussianProcess<T>::get_minimum_of_training_data_outputs() {
        if (!MelonModel<T>::_modelLoaded) {
            throw MelonException("  Error: No Gaussian process loaded.");
        }
	...
	template<typename T>
	double GaussianProcess<T>::get_maximum_of_training_data_outputs() {
		if (!MelonModel<T>::_modelLoaded) {
			throw MelonException("  Error: No Gaussian process loaded.");
		}
	...
	template<typename T>
	std::vector<double> GaussianProcess<T>::get_observations() {
        if (!MelonModel<T>::_modelLoaded) {
            throw MelonException("  Error: No Gaussian process loaded.");
        }
	...
	template <typename T>
	T GaussianProcess<T>::calculate_prediction_reduced_space(std::vector<T> input) {
	...
	template <typename T>
	T GaussianProcess<T>::calculate_variance_reduced_space(std::vector<T> input) {
		std::vector<T> dummyConstraints;
	...
	template <typename T>
	T GaussianProcess<T>::calculate_prediction_full_space(std::vector<T> input, std::vector<T> internalVariables, std::vector<T>& constraints) {
		try {
            if (!MelonModel<T>::_modelLoaded) {
                throw MelonException("  Error while calculating Gaussian process prediction: No model was loaded yet.");
            }
	...
	template <typename T>
	T GaussianProcess<T>::calculate_variance_full_space(std::vector<T> input, std::vector<T> internalVariables, std::vector<T>& constraints) {
		try {
            if (!MelonModel<T>::_modelLoaded) {
                throw MelonException("  Error while calculating Gaussian process variance: No model was loaded yet.");
            }
    ...
    template <typename T>
    std::pair<T,T> GaussianProcess<T>::calculate_prediction_and_variance_full_space(std::vector<T> input, std::vector<T> internalVariables, std::vector<T>& constraints) {
        try {
            if (!MelonModel<T>::_modelLoaded) {
    ...
    template <typename T>
    template <typename RandomAccessIterator>
    std::vector<T> GaussianProcess<T>::_calculate_covariance_vector(std::vector<T> input, RandomAccessIterator& internalVariables, const bool fullSpace, std::vector<T>& constraints) {
	...
	template <typename T>
    template <typename RandomAccessIterator>
	T GaussianProcess<T>::_calculate_prediction(std::vector<T> covarianceVector, RandomAccessIterator& internalVariables, const bool fullSpace, std::vector<T>& constraints) {
	...
	template <typename T>
    template <typename RandomAccessIterator>
	T GaussianProcess<T>::_calculate_variance(std::vector<T> covarianceVector, RandomAccessIterator& internalVariables, const bool fullSpace, std::vector<T>& constraints) {
		...
		T normalizedResult = _data->K.at(0).at(0) - ki;
        if (fullSpace) {
	...
	template <typename T>
	unsigned int GaussianProcess<T>::get_number_of_full_space_variables_prediction() {
        if (!MelonModel<T>::_modelLoaded) {
            throw MelonException("  Error: No Gaussian process loaded.");
        }
	...
	template <typename T>
	void GaussianProcess<T>::get_full_space_variables_prediction(unsigned int& variableNumber, std::vector<std::string>& variableNames, std::vector<std::pair<double,double>>& variableBounds) {
        if (!MelonModel<T>::_modelLoaded) {
            throw MelonException("  Error: No Gaussian process loaded.");
        }
	...
	template <typename T>
	unsigned int GaussianProcess<T>::get_number_of_full_space_variables_variance() {
        if (!MelonModel<T>::_modelLoaded) {
            throw MelonException("  Error: No Gaussian process loaded.");
        }
    ...
    template <typename T>
    void GaussianProcess<T>::get_full_space_variables_variance(unsigned int& variableNumber, std::vector<std::string>& variableNames, std::vector<std::pair<double, double>>& variableBounds) {
        if (!MelonModel<T>::_modelLoaded) {
            throw MelonException("  Error: No Gaussian process loaded.");
        }
    ...
    // Get the number of internal variables used in the calculation of the prediction
    template <typename T>
    unsigned int GaussianProcess<T>::get_number_of_full_space_variables_prediction_and_variance() {
        if (!MelonModel<T>::_modelLoaded) {
            throw MelonException("  Error: No Gaussian process loaded.");
        }
    ...
    template <typename T>
    void GaussianProcess<T>::get_full_space_variables_prediction_and_variance(unsigned int& variableNumber, std::vector<std::string>& variableNames, std::vector<std::pair<double, double>>& variableBounds) {
        if (!MelonModel<T>::_modelLoaded) {
            throw MelonException("  Error: No Gaussian process loaded."); }
