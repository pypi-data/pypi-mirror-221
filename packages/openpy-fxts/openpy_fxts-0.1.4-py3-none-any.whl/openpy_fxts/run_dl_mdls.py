...
import os
import pathlib
import openpy_fxts as fxts
import openpy_fxts.mdls_to_fx.simple.dlm_Conv1D
import openpy_fxts.mdls_to_fx.simple.dlm_RNN
...
class run_all_models:
    ...
    def _rnn(self):
        if self.name_mdl == 'RNN_Dense':
            return self.run_case_mdl(fxts.RNN_Dense_class)
        if self.name_mdl == 'Multi_RNN_Dense':
            return self.run_case_mdl(fxts.Multi_RNN_Dense_class)
    ...
    def _conv1d(self):
        ...
        if self.name_mdl == 'TimeDist_Conv1D_RNN':
            return self.run_case_mdl(fxts.TimeDist_Conv1D_RNN_class)
        if self.name_mdl == 'Conv1D_RNN':
            return self.run_case_mdl(fxts.Conv1D_RNN_class)
    ...
    def _stacked(self):
        if self.name_mdl == 'RNN_Stacked':
            return self.run_case_mdl(fxts.RNN_Stacked)
        if self.name_mdl == 'BiRNN_Stacked':
            return self.run_case_mdl(fxts.BiLSTM_Stacked)
        if self.name_mdl == 'Stacked_SciNet':
            return self.run_case_mdl(fxts.Stacked_SciNet)

    def _encdec(self):
        if self.name_mdl == 'EncDec_RNN':
            return self.run_case_mdl(fxts.EncDec_RNN_class)
    ...
    def _seq2seq(self):
        if self.name_mdl == 'Seq2Seq_RNN':
            return self.run_case_mdl(fxts.Seq2Seq_RNN_class)
        if self.name_mdl == 'Seq2Seq_RNN2':
            return self.run_case_mdl(fxts.Seq2Seq_RNN2_class)
        if self.name_mdl == 'Seq2Seq_RNN_Batch_Drop':
            return self.run_case_mdl(fxts.Seq2Seq_RNN_Batch_Drop_class)
        if self.name_mdl == 'Seq2Seq_BiRNN':
            return self.run_case_mdl(fxts.Seq2Seq_BiRNN_class)
        if self.name_mdl == 'Seq2Seq_BiLSTM2':
            return self.run_case_mdl(fxts.Seq2Seq_BiRNN2_class)
        if self.name_mdl == 'Seq2Seq_Conv1D_BiRNN':
            return self.run_case_mdl(fxts.Seq2Seq_Conv1D_BiRNN_class)
        if self.name_mdl == 'Seq2Seq_Multi_Head_Conv1D_BiRNN':
            return self.run_case_mdl(fxts.Seq2Seq_Multi_Head_Conv1D_BiRNN_class)
        if self.name_mdl == 'Seq2Seq_RNN_with_Luong_Attention':
            return self.run_case_mdl(fxts.Seq2Seq_RNN_with_Luong_Attention_class)
        if self.name_mdl == 'Seq2Seq_BiRNN_with_Attention':
            return self.run_case_mdl(fxts.Seq2Seq_BiRNN_with_Attention_class)
        if self.name_mdl == 'Seq2Seq_Conv2D_RNN':
            return self.run_case_mdl(fxts.Seq2Seq_Conv2D_RNN_class)
