...
from openpy_fxts.base_tf import tkl, K, tf, tkr
from openpy_fxts.layers.attention_class import SeqSelfAttention
...
class dense_drop(tkl.Layer):
    ...
    def __init__(
        ...
        for j, (unit, act) in enumerate(zip(n_units, list_act)):
            self.hidden_dense.append(tkl.Dense(units=unit, activation=act))
            if is_drop:
...
class birnn_attention_conv1d(tkl.Layer):
    ...
    def __init__(
        ...
        super(birnn_attention_conv1d, self).__init__()
        if bidirectional:
...
class multi_birnn_attention_conv1d(tkl.Layer):
    def __init__(
            self,
            mdl: object = None,
            bidirectional: bool = True,
        ...
        super(multi_birnn_attention_conv1d, self).__init__()
