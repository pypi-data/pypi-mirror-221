...
def download_from_file_record(record, dispatcher_url=config["DISPATCHER_URL"], token=config["TOKEN"]):
    url = f"{dispatcher_url}/get_purl"
    print(url)
    purl = requests.post(
        url,
...
def download_from_registered_file(path, dispatcher_url=config["DISPATCHER_URL"], token=config["TOKEN"]):
    purl = requests.post(
