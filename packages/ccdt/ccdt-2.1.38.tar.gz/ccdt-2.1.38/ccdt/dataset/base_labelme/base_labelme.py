...
class BaseLabelme(metaclass=SingletonMeta):
    ...
    def filter_images(self, parameter):
        ...
        for dataset in tqdm(self.datasets):
            for flag, value in dataset.get('labelme_info').get('flags').items():
                # 有=true、没有=false；训练=true、不训练=false；要=true、不要=false；真=true、假=false；1=true、0=false；开=true、关=false；勾选=true、不勾选=false；
                if parameter.right_check == value and flag == "1":  # 为真的表示参与训练，键=1则值必为真
    ...
    def bhattacharyya_filter_labelme(self, parameter):
        ...
        @param parameter:
        """
        print(f'根据巴氏距离，进行图像内容相似度去重开始')
        histogram_lists = []
        print("开始计算所有图像直方图。。。")
        for dataset in tqdm(self.datasets):
            if parameter.opt == 'global':  # 全局直方图计算
                histogram_lists.append(self.calc_histogram_block(dataset.get('full_path')))
            if parameter.opt == 'local':  # 局部直方图计算，局部计算更精准
                histogram_lists.append(self.hog_histogram(dataset.get('full_path')))
        histogram_lists = np.array(histogram_lists)
        print("所有图像直方图计算完成。。。")
        print('---------------------------------------')
        print('对直方图进行降维请耐心等待。。。')
        model = PCA(n_components=0.98)  # 使用它来执行主成分分析和降维操作
        ...
        for index, dataset in tqdm(enumerate(self.datasets)):
            if len(output_idx) == 0:
                ...
                cur_hist = histogram_lists[index]
                similarity1 = 0
                for j in range(len(output_idx)):
                    cpa_hist = histogram_lists[output_idx[j]]
                    similarity1 = cv2.compareHist(cur_hist, cpa_hist, 0)
