...
def matrices_to_density(lm):
	...
	for m,det,v in lm:
		if len(v) == 0:
			s += simplex_density(m, det=det)
		else:
			l.append(InfSum(simplex_density(m, det=det), v))
	if len(l) == 0:
		return s
	else:
		if s != 0:
...
def domain_to_density(d):
	return matrices_to_density(formulae_to_matrices(d, det=True))

def simplicies_density(lm):
	f = 0
	for m,lvar in lm:
		f += simplex_density(m)
	return f
