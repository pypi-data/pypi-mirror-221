...
def now(floor: bool = True) -> datetime:
    """
    Current utc datetime with optional minute level accuracy / granularity.

    Args:
        floor: If true (default), removes the second and microseconds from the current datetime.
...
def now_timestamp(floor: bool = False) -> int:
    """
    Current utc timestamp.

    Args:
        floor: If true, removes the second and microseconds from the current datetime.

    Returns:
        UTC epoch millis timestamp
    """
    return to_timestamp(now(floor))
