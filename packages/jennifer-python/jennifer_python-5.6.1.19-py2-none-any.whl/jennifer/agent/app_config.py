...
class AppConfig(object):
    ...
    def load_config(self, config_path):
        from jennifer.agent import config_parser, is_python30_below
        ...
        try:
            ...
            if len(self.cache) != 0:
                msg = str(os.getpid()) + ' jennifer ' + 'config_changed'
                print_out.write(msg)

            self.cache = {}
            self.config.read(config_path)
        except Exception as e:
            msg = str(os.getpid()) + ' jennifer.exception ' + 'load_config ' + config_path + ' ' + str(e)
            print_out.write(msg)
            traceback.print_exc()
