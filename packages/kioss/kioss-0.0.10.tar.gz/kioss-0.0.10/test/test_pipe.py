...
class TestPipe(unittest.TestCase):
    ...
    def test_map(self):
        ...
        # threaded
        self.assertSetEqual(
            set(Pipe(range(8)).map(func, num_threads=2)), set(map(func, range(8)))
        )
        ...
        num_threads = 2
        pipe = Pipe(range(N)).map(ten_millis_identity, num_threads=num_threads)
        self.assertAlmostEqual(
            pipe.time(),
            TEN_MS * N / num_threads,
            delta=0.3 * (TEN_MS * N) / num_threads,
        )
        # sidify
        l: List[int] = []
