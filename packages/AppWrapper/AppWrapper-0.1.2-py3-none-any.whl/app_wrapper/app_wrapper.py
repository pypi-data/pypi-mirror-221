...
class AppWrapper:
    def __init__(self):
        try:
            ...
            self.app_input_files = ast.literal_eval(os.environ["APP_INPUT_FILES"])

            if len(self.app_input_files) != 0:
                self.download_file_from_s3()
        except Exception as e:
            error_log = f"[AppWrapper] - init Failed: {str(e)}"
    ...
    def validate_result_format(self, result: dict):
        ...
        if result["type"] == "download" and "file_path" not in result:
            error_log = "App result should include 'file_path' key for download type"
            self.update_status(Status.Failed.value, error_log)
    ...
    def __call__(self, func):
        def inner(*args, **kwargs):
            ...
            try:
                ...
                requests.post(f"{self.TOAD_HOST}/output/", data=json.dumps(data))
