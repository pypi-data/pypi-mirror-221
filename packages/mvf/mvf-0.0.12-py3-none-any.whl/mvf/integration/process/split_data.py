...
def split_data(product: dict, params: dict):
    ...
    if params['split_type'] == 'train_test':
        ...
        if 'temporal_split' in params:
            assert numpy.abs((X_test.shape[0] / X_train.shape[0]) - params['test_size']) < 1e-2, 'The actual test size differs too much from the test_size parameter. If you have a very small dataset, choose an appropriate test size for the number of data points.'
            test_data = pandas.concat([X_test, y_test], axis=1)
            train_data = pandas.concat([X_train, y_train], axis=1)
            merged = test_data.merge(train_data, how='left', on=['group', 'X'], suffixes=['_test', '_train'])
            # the train and test series share no instances
            assert (merged.y_test.notna() & merged.y_train.notna()).sum() == 0
        else:
            assert numpy.abs((X_test.shape[0] / (X_test.shape[0] + X_train.shape[0])) - params['test_size']) < 1e-2, 'The actual test size differs too much from the test_size parameter. If you have a very small dataset, choose an appropriate test size for the number of data points.'
