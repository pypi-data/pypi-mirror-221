...
#pylint: disable=too-many-arguments, too-many-branches, too-many-statements
    ...
    def fit(self,
        ...
        train_length : int
            ...
            The constraint is that ``train_length`` :math:`\geq 2^{\text{num_levels}}`,
            where ``num_levels`` is specified in the initialization function; in the meantime,
            it cannot be larger than the length of ``data``.

            Note that if :code:`warm_start = True`, then this parameter
            is not valid.
...
<<<<<<< HEAD   (71dc59 correct param name)
            When set to True, reuse the ``model_`` of current object to train.
            We provide a method called `load_model()` to load other model.
=======
...
>>>>>>> CHANGE (eea7aa correct spelling errors in doc)
