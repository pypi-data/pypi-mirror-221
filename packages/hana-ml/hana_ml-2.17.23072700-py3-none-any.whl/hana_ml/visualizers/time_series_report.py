...
class DatasetAnalysis(object):
    ...
    def seasonal_item(self):
        ...
        year_column_name = data_.columns[2]
        lines_to_plot = data_.distinct(year_column_name).collect().sort_values(year_column_name, ascending=True)[year_column_name].to_list()
        ...
        for line_to_plot in lines_to_plot:
            ...
            temp_df = data_.filter('"{}"={}'.format(year_column_name, line_to_plot))
            ...
            temp_pf = temp_df.collect().sort_values(temp_df.columns[0], ascending=True)

            x = []
            y = []
            for xd in list(temp_pf.iloc[:, 0]):
                xd = convert_value_to_target_type(xd)
                push_valid_value(x, xd)
            x.insert(0, 'MONTH')
            for yd in list(temp_pf.iloc[:, 1].astype(float)):
                yd = convert_value_to_target_type(yd)
                push_valid_value(y, yd)
