...
import trajectopy.util.reading as reading
from trajectopy.alignment.functional_model.equations import transformed_x_from, transformed_y_from, transformed_z_from
...
class Trajectory:
    ...
    def __eq__(self, other: "Trajectory") -> bool:
        return (
            ...
            and np.allclose(self.rot.as_quat(), other.rot.as_quat())
    ...
    @classmethod
    def from_file(cls: "Trajectory", filename: str) -> "Trajectory":
        ...
        """
        header_data, trajectory_data = reading.read_data(filename, dtype=object)

        tstamps = reading.extract_trajectory_timestamps(header_data=header_data, trajectory_data=trajectory_data)
        pos = reading.extract_trajectory_pointset(header_data=header_data, trajectory_data=trajectory_data)
        arc_lengths = reading.extract_trajectory_arc_lengths(header_data=header_data, trajectory_data=trajectory_data)
        speed_3d = reading.extract_trajectory_speed(header_data=header_data, trajectory_data=trajectory_data)
        sort_index = infer_sort_index(sorting=header_data.sorting, tstamps=tstamps)
        rot = reading.extract_trajectory_rotations(header_data=header_data, trajectory_data=trajectory_data)
    ...
    def all(self) -> np.ndarray:
        ...
        """
        return np.c_[self.tstamps, self.pos.xyz, self.rot.as_quat()]
    ...
    def se3(self) -> list[np.ndarray]:
        ...
        if not self.has_orientation:
            matrices = np.tile(np.eye(3), (len(xyz), 1, 1))
        else:
            matrices = self.rot.as_matrix()
    ...
    def divide_into_laps(self) -> Union[None, list["Trajectory"]]:
    ...
    def apply_alignment(self, alignment_parameters: AlignmentParameters, inplace: bool = True) -> "Trajectory":
        ...
        (
            ...
            lever_y,
            lever_z,
        ) = self._prepare_alignment_application(alignment_parameters, traj_self)

        speed_3d = traj_self.speed_3d
        speed_x, speed_y, speed_z = speed_3d[:, 0], speed_3d[:, 1], speed_3d[:, 2]

        trafo_x = transformed_x_from(
            x_from=traj_self.pos.x,
            y_from=traj_self.pos.y,
            z_from=traj_self.pos.z,
            euler_x=euler_x,
            euler_y=euler_y,
            euler_z=euler_z,
            speed_x=speed_x,
            speed_y=speed_y,
            speed_z=speed_z,
            sim_trans_x=alignment_parameters.helmert.trans_x.value,
            sim_rot_x=alignment_parameters.helmert.rot_x.value,
            sim_rot_y=alignment_parameters.helmert.rot_y.value,
            sim_rot_z=alignment_parameters.helmert.rot_z.value,
            sim_scale=alignment_parameters.helmert.scale.value,
            time_shift=alignment_parameters.time_shift.value,
            lever_x=lever_x,
            lever_y=lever_y,
            lever_z=lever_z,
        )

        trafo_y = transformed_y_from(
            x_from=traj_self.pos.x,
            y_from=traj_self.pos.y,
            z_from=traj_self.pos.z,
            euler_x=euler_x,
            ...
            speed_y=speed_y,
            speed_z=speed_z,
            sim_trans_y=alignment_parameters.helmert.trans_y.value,
            sim_rot_x=alignment_parameters.helmert.rot_x.value,
            sim_rot_y=alignment_parameters.helmert.rot_y.value,
            sim_rot_z=alignment_parameters.helmert.rot_z.value,
            sim_scale=alignment_parameters.helmert.scale.value,
        ...
        trafo_z = transformed_z_from(
            ...
            speed_y=speed_y,
            speed_z=speed_z,
            sim_trans_z=alignment_parameters.helmert.trans_z.value,
            sim_rot_x=alignment_parameters.helmert.rot_x.value,
            sim_rot_y=alignment_parameters.helmert.rot_y.value,
            sim_scale=alignment_parameters.helmert.scale.value,
            time_shift=alignment_parameters.time_shift.value,
        ...
        if traj_self.rot is not None:
            traj_self.rot = traj_self.rot * alignment_parameters.helmert.rotation_set
