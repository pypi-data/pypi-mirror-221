...
from trajectopy.util.definitions import UNIT_FORMAT_RULES, Unit
from trajectopy.util.printing import list2box
...
class Parameter:
    ...
    def __sub__(self, other: "Parameter") -> "Parameter":
        return Parameter(value=self.value - other.value, variance=np.sqrt(self.variance**2 + other.variance**2))
...
class ParameterSet(ABC):
    ...
    def _get(self, key: str, enabled_only: bool = False) -> np.ndarray:
        val_list = []
        for param in self.__parameter_fields:
            if param.enabled == False and enabled_only:
                continue
...
class AlignmentParameters(ParameterSet):
    ...
    def from_file(cls, filename: str) -> "AlignmentParameters":
        ...
        params.values = alignment_data.iloc[:, 0].to_numpy()
        params.enabled_bool_list = alignment_data.iloc[:, 1].to_numpy(dtype=bool)
        params.set_covariance_matrix(alignment_data.iloc[:, 2:].to_numpy())
        return params
