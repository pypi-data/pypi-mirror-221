...
from trajectopy.alignment.parameters import Parameter

from trajectopy.approximation.util import least_squares
...
def direct_timeshift(
    *,
    ...
    if not len(xyz_to) == len(xyz_from):
        logging.error("estimate_leverarm: All arrays must have equal dimensions!")
    ...
    if weights is None:
        ...
        logger.debug("Using custom weighting (danish method)")
    ...
    return Parameter(value=est_params[0], name="Time shift", unit=Unit.SECOND), residuals
