...
from trajectopy.util.datahandling import lengths_from_xyz, moving
...
class SpatialSorter:
    ...
    def _print_mls_params(self) -> None:
        """Print Moving-Least-Squares approximation parameters"""
        info_str = (
            f"\n ________________________________________________\n"
            f"| -------------- MLS Approximation ------------- |\n"
            f"| Neighborhood Distance         {self.voxel_size:<17}|\n"
            f"| Maximum point movement        {self.movement_threshold:<17}|\n"
            f"|________________________________________________|\n"
        )
        print(info_str)

    @property
    def idx_sort(self) -> list:
    ...
    def function_of(self) -> np.ndarray:
        ...
        return lengths_from_xyz(self.sort(self.mls_unsorted))
    ...
    def lap_indices(self) -> np.ndarray:
        ...
        a new lap begins
        """
        return detect_laps_by_length(self.sort(self.mls_arc_lengths, inverse=True))
    ...
    def reconstruct(self) -> list:
        ...
        if len(idx_missing) > 0 and not self.discard_missing:
            ...
            logger.info(f"Inserting missing points ({len(idx_missing)}).")
        ...
        self._idx_sort = begin_with(idx=self._idx_sort, begin=np.argmax(self._mls_unsorted[:, 2]))

        # check if direction is correct
        mls_sorted = self.sort(self._mls_unsorted)
        if _detect_direction(mls_sorted) != self.direction:
            logger.info("Adjusted direction of travel!")
            self._idx_sort = np.flipud(self.idx_sort).tolist()

        return self._idx_sort

    def sort(self, array: Union[list, np.ndarray], inverse: bool = False) -> Union[list, np.ndarray]:
        """Function that applies the sorting index to an array / list

        Args:
            array (Union[list, np.ndarray]): array or list that needs
                                             to get sorted
        ...
        Returns:
            Union[list, np.ndarray]: Sorted array or list
        """
        idx_sort = np.argsort(self.idx_sort) if inverse else self.idx_sort

        if isinstance(array, np.ndarray):
            return array[idx_sort, :] if array.ndim > 1 else array[idx_sort]

        return [array[idx] for idx in idx_sort]


def _insert_point(
    missing_point_index: int, missing_point: np.ndarray, idx_sort: list, xyz_sorted: np.ndarray
...
def reconstruct_spatial_sorting(xyz: np.ndarray) -> Tuple[list, list]:
    ...
    logger.info(f"found {len(d1_nodes)} nodes of degree 1")
    ...
    logger.info(f"found minimum path length: {shortest_path_lengths:.4f} m")
...
def compute_mst(xyz: np.ndarray) -> Tuple[nx.Graph, list]:
    ...
    logging.info(f"{len(missing)} points were discarded during delaunay triangulation!")
