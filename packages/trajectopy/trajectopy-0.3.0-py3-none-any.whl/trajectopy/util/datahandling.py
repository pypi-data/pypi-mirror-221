...
class Grid:
    ...
    def scaled_grid_cells(self) -> np.ndarray:
        return np.c_[self.x_bin_locations * self.pixel_size, self.y_bin_locations * self.pixel_size, self.values]
...
def lengths_from_xyz(xyz: np.ndarray) -> np.ndarray:
    ...
    if not isinstance(xyz, np.ndarray):
        logger.error(f"Invalid data type {type(xyz)}.")
        return np.array([])
...
def remove_nan_vals(a: np.ndarray) -> np.ndarray:
    ...
    if isinstance(a, np.ndarray):
        return a[~np.isnan(a)]
    logger.error(f"Invalid input type {type(a)}.")
    return np.array([])
...
def togrid(xyz: np.ndarray, data: np.ndarray, grid_mp: int = 1) -> Grid:
