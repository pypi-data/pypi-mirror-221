...
from trajectopy.gui.models.entry import AlignmentEntry, AbsoluteDeviationEntry
from trajectopy.gui.models.selection import ResultSelection
from trajectopy.gui.requests.file_request import FileRequest
...
class ResultContextMenu(QtWidgets.QMenu):
    ...
    def __init__(self, parent) -> None:
        ...
        self._selection: ResultSelection = None
    ...
    def property_context(self) -> None:
        ...
        property_action.triggered.connect(
            lambda: self.ui_request.emit(UIRequest(type=UIRequestType.RES_PROPERTIES, selection=self.get_selection()))
        )
        ...
        if all(isinstance(entry, AbsoluteDeviationEntry) for entry in self.get_selection().entries):
            ...
            plot_action.triggered.connect(
                lambda: self.plot_request.emit(
                    PlotRequest(type=PlotRequestType.MULTI_DEVIATIONS, selection=self.get_selection())
                )
            )
    ...
    def plot_single(self):
        ...
        if isinstance(self.get_selection().entries[0], AbsoluteDeviationEntry):
            plot_action.triggered.connect(
                lambda: self.plot_request.emit(
                    PlotRequest(type=PlotRequestType.SINGLE_DEVIATIONS, selection=self.get_selection())
                )
            )
            self.addAction(plot_action)
            ...
            plot_laps_action.triggered.connect(
                lambda: self.plot_request.emit(
                    PlotRequest(type=PlotRequestType.DEVIATION_LAPS, selection=self.get_selection())
                )
            )
            plot_laps_action.setEnabled(self.get_selection().entries[0].deviations.state.sorting_known)
    ...
    def edit_single(self):
        ...
        export_action.triggered.connect(
            lambda: self.ui_request.emit(UIRequest(type=UIRequestType.EXPORT_RES, selection=self.get_selection()))
        )
