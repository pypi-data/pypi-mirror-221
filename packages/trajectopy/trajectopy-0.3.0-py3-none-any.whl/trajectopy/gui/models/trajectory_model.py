...
class TrajectoryTableModel(RequestTableModel):
    ...
    def __init__(self):
        ...
        self.items: list[TrajectoryEntry]

    def get_request(self) -> TrajectoryModelRequest:
    ...
    def export_all(self) -> None:
        for item in self.items:
            trajectory_filename = os.path.join(self.get_request().selection, f"{item.id}.traj")
            settings_filename = os.path.join(self.get_request().selection, f"{item.id}.yaml")
            self.file_request.emit(
                FileRequest(
                    type=FileRequestType.WRITE_TRAJ,
                    selection=TrajectorySelection(entries=[item]),
    ...
    def write_trajectory_order(self):
        trajectory_order_filename = os.path.join(self.get_request().selection, "trajectory_order.txt")
        self.file_request.emit(
            FileRequest(
    ...
    def rename_trajectories(self) -> None:
        input_name, ok = QInputDialog.getText(
            None, "Please enter a name", "Name:", text=self.get_request().selection.entries[0].name
        )
        if ok and input_name is not None:
            self.get_request().selection.entries[0].trajectory.name = input_name
        self.layoutChanged.emit()
    ...
    def set_as_reference(self) -> None:
        ...
        selected_id = self.get_request().selection.entries[0].id
        for entry in self.items:
            entry.set_as_reference = entry.id == selected_id
        self.layoutChanged.emit()
