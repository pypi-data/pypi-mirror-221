...
def dict2table(
    ...
    max_value_width = max(value_width, max(len(str(value)) for value in input.values()))
    max_key_width = max(key_width, max(len(str(key)) for key in input))
...
def list2box(
    ...
    Args:
        ...
        field_length (int, optional): The width of each field in the box. Defaults to None.
    ...
    # how much characters are the field rows
    field_lengths = (
        field_length if field_length is not None else max(len(str(item)) for item in input_list) + left_right_width
    )
