...
from trajectopy.gui.requests.result_model_request import ResultModelRequest
...
class TrajectoryTableView(QtWidgets.QTableView):
    ...
    def selection(self) -> TrajectorySelection:
        return TrajectorySelection(entries=self.selected_entries, reference_entry=self.model().reference_entry)

    @property
    def selected_entries(self) -> list[TrajectoryEntry]:
        return [
            self.model().items[row_index] for row_index in [row.row() for row in self.selectionModel().selectedRows()]
        ]
    ...
    def keyPressEvent(self, e: QtGui.QKeyEvent) -> None:
        ...
        if e.key() == Qt.Key.Key_T:
            self.ui_request.emit(UIRequest(type=UIRequestType.EPSG_SELECTION, selection=self.selection))
        ...
        if e.key() == Qt.Key.Key_P:
            self.ui_request.emit(UIRequest(type=UIRequestType.TRAJ_PROPERTIES, selection=self.selection))

        if e.key() == Qt.Key.Key_V:
            self.plot_request.emit(PlotRequest(type=PlotRequestType.TRAJECTORIES, selection=self.selection, dim=2))
        ...
        if e.key() == Qt.Key.Key_S:
            self.ui_request.emit(UIRequest(type=UIRequestType.TRAJ_SETTINGS, selection=self.selection))
        ...
        if e.key() == Qt.Key.Key_E:
            self.ui_request.emit(UIRequest(type=UIRequestType.EXPORT_TRAJ, selection=self.selection))
