...
from trajectopy.gui.requests.plot_requests import PlotRequest
...
class ResultTableView(QtWidgets.QTableView):
    ...
    @property
    def selected_entries(self) -> list[ResultSelection]:
        return [
    ...
    def keyPressEvent(self, e: QtGui.QKeyEvent) -> None:
        ...
        if e.key() == Qt.Key.Key_P:
            self.ui_request.emit(UIRequest(type=UIRequestType.RES_PROPERTIES, selection=self.selection))
        ...
        if e.key() == Qt.Key.Key_E:
            self.ui_request.emit(UIRequest(type=UIRequestType.EXPORT_RES, selection=self.selection))
