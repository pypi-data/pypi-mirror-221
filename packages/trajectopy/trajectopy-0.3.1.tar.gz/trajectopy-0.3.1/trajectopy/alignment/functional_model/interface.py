...
from trajectopy.alignment.data import ObservationContainer
from trajectopy.alignment.functional_model.equations import (
...
class FunctionalRelationship:
    ...
    def eval(
        self,
        func: Callable,
        observations: ObservationContainer,
        ...
        if "functional_relationship_x" in func.__name__:
            return func(
                ...
                observations.z_from,
                observations.x_to,
                observations.euler_x,
                observations.euler_y,
                observations.euler_z,
                observations.speed_x,
                observations.speed_y,
                observations.speed_z,
                parameters.sim_trans_x.value * np.ones((len(observations.x_from),)),
                parameters.sim_rot_x.value * np.ones((len(observations.x_from),)),
                parameters.sim_rot_y.value * np.ones((len(observations.x_from),)),
                parameters.sim_rot_z.value * np.ones((len(observations.x_from),)),
                parameters.sim_scale.value * np.ones((len(observations.x_from),)),
                parameters.time_shift.value * np.ones((len(observations.x_from),)),
                parameters.lever_x.value * np.ones((len(observations.x_from),)),
                parameters.lever_y.value * np.ones((len(observations.x_from),)),
                parameters.lever_z.value * np.ones((len(observations.x_from),)),
            )
        ...
        if "functional_relationship_y" in func.__name__:
            return func(
                ...
                observations.speed_y,
                observations.speed_z,
                parameters.sim_trans_y.value * np.ones((len(observations.x_from),)),
                parameters.sim_rot_x.value * np.ones((len(observations.x_from),)),
                parameters.sim_rot_y.value * np.ones((len(observations.x_from),)),
                parameters.sim_rot_z.value * np.ones((len(observations.x_from),)),
                parameters.sim_scale.value * np.ones((len(observations.x_from),)),
        ...
        if "functional_relationship_z" in func.__name__:
            return func(
                ...
                observations.speed_y,
                observations.speed_z,
                parameters.sim_trans_z.value * np.ones((len(observations.x_from),)),
                parameters.sim_rot_x.value * np.ones((len(observations.x_from),)),
                parameters.sim_rot_y.value * np.ones((len(observations.x_from),)),
                parameters.sim_scale.value * np.ones((len(observations.x_from),)),
                parameters.time_shift.value * np.ones((len(observations.x_from),)),
