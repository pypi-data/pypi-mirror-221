...
class TrajectoryApproximation(Trajectory):
    ...
    def __init__(
        ...
        settings: ApproximationSettings = ApproximationSettings(),
        state: TrajectoryProcessingState = None,
    ) -> None:
        ...
        self.state_before_approximation = copy.deepcopy(state)
        self.approximation_epsg = self.pos.epsg
        self.state.approximated = True
        self.cubic_approximations: list[CubicApproximation] = None

        return self._perform_approximation(settings)
    ...
    def _interpolate_positions(self, tstamps: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            logger.warning(f"Falling back to normal interpolation: {e}")
            return super()._interpolate_positions(tstamps)
