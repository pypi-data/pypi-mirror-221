...
from trajectopy.approximation.voxelizer import Voxelizer
...
class Line3D:
    ...
    @classmethod
    def from_points(cls: "Line3D", points: np.ndarray) -> "Line3D":
...
def mls_iterative(
    ...
    while avg_point_movement > movement_threshold:
        logger.info(f"Iteration {cnt} ...")
        xyz, avg_point_movement = mls_single(xyz=xyz, voxel_size=voxel_size, k_nearest=k_nearest)
        cnt += 1
        logger.info(f"... done! Average point movement: {avg_point_movement:.4f} m")
        if cnt > 10:
