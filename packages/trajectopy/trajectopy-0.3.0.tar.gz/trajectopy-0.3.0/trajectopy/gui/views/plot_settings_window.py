...
class PlotSettingsGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent) -> None:
        ...
        self._request: PlotSettingsRequest = None
    ...
    def get_request(self) -> PlotSettingsRequest:
        return self._request

    def set_request(self, request: PlotSettingsRequest) -> None:
    ...
    def import_settings_from_session(self):
        plot_settings_request = self.get_request()
        filename = os.path.join(plot_settings_request.selection, "plot_settings.yaml")

        if not os.path.isfile(filename):
            return

        self.settings = PlotSettings.from_file(filename)
        print(f"Reading {os.path.join(plot_settings_request.selection, 'plot_settings.yaml')}")
        self.update_view()
        self.plot_request.emit(PlotRequest(type=PlotRequestType.UPDATE_SETTINGS, plot_settings=self.settings))

    def export_settings_to_session(self):
        plot_settings_request = self.get_request()
        self.settings.to_file(os.path.join(plot_settings_request.selection, "plot_settings.yaml"))
