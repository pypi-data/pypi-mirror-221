...
class tieba(allClassParent):
    ...
    def tieba_list_more(self):
        content = self.session.get(url="https://tieba.baidu.com/f/like/mylike?&pn=1", timeout=(5, 20), allow_redirects=False, cookies=cookiejar_from_dict(self.cookie_dict, cookiejar=None, overwrite=True))
        try:
        ...
        while next_page <= pn:
            ...
            content = self.session.get(
                url=f"https://tieba.baidu.com/f/like/mylike?&pn={next_page}", timeout=(5, 20), allow_redirects=False, cookies=cookiejar_from_dict(self.cookie_dict, cookiejar=None, overwrite=True)
            )
