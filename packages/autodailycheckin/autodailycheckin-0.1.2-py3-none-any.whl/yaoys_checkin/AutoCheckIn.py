...
def checkin_task(task_name=None, config=None, cls_name=None, checkin_message=None, retry=None):
    if checkin_message is None:
        checkin_message = []
    message = []
    print_message(is_print=config['common_config']['is_print'], message='{}start checkin {} ....'.format('' if retry is False else 'retry ', task_name))
    # driver = None
    # if 'chrome_path' in config and 'common_config' in config:
    #     driver = get_driver(chrome_path=config['common_config']['chrome_path'])
    #     if driver is None or len(str(driver)) <= 0:
    #         driver = None
    ...
    print_message(is_print=config['common_config']['is_print'], message='{} checkin complete, checkin message:{}'.format(task_name, ''.join(message)))
...
def checkin_push_message(config=None, cls_name=None, checkin_message=None, retry=None):
    ...
    if token is not None and str(token) != '':
        print_message(is_print=config['common_config']['is_print'], message='{} push message to {} ....'.format('' if retry is None else 'retry', cls_name[0]))
        ...
        print_message(is_print=config['common_config']['is_print'], message='push message to {} complete'.format(cls_name[0]))
...
def checkin(**kwargs):
    ...
    if config is None:
        raise Exception('The config json is None')
    checkin_message = []
    if 'is_scheduler' in config['common_config'] and config['common_config']['is_scheduler'] is True:
        print('\n' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + ': The timing config is day_of_week=>{},hour=>{},minute=>{}'.format(config['scheduler']['timing_day_of_week'], config['scheduler']['timing_hour'], config['scheduler']['timing_minute']))
    print_message(is_print=config['common_config']['is_print'], message='\n##################################start auto checkin' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + '##################################')
    log_info('*********************************************************************************************************************************', my_logger=checkin_logger)
    log_info('##################################start auto checkin  ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + '  ##################################', my_logger=checkin_logger)
    ...
    for task_name, cls_name in checkin_class.items():
        ...
        try:
            ...
            if cls_name is None:
                log_info('*******************************checkin class is error*******************************\n', my_logger=checkin_logger)
                continue
            # 配置文件中不存在该平台的相关配置
            if cls_name['cookie_name'] not in config['cookieOrUser']:
            ...
            if 'is_checkin' in config['cookieOrUser'][cls_name['cookie_name']] and config['cookieOrUser'][cls_name['cookie_name']]['is_checkin'] is False:
                checkin_message.append('[{}] 该平台 is_checkin 为false, 不执行签到\n'.format(task_name))
                log_info('##################################{} skip checkin##################################'.format(task_name), my_logger=checkin_logger)
                not_checkin_count += 1
                continue
            ...
            if 'checkin_verification' not in config['cookieOrUser'][cls_name['cookie_name']] or config['cookieOrUser'][cls_name['cookie_name']]['checkin_verification'] is None \
                ...
                not_checkin_count += 1
                log_info('##################################{} checkin_verification is None##################################'.format(task_name), my_logger=checkin_logger)
                continue
            ...
            if retry is False:
                ...
                checkin_message.append('[{}] checkin error,error message: {} \n'.format(task_name, str(e)))
                log_info('*******************************{} checkin error,error message: {}*******************************\n'.format(task_name, str(e)), my_logger=checkin_logger)
        finally:
            continue
    ...
    if 'is_push_message' in config['push_message'] and config['push_message']['is_push_message'] is True:
        for key, value in message_class.items():
            ...
            try:
                ...
                if retry is False:
                    retry = True
                    log_info('*******************************push message retry {}*******************************\n'.format(str(message_name)), my_logger=checkin_logger)
                    checkin_push_message(config=config, cls_name=cls_name, checkin_message=checkin_message)
                else:
                    print_message(is_print=config['common_config']['is_print'], message='{} push message error'.format(message_name) + str(e))
                    log_info('*******************************push message error,error message: {}*******************************\n'.format(str(e)), my_logger=checkin_logger)
            finally:
                continue
    else:
        log_info('不推送签到信息', my_logger=checkin_logger)
    log_info('*******************************push message complete*******************************', my_logger=checkin_logger)
    # print_message(is_print=config['common_config']['is_print'], message='###############################all checkin complete ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + '###################################\n')
    log_info('###############################all checkin complete  ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + '  ###################################', my_logger=checkin_logger)
    # print(checkin_message[0].replace('\n', '') + '\t' + str(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())) + " 今日所有签到已结束")
    # for i in range(30, 0, -1):
    #     if 'is_scheduler' in config['common_config'] and config['common_config']['is_scheduler'] is True:
    #         print('\r', f"将在 {i} 秒后自动结束本次签到流程并等待下一次签到执行", end="", flush=True)
    #     else:
    #         print('\r', f"将在 {i} 秒后自动结束本次签到流程", end="", flush=True)
    #     time.sleep(1)
    log_info('###############################' + checkin_message[0].replace("\n", "") + '\t' + str(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())) + " 今日所有签到已结束" + '  ###################################', my_logger=checkin_logger)
    if config['common_config'][''] == 1:
...
def start_checkin(sched=None, json_file_path=None):
    try:
        ...
        # print('The timing config is: day_of_week=>{},hour=>{},minute=>{}'.format(day_of_week, hour, minute))
        # 如果是定时任务
        if 'is_scheduler' in config_json['common_config'] and config_json['common_config']['is_scheduler'] is True:
            job_defaults = {
                'coalesce': True,
            ...
            if sched is None:
                ...
                sched.add_job(func=checkin,
                              ...
                              max_instances=1)
                # sched.add_job(func=checkin, id='checkin',
                #               trigger='interval', timezone='Asia/Shanghai', seconds=3, max_instances=1,
                #               kwargs={'checkin_logger': checkin_logger, 'json_file_path': json_file_path, 'scheduler': sched})
                sched.start()
        # 如果不是定时任务，则立即执行一次签到
        else:
