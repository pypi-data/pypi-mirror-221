...
class EquipmentShell(Equipments):
    ...
    @staticmethod
    # Tested and mocked
    def get_most_recent_file_info(name_group_directory: str, directory_path: str, connection: ftplib.FTP) \
            -> dict[str, any]:
        result = {"file": "", "modification_date": "PAS DE FICHIER TROUVÉ", "file_size": 0,
        ...
        try:
            with ftplib.FTP() as connection:
                for sub_directory in sub_directories:
                    ...
                    connection.connect(host=server_host)
                    connection.login(user=server_user, passwd=ConfigsShell.get_value(f"{server_host}_{server_user}"))
                    content[sub_directory[0]] = EquipmentShell.get_most_recent_file_info(directory_path=directory_path,
                                                                                         connection=connection,
                                                                                         name_group_directory=name_equipment_directory)
