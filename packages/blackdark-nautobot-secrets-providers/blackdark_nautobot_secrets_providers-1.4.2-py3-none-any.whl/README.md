...
```no-highlight
pip install blackdark-nautobot-secrets-providers
```
...
```no-highlight
pip install blackdark-nautobot-secrets-providers[all]
```
...
```no-highlight
pip install blackdark-nautobot-secrets-providers[aws]
```
...
```no-highlight
pip install blackdark-nautobot-secrets-providers[hashicorp]
```
...
```no-highlight
pip install blackdark-nautobot-secrets-providers[thycotic]
```
...
```no-highlight
echo blackdark-nautobot-secrets-providers >> local_requirements.txt
```
...
PLUGINS = ["blackdark_nautobot_secrets_providers"]
...
* `project_name`: the default docker compose project name (default: blackdark_nautobot_secrets_providers)
...
Using **PyInvoke** these configuration options can be overridden using [several methods](http://docs.pyinvoke.org/en/stable/concepts/configuration.html).  Perhaps the simplest is simply setting an environment variable `INVOKE_blackdark_nautobot_secrets_providers_VARIABLE_NAME` where `VARIABLE_NAME` is the variable you are trying to override.  The only exception is `compose_files`, because it is a list it must be overridden in a yaml file.  There is an example `invoke.yml` (`invoke.example.yml`) in this directory which can be used as a starting point.
...
Each command can be executed with `invoke <command>`. Environment variables `INVOKE_blackdark_nautobot_secrets_providers_PYTHON_VER` and `INVOKE_blackdark_nautobot_secrets_providers_NAUTOBOT_VER` may be specified to override the default versions. Each command also has its own help `invoke <command> --help`
...
```no-highlight
alias vault="docker exec -it blackdark_nautobot_secrets_providers_vault_1 vault"
```
