...
def render_change_log_to_text(entries: List[ChangeLogEntry],
                              render_config: ChangeLogRenderConfig = None) -> str:
    ...
    return template.render(change_log_entries=entries, config=render_config)
...
def render_change_log_to_file(entries: List[ChangeLogEntry], file_path: str,
    ...
    with open(file_path, 'w', encoding='utf-8') as file_p:
        file_p.write(render_change_log_to_text(entries, render_config))
