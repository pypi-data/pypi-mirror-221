import re
from utils.num_unit import NUM_UNIT
...
class Convert_Unit:
    ...
    def change_kor(self, sentence: str) -> (str, str):
        ...
        for tag in re.findall(r"[0-9]+", sentence):
            pred = Num2Word.number_to_kor(tag)
            tmp_tagging = tmp_tagging.replace(tag, pred)

        preds = tmp_tagging.replace("~", ' ')
        return tagging, preds

    def change_num_unit(self, sentence: str) -> str:
        ''' 숫자 단위 명사 한글로 바꾸기 '''
        r_unit_pattern = r"\d+[ㄱ-ㅣ가-힣]*[a-zA-Z]+"
        if re.search(r_unit_pattern, sentence):
            r_eng_pattern = r"[a-zA-Z]+"
            for x in re.findall(r_eng_pattern, sentence):
                if x in self.num_unit:
                    sentence = sentence.replace(x, self.num_unit[x])
        return sentence