...
def filename_only(para_combination):
    ...
    for key in list(para_dict.keys()):
        ...
        if key == 'pairs':
            para = ''
            if str(para_combination[key][0]).isdigit():
                para = para + str(para_combination[key][0]).zfill(5)
            else:
                para = para + str(para_combination[key][0])

            para = para + '_'

            if str(para_combination[key][1]).isdigit():
                para = para + str(para_combination[key][1]).zfill(5)
            else:
                para = para + str(para_combination[key][1])
...
def generate_filepath(para_combination, folder=''):
    ...
    filepath = os.path.join(folder, output_folder, f'{para_combination["pairs"][0]}_{para_combination["pairs"][1]}_{reference_code}.{file_format}')
    ...
    if isinstance(risk_free_rate, str):
        try:
            if risk_free_rate == 'geometric_mean':
                ...
                risk_free_rate = pairguy.get_geometric_mean_of_yearly_rate(start_date_year, end_date_year)
            else:
                risk_free_rate = pairguy.get_latest_fed_fund_rate()
        except:
...
def calculate_win_rate_info(df):
    num_of_trade = list( (df['action'] == 'buy') | (df['action'] == 'sell') ).count(True)
...
def calculate_win_rate(df_csv):
    df = df_csv[['date', 'realized_pnl', 'action_0']].copy()
    df = df.rename(columns={'action_0': 'action'})
