...
class Components:
    ...
    def update_checkbox_div(self, para_dict, result_df):
        ...
        for key in para_dict:
            ...
            if key == 'pairs':
                ...
                for tup in unique_values:
                    tup_list = ast.literal_eval(tup)
                    unique_.add(tup_list[0])
                    unique_.add(tup_list[1])
                    unique_sort = list(unique_)
                    unique_sort.sort()

                checkbox_values['code'] = unique_sort
            else:
        ...
        for i, para_name in enumerate (para_dict):
            ...
            if para_name == 'pairs':
                for option in checkbox_values['code']:
                    if isinstance(option, int):
                        _options.append(str(option).zfill(5))
                    else:
                        _options.append(option)
    ...
    def aggregate_df(self):
        ...
        for folder in folders:
            ...
            for i, row in df_saved.iterrows():
                ...
                file_format = para_combination['file_format']
                save_name = pairguy.filename_only(para_combination)
                ref_code = pairguy.path_reference_code(save_name)
        ...
        if isinstance(risk_free_rate, str):
            try:
                if risk_free_rate == 'geometric_mean':
                    ...
                    risk_free_rate_float = pairguy.get_geometric_mean_of_yearly_rate(start_date_year, end_date_year)
                else:
                    risk_free_rate_float = pairguy.get_latest_fed_fund_rate()
            except:
