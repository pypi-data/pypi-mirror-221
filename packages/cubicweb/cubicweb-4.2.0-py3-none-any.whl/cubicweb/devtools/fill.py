...
def get_max_length(eschema, attrname):
    ...
    for cst in eschema.relation_definition(attrname).constraints:
...
""" % (
    ...
    def generate_tzdatetime(self, entity, attrname, index):
        """generates a random date (format is 'yyyy-mm-dd HH:MM')"""
        base = datetime(
            randint(2000, 2004),
            randint(1, 12),
    ...
    def _actual_boundary(self, entity, attrname, boundary):
        if isinstance(boundary, Attribute):
            ...
            if boundary.attr not in entity:
    ...
    def get_bounds(self, entity, attrname):
        ...
        for cst in self.eschema.relation_definition(attrname).constraints:
...
def make_entity(
    ...
    for rschema, attrschema in eschema.attribute_definitions():
        ...
        if (
            ...
            and attrname[:-7] in eschema.subject_relations.values()
        ):
...
def composite_relation(rschema):
    for obj in rschema.objects():
        if (
            obj.relation_definition(rschema, "object", take_first=True).composite
            == "subject"
        ):
            return True
    for obj in rschema.subjects():
        if (
            obj.relation_definition(rschema, "subject", take_first=True).composite
            == "object"
        ):
            return True
    return False
...
class RelationsQueriesGenerator:
        ...
        for rschema in rels:
            ...
            for subj, obj in rschema.relation_definitions:
                ...
                if rschema.symmetric and (obj, subj) in sym:
                    continue
                subjcard, objcard = rschema.relation_definition(subj, obj).cardinality
    ...
    def make_relation_queries(self, sedict, oedict, rschema, subj, obj):
        rdef = rschema.relation_definition(subj, obj)
