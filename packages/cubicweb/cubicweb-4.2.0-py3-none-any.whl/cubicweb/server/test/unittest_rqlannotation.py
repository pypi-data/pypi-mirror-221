...
class RQLAnnotatorTC(BaseQuerierTC):
    def setUp(self):
        handler = get_test_db_handler(ApptestConfiguration("data", __file__))
    ...
    def test_0_1(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx,
                "Any SEN,RN,OEN WHERE X from_entity SE, "
    ...
    def test_0_4(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any A,B,C WHERE A eid 12,A comment B, " "A ?wf_info_for C"
            )
    ...
    def test_0_7(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Personne X,Y where X nom NX, " "Y nom NX, X eid XE, not Y eid XE"
            )
    ...
    def test_0_12(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Personne P WHERE P concerne A, " 'A concerne S, S nom "Logilab"'
            )
            self.assertEqual(rqlst.defined_vars["P"]._q_invariant, True)
    ...
    def test_1_0(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any X,Y WHERE X created_by Y, " "X eid 5, NOT Y eid 6"
            )
            self.assertEqual(rqlst.defined_vars["Y"]._q_invariant, True)
    ...
    def test_1_1(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any X,Y WHERE X created_by Y, X eid 5, " "NOT Y eid IN (6,7)"
            )
            self.assertEqual(rqlst.defined_vars["Y"]._q_invariant, True)
    ...
    def test_8(self):
        with self.admin_access.cnx() as cnx:
            ...
            rqlst = self._prepare(
                cnx,
                "DISTINCT Any X WHERE A concerne X, "
    ...
    def test_diff_scope_identity_deamb(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx,
                "Any X WHERE X concerne Y, Y is Note, "
    ...
    def test_has_text_1(self):
        with self.admin_access.cnx() as cnx:
            ...
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, True)
            self.assertEqual(
                rqlst.defined_vars["X"].stinfo["principal"].r_type, "has_text"
            )

    def test_has_text_2(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any X WHERE X is Personne, " 'X has_text "coucou"'
            )
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, True)
            self.assertEqual(
                rqlst.defined_vars["X"].stinfo["principal"].r_type, "has_text"
    ...
    def test_not_relation_1(self):
        with self.admin_access.cnx() as cnx:
            ...
            rqlst = self._prepare(
                cnx, "Any P,G WHERE P require_group G, " "NOT X require_permission P"
            )
            self.assertEqual(rqlst.defined_vars["P"]._q_invariant, False)
    ...
    def test_not_relation_2(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "TrInfo X WHERE X eid 2, " "NOT X from_state Y, Y is State"
            )
            self.assertEqual(rqlst.defined_vars["Y"]._q_invariant, True)
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, False)
    ...
    def test_not_relation_4_5(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any X WHERE NOT Y evaluee X, " "Y eid %s, X is Note" % self.ueid
            )
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, False)
    ...
    def test_not_relation_5_2(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx,
                'DISTINCT Any X,Y WHERE X name "CWGroup", '
                "Y eid IN(1, 2, 3), NOT X read_permission Y",
            )
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, False)
            self.assertEqual(rqlst.defined_vars["Y"]._q_invariant, False)
    ...
    def test_exists_1(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any U WHERE U eid IN (1,2), EXISTS(X owned_by U)"
            )
    ...
    def test_exists_2(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any U WHERE EXISTS(U eid IN (1,2), X owned_by U)"
            )
            self.assertEqual(rqlst.defined_vars["U"]._q_invariant, False)
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, True)
    ...
    def test_exists_3(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any U WHERE EXISTS(X owned_by U, X bookmarked_by U)"
            )
            self.assertEqual(rqlst.defined_vars["U"]._q_invariant, False)
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, True)
    ...
    def test_not_exists_1(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any U WHERE NOT EXISTS(X owned_by U, " "X bookmarked_by U)"
            )
            self.assertEqual(rqlst.defined_vars["U"]._q_invariant, False)
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, True)
    ...
    def test_or_1(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any X WHERE X concerne B OR " "C concerne X, B eid 12, C eid 13"
            )
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, False)
    ...
    def test_or_2(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx,
                "Any X WHERE X created_by U, X concerne B OR "
    ...
    def test_or_exists_1(self):
        with self.admin_access.cnx() as cnx:
            ...
            rqlst = self._prepare(
                cnx,
                'DISTINCT Any A,S WHERE A is Affaire, S nom "chouette", '
    ...
    def test_or_exists_2(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx,
                'Any U WHERE EXISTS(U in_group G, G name "managers") OR '
    ...
    def test_or_exists_3(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx,
                "Any COUNT(S),CS GROUPBY CS ORDERBY 1 DESC LIMIT 10 "
    ...
    def test_remove_from_deleted_source_2(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Note X WHERE X eid IN (999998, 999999), NOT X cw_source Y"
            )
            self.assertEqual(rqlst.defined_vars["X"]._q_invariant, False)
            self.assertEqual(rqlst.defined_vars["Y"]._q_invariant, True)
    ...
    def test_has_text_security_cache_bug(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx,
                'Any X WHERE X has_text "toto" WITH X BEING '
