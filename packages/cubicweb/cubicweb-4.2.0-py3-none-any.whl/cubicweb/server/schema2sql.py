...
def eschema_sql_def(dbhelper, eschema, skip_relations=(), prefix=""):
    ...
    attrs = [
        ...
        if attrdef[0].type not in skip_relations
    ]
    attrs += [
        (rschema, None)
        for rschema in list(eschema.subject_relations.values())
...
def eschema2sql(dbhelper, eschema, skip_relations=(), prefix=""):
    ...
    for rschema, aschema in attrs:
        if aschema is None:  # inline relation
            continue
        rdef = rschema.relation_definition(eschema.type, aschema.type)
    ...
    for i in range(len(attrs)):
        rschema, attrschema = attrs[i]
        if attrschema is None or eschema.relation_definition(rschema).indexed:
            yield dbhelper.sql_create_index(table, prefix + rschema.type)
        if attrschema and any(
            isinstance(cstr, UniqueConstraint)
            for cstr in eschema.relation_definition(rschema).constraints
        ):
