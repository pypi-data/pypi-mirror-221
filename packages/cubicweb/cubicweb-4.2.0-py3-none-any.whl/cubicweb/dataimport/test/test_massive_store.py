...
from cubicweb.dataimport.test import test_stores
...
where table_schema = %(s)s""",
    ...
    def test_simple_update(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            store.finish()
        with self.admin_access.repo_cnx() as cnx:
            rset = cnx.execute("Any X LIMIT 2000 WHERE X is Location")
            self.assertEqual(len(rset), 2000)
            store = MassiveObjectStore(cnx)
            for (eid,) in rset:
                store.prepare_update_entity("Location", eid, name="Poulet")
            store.flush()
            store.finish()
        with self.admin_access.repo_cnx() as cnx:
            rset = cnx.execute("Any X WHERE X is Location, X name 'Poulet'")
            self.assertEqual(len(rset), 2000)
    ...
    def test_simple_update_no_drop_index_nor_constraints(self):
        with self.admin_access.repo_cnx() as cnx:
            store = MassiveObjectStore(cnx, drop=False)
            self.push_geonames_data(self.datapath("geonames.csv"), store)
            store.flush()
    ...
    def test_massive_store_table_deleted_on_failure(self):
        """assert that the 'store.finish' has cleaned the automatically created
        table if an exception is raised during the import"""
        ...
        with self.admin_access.repo_cnx() as cnx:
            try:
                with MassiveObjectStore(cnx) as store:
                    store.prepare_insert_entity("Location", name="toto")
                    store.flush()
                    raise FakeException
            except FakeException:
                # assert no location has been inserted
                nb_locations = cnx.system_sql(
                    """
                       SELECT COUNT(*) FROM cw_location;
                    """
                ).fetchone()[0]
                self.assertEqual(nb_locations, 0)

                # assert cwmassive_initialized table has been deleted
                result = cnx.system_sql(
                    """
                    SELECT EXISTS (
                       SELECT FROM information_schema.tables
                       WHERE table_name = 'cwmassive_initialized'
                    );
                    """
                )
                self.assertFalse(result.fetchone()[0])

                # assert there is no temporary table left
                result = cnx.system_sql(
                    """
                    SELECT EXISTS (
                       SELECT FROM information_schema.tables
                       WHERE table_name ILIKE 'cw_location_%'
                    );
                    """
                )
                self.assertFalse(result.fetchone()[0])
