...
def _add_relation_definition_no_perms(subjtype, rtype, objtype):
    rschema = fsschema.relation_schema_for(rtype)
    rdef = rschema.relation_definitions[(subjtype, objtype)]
    rdef.rtype = schema.relation_schema_for(rtype)
    rdef.subject = schema.entity_schema_for(subjtype)
    rdef.object = schema.entity_schema_for(objtype)
...
def replace_eid_sequence_with_eid_numrange(session):
    ...
    except Exception:  # programming error, already migrated
        session.rollback()
        return
...
if applcubicwebversion < (3, 18, 0) and cubicwebversion >= (3, 18, 0):
    ...
    # things later.
    rdefeid = schema["defaultval"].relation_definitions.values()[0].eid
    rql(
...
if applcubicwebversion <= (3, 14, 0) and cubicwebversion >= (3, 14, 0):
    if "require_permission" in schema and "localperms" not in repo.config.cubes():
...
if (
    ...
    and "CWUniqueTogetherConstraint" not in schema
):
    add_entity_type("CWUniqueTogetherConstraint")

if ("CWUniqueTogetherConstraint", "CWRType") not in schema[
    "relations"
