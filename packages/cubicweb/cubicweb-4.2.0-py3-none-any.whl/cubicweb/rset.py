...
from logilab.common.registry import RegistryNotFound
...
class ResultSet:
    ...
    def possible_actions(self, **kwargs):
        ...
        if "actions" not in self.req.vreg:
            raise RegistryNotFound(
                "There is no 'actions' registry. Define it or install the web cube"
            )
    ...
    def get_entity(self, row, col):
        ...
        try:
            eschema = self.req.vreg.schema.entity_schema_for(etype)
            if eschema.final:
    ...
    def _rset_structure(self, eschema, entity_col):
        ...
        get_rschema = eschema.schema.relation_schema_for
        attr_cols = {}
        rel_cols = {}
        ...
        for i, attr, role in attr_desc_iterator(select, col, entity_col):
            ...
            if rschema.final:
                ...
                rdef = eschema.relation_definition(attr, role, take_first=True)
    ...
    def _locate_query_params(self, rqlst, row, col):
        ...
        if self.req.vreg.schema.entity_schema_for(etype).final:
            for select in rqlst.children:
                ...
                for i in range(len(select.selection)):
                    ...
                    if (
                        coletype is None
                        or self.req.vreg.schema.entity_schema_for(coletype).final
                    ):
                        continue
                    try:
