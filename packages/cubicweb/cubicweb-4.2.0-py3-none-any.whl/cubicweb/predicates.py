...
class is_instance(EClassPredicate):
    ...
    def score_class(self, eclass, req):
        if not hasattr(req.vreg, "_is_instance_predicate_cache"):
...
class relation_possible(EntityPredicate):
    ...
    * `strict`, boolean (default to False) telling what to do when the user has
    ...
    def _get_rschema(self, eclass):
        ...
        if self.role == "object":
            return eschema.object_relations.get(self.rtype)
        else:
            return eschema.subject_relations.get(self.rtype)
        ...
        if self.target_etype is not None:
            try:
                rdef = rschema.role_relation_definition(
                    eschema, self.target_etype, self.role
                )
            except KeyError:
                return 0
            ...
            teschema = req.vreg.schema.entity_schema_for(self.target_etype)
    ...
    def score_entity(self, entity):
        ...
        if self.action:
            if self.target_etype is not None:
                try:
                    rschema = rschema.role_relation_definition(
    ...
    * else check permission is granted for each entity found in the column
    ...
    def __call__(self, cls, req, rset=None, row=None, col=0, entity=None, **kwargs):
        ...
        if row is None:
            score = 0
            need_local_check = []
            geteschema = req.vreg.schema.entity_schema_for
            user = req.user
            action = self.action
            ...
            if need_local_check:
                ...
                for i, row in enumerate(rset):
                    if rset.description[i][col] not in need_local_check:
                        continue
...
class match_view(ExpectedValuePredicate):
    ...
    def __call__(self, cls, req, view=None, **kwargs):
        if view is None or view.__regid__ not in self.expected:
            return 0
        return 1
...
class match_context(ExpectedValuePredicate):
    def __call__(self, cls, req, context=None, **kwargs):
        if context not in self.expected:
            return 0
        return 1
...
class match_search_state(ExpectedValuePredicate):
    ...
    def __call__(self, cls, req, **kwargs):
        try:
            if req.search_state[0] not in self.expected:
                return 0
...
class specified_etype_implements(is_instance):
    ...
    def __call__(self, cls, req, **kwargs):
        if "etype" in kwargs:
            ...
            if "etype" not in req.form:
                return 0

            etype = req.form["etype"]

            if etype.lower() not in req.vreg.case_insensitive_etypes:
                return 0
        ...
        if score:
            eschema = req.vreg.schema.entity_schema_for(etype)
