...
class PropagateRelationHook(Hook):
    ...
    def __call__(self):
        ...
        for eid in (self.eidfrom, self.eidto):
            ...
            if (
                self.main_rtype
                not in self._cw.vreg.schema.entity_schema_for(etype).subject_relations
            ):
                return
...
class PropagateRelationAddHook(Hook):
    ...
    def __call__(self):
        eschema = self._cw.vreg.schema.entity_schema_for(
            self._cw.entity_type(self.eidfrom)
        )
        execute = self._cw.execute
        for rel in self.subject_relations:
            if (
                rel in eschema.subject_relations
                and rel not in self.skip_subject_relations
            ):
                execute(
        ...
        for rel in self.object_relations:
            if (
                rel in eschema.object_relations
                and rel not in self.skip_object_relations
            ):
                execute(
                    "SET R %s P WHERE X eid %%(x)s, P eid %%(p)s, "
