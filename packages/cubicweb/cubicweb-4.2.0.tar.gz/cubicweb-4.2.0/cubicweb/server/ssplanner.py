...
from rql.stmts import Delete, Insert, Set, Union as UnionStatement, Select
from yams import KEYWORD_MAP
...
if TYPE_CHECKING:
    from cubicweb.server.querier import ExecutionPlan, InsertPlan
    ...
    for relation in rqlst.main_relations:
        ...
        try:
            ...
            if isinstance(rhs, Constant) and not rhs.uid:
                ...
                attrtype = eschema.subject_relations[rtype].objects(eschema)[0]
                value = _eval_const_attribute(rhs, plan.args, attrtype)
            ...
            if isinstance(rhs, Constant):
                ...
                if checkread and eid not in neweids:
                    with cnx.security_enabled(read=False):
                        cnx.vreg.schema.entity_schema_for(
                            cnx.entity_type(eid)
                        ).check_perm(cnx, "read", eid=eid)
...
class SSPlanner:
    ...
    def build_plan(self, plan: Union["ExecutionPlan", "InsertPlan"]) -> None:
        """build an execution plan from a RQL query

        do nothing here, dispatch according to the statement type
        """
        if isinstance(plan.rqlst, Set):
            steps = self.build_set_plan(plan, plan.rqlst)
        elif isinstance(plan.rqlst, Delete):
            steps = self.build_delete_plan(plan, plan.rqlst)
        elif isinstance(plan.rqlst, UnionStatement):
            steps = self.build_select_plan(plan, plan.rqlst)
        elif isinstance(plan.rqlst, Insert):
            steps = self.build_insert_plan(plan, plan.rqlst)
        else:
            raise ValueError("plan is not associated to a valid statement")
        ...
        for edef, rdefs in to_select.items():
            ...
            for i, (rtype, term, reverse) in enumerate(rdefs):
                ...
                if reverse:
                    ...
                    rschema = eschema.subject_relations[rtype]
    ...
    def build_set_plan(self, plan, rqlst):
        ...
        getrschema = self.schema.relation_schema_for
        ...
        for i, relation in enumerate(rqlst.main_relations):
            ...
            if rhskey not in selectedidx:
                if isinstance(rhs, Constant):
                    attrtype = rschema.objects()[0]
                    rhsinfo = (
                        _CONSTANT,
                        _eval_const_attribute(rhs, plan.args, attrtype),
                        residx,
                    )
...
def _eval_const_attribute(const_node, kwargs, attrtype):
    if const_node.type == "Substitute":
        return kwargs[const_node.value]
    if const_node.type in ["Date", "Datetime"]:
        assert attrtype in ["Datetime", "TZDatetime", "Date"]
        return KEYWORD_MAP[attrtype][const_node.value]()
    else:
