...
def prefill_entity_caches(entity):
    ...
    for rschema in list(entity.e_schema.object_relations.values()):
        rtype = str(rschema)
...
class Repository:
            ...
            if not config.creating:
                ...
                with self.internal_cnx() as cnx:
                    for etype, eid in cnx.execute(
                        ...
                        try:
                            self.schema.entity_schema_for(etype).eid = eid
                        except KeyError:
    ...
    def authenticate_user(self, cnx, login, **authinfo):
        ...
        if (
            ...
            and cwuser.cw_adapt_to("IWorkflowable").state
            not in cwuser.AUTHENTICABLE_STATES
        ):
    ...
    def glob_update_entity(self, cnx, edited):
        ...
        try:
            ...
            for attr in list(edited):
                ...
                if rschema.final:
                    if getattr(
                        eschema.relation_definition(attr), "fulltextindexed", False
                    ):
