...
def sort_term_selection(sorts, rqlst, groups):
    ...
    for sortterm in sorts:
        ...
        if (
            ...
            and str(term) not in selectionidx
...
class SQLGenerator:
    ...
    def _visit_outer_join_relation(self, relation, rschema):
        ...
        if leftalias is None:
            if leftvar.stinfo["principal"] is not relation:
                ...
                relation_schema_for = self.schema.relation_schema_for
                for orelation in leftvar.stinfo["relations"]:
                    if (
                        orelation is not relation
                        and not relation_schema_for(orelation.r_type).final
                    ):
                        break
                else:
    ...
    def visit_variablerefattributeaccess(self, variableref):
        """get the sql name for a variable reference"""
        # use accept, .variable may be a variable or a columnalias
        return variableref.variable.accept(self)

    def visit_variablerefmethodcall(self, variableref):
        """get the sql name for a variable reference"""
        # use accept, .variable may be a variable or a columnalias
        return variableref.variable.accept(self)
    ...
    def visit_variable(self, variable):
        ...
        if variable.stinfo["attrvar"]:
            ...
            if principal is None:
                ...
                if self.schema.relation_schema_for(principal.r_type).inlined:
                    sql = self._linked_var_sql(variable)
                else:
    ...
    def _var_info(self, var):
        ...
        if self.schema.entity_schema_for(etype).final:
