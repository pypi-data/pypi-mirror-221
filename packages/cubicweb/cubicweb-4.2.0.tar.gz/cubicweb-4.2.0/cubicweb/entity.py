...
def greater_card(rschema, subjtypes, objtypes, index):
    for subjtype in subjtypes:
        for objtype in objtypes:
            card = rschema.relation_definition(subjtype, objtype).cardinality[index]
            if card in "+*":
                return card
    return "1"
...
def can_use_rest_path(value):
    ...
    # make sure value is a string or has a valid string representation
    value = str(value)
...
def pruned_lt_info(eschema, lt_infos):
    ...
    for (lt_rtype, lt_role), eids in lt_infos.items():
        ...
        if not len(eids) == 1:
            continue
        lt_card = eschema.relation_definition(lt_rtype, lt_role).cardinality[
...
class Entity(AppObject):
    ...
    def __initialize__(cls, schema):
        ...
        cls.e_schema = eschema = schema.entity_schema_for(etype)
    ...
    def _fetch_ambiguous_rtypes(cls, select, var, fetchattrs, subjtypes, schema):
        ...
        for attr in fetchattrs.copy():
            rschema = schema.relation_schema_for(attr)
            if rschema.final:
                continue
            ttypes = None
            for subjtype in subjtypes:
    ...
    def _fetch_restrictions(
        ...
        for attr in sorted(fetchattrs):
            try:
                rschema = eschema.subject_relations[attr]
            except KeyError:
                cls.warning(
                ...
                continue
            # XXX take_first=True to remove warning triggered by ambiguous inlined relations
            rdef = eschema.relation_definition(attr, take_first=True)
    ...
    def cw_rest_attr_info(cls):
        ...
        * 'eid'
        ...
        if cls.rest_attr:
            ...
            for rschema in list(cls.e_schema.subject_relations.values()):
                if (
                    rschema.final
                    and rschema not in ("eid", "cwuri")
                    and cls.e_schema.has_unique_values(rschema)
                    and cls.e_schema.relation_definition(rschema.type).cardinality[0]
                    == "1"
                ):
    ...
    def _cw_build_entity_query(cls, kwargs):
        ...
        for attr, value in kwargs.items():
            ...
            rschema = (
                eschema.subject_relations[attr]
                if role == "subject"
                else eschema.object_relations[attr]
            )
    ...
    def copy_relations(self, ceid):  # XXX cw_copy_relations
        ...
        for rschema in list(self.e_schema.subject_relations.values()):
            ...
            if getattr(self, rschema.type):
                continue
            # XXX take_first=True to remove warning triggered by ambiguous relations
            rdef = self.e_schema.relation_definition(rschema, take_first=True)
        ...
        for rschema in list(self.e_schema.object_relations.values()):
            ...
            if rschema.type in skip_copy_for["object"]:
                continue
            # XXX take_first=True to remove warning triggered by ambiguous relations
            rdef = self.e_schema.relation_definition(rschema, "object", take_first=True)
            # skip composite relation
            if rdef.composite:
                continue
            # skip relation with card in ?1 else we either change the copied
        ...
        for rschema in list(self.e_schema.subject_relations.values()):
            ...
            if rschema.inlined:
                ...
                if all(
                    ...
                    and rschema.relation_definition(self.e_schema, e).get_groups("read")
                    for e in targets
                ):
    ...
    def _cw_to_complete_attributes(self, skip_bytes=True, skip_pwd=True):
        for rschema, attrschema in self.e_schema.attribute_definitions():
            ...
            rdef = rschema.relation_definition(self.e_schema, attrschema)
    ...
    def _cw_compute_linkable_rql(
        ...
        rdef = rschema.role_relation_definition(self.e_schema, targettype, role)
    ...
    def cw_set_relation_cache(self, rtype, role, rset):
        ...
        if rset:
            ...
            rschema = self._cw.vreg.schema.relation_schema_for(rtype)
            if role == "subject":
                rcard = rschema.relation_definition(
                    self.e_schema, related[0].e_schema
                ).cardinality[1]
                target = "object"
            else:
                rcard = rschema.relation_definition(
                    related[0].e_schema, self.e_schema
                ).cardinality[0]
                target = "subject"
            if rcard in "?1":
