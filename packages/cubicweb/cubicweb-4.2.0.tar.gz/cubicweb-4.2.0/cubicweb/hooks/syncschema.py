...
def insert_rdef_on_subclasses(cnx, eschema, rschema, rdefdef, props):
    ...
    object = rschema.schema.entity_schema_for(rdefdef.object)
    for specialization in eschema.specialized_by(False):
        if (specialization, rdefdef.object) in rschema.relation_definitions:
            continue
...
class _MockEntity:  # XXX use a named tuple with python 2.6
    ...
    def precommit_event(self):
        ...
        for rtype in META_RTYPES - VIRTUAL_RTYPES:
            try:
                ...
                continue
            if not rschema.relation_definitions:
                self.warning("rtype %s has no relation definition yet", rtype)
                continue
            sampletype = rschema.subjects()[0]
            desttype = rschema.objects()[0]
            try:
                rdef = copy(rschema.relation_definition(sampletype, desttype))
            except KeyError:
...
class CWRTypeUpdateOp(MemSchemaOperation):
    ...
    def precommit_event(self):
        ...
        if "fulltext_container" in self.values:
            ...
            for subjtype, objtype in rschema.relation_definitions:
    ...
    * register an operation to add the relation definition to the
    ...
    def init_rdef(self, **kwargs):
        ...
        try:
            ...
            # probably buggy)
            rdef = self.cnx.vreg.schema.rschema(rdefdef.name).relation_definitions[
                rdefdef.subject, rdefdef.object
            ]
            assert rdef.infered
        else:
            rdef = self.cnx.vreg.schema.relation_schema_for(
                rdefdef.name
            ).relation_definitions[rdefdef.subject, rdefdef.object]
    ...
    def precommit_event(self):
        ...
        schema = cnx.vreg.schema
        try:
            eschema = schema.entity_schema_for(rdefdef.subject)
        except KeyError:
            return  # entity type currently being added
        # propagate attribute to children classes
        rschema = schema.relation_schema_for(rdefdef.name)
        ...
        if getattr(entity, "formula", None):
            ...
            rdefdef.relation_type = rdefdef.name
...
class CWRelationAddOp(CWAttributeAddOp):
    ...
    * if this is an inlined relation, add the necessary column else if it's the
    ...
    def precommit_event(self):
        ...
        schema = cnx.vreg.schema
        rtype = rdefdef.name
        rschema = schema.relation_schema_for(rtype)
        ...
        if rschema.inlined:
            ...
            if rschema.symmetric:
                ...
                relation_already_defined = len(rschema.relation_definitions) > 2
            else:
                relation_already_defined = len(rschema.relation_definitions) > 1
...
class RDefUpdateOp(MemSchemaOperation):
    ...
    def precommit_event(self):
        cnx = self.cnx
        rdef = self.rdef = self.rschema.relation_definitions[self.rdefkey]
        # update the in-memory schema first
        ...
        for prop in ("symmetric", "inlined", "fulltext_container"):
            if prop in entity.cw_edited:
                ...
                if old != new:
                    newvalues[prop] = new
        if newvalues:
            rschema = self._cw.vreg.schema.relation_schema_for(entity.name)
