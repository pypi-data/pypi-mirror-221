...
class SchemaModificationHooksTC(CubicWebTC):
    ...
    def test_base(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertNotIn(
                "name", list(schema["Societe2"].subject_relations.values())
            )
            self.assertNotIn(
                "concerne2", list(schema["Societe2"].subject_relations.values())
            )
            self.assertFalse(self.index_exists(cnx, "Societe2", "name"))
            cnx.commit()
            self.assertIn("name", list(schema["Societe2"].subject_relations.values()))
            self.assertIn(
                "concerne2", list(schema["Societe2"].subject_relations.values())
            )
            ...
            cnx.commit()
            self.assertIn(
                "concerne2", list(schema["CWUser"].subject_relations.values())
            )
            self.assertNotIn(
                "concerne2", list(schema["Societe2"].subject_relations.values())
            )
    ...
    def test_perms_synchronization_2(self):
        with self.admin_access.repo_cnx() as cnx:
            schema = self.repo.schema["in_group"].relation_definitions[
                ("CWUser", "CWGroup")
            ]
    ...
    def test_indexed_change(self):
        with self.admin_access.repo_cnx() as cnx:
            try:
                ...
                self.assertTrue(
                    self.schema["name"]
                    .relation_definition("Workflow", "String")
                    .indexed
                )
    ...
    def test_update_constraint(self):
        with self.admin_access.repo_cnx() as cnx:
            rdef = self.schema["Transition"].relation_definition("type")
    ...
    def test_add_constraint(self):
        with self.admin_access.repo_cnx() as cnx:
            rdef = self.schema["EmailPart"].relation_definition("ordernum")
    ...
    def test_eschema_composite_properties(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            parts_rdef = email_eschema.relation_definition("parts")
