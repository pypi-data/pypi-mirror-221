...
class MassiveObjectStore(stores.RQLObjectStore):
    ...
    def __init__(
        ...
        self._data_entities_to_update = defaultdict(list)
        self._initialized = {}
    ...
    def _drop_metadata_constraints(self):
        ...
        rtypes = [
            ...
            if not self.schema.relation_schema_for(rtype).final
        ]
    ...
    def prepare_update_entity(self, etype, eid, **data):
        """Given an entity type, eid, attributes and inlined relations, returns
        the updated entity's eid.
        """
        entity = self._cnx.entity_from_eid(eid)
        assert entity.cw_etype == etype, "Trying to update with wrong type %s" % etype
        initialized_key = f"update_{etype}"
        if initialized_key not in self._initialized:
            if not self.slave_mode:
                self.master_init(commit=False)
            tablename = f"cw_{etype.lower()}_to_update"
            tmp_tablename = "%s_%s" % (tablename, self.uuid)
            self.sql(
                "INSERT INTO cwmassive_initialized VALUES (%(e)s, 'update_etype', %(uuid)s)",
                {"e": etype, "uuid": self.uuid},
            )
            attr_defs = eschema_sql_def(self._source_dbhelper, self.schema[etype])
            self.sql(
            ...
            self._initialized[initialized_key] = [attr for attr, _ in attr_defs]

        # we do that to avoid a lot of warning :)
        completed_data = {attr: None for attr in self._initialized[initialized_key]}
        self._data_entities_to_update[etype].append(
            {**completed_data, **data, "eid": eid}
        )
        return eid
    ...
    def _finish(self):
        ...
        if self._dbh.table_exists("cwmassive_initialized"):
            ...
            entities_to_update = defaultdict(list)
            for retype, _type, uuid in cu.fetchall():
                if _type == "rtype":
                    relations[retype].append(uuid)
                elif _type == "update_etype":
                    entities_to_update[retype].append(uuid)
                else:  # _type = 'etype'
            ...
            # get back entity to update data from the temporary tables
            for etype, uuids in entities_to_update.items():
                tablename = "cw_%s" % etype.lower()
                attr_defs = eschema_sql_def(self._source_dbhelper, self.schema[etype])
                set_parts = []
                for attr, _ in attr_defs:
                    if attr == "eid":
                        continue
                    set_parts.append(
                        f"cw_{attr} = COALESCE(TMP.cw_{attr}, {tablename}.cw_{attr})"
                    )

                self._dbh.drop_constraints(tablename)
                for uuid in uuids:
                    tmp_tablename = "%s_to_update_%s" % (tablename, uuid)
                    self.sql(
                        f"""
                        UPDATE {tablename}
                        SET {",".join(set_parts)}
                        FROM {tmp_tablename} as TMP
                        WHERE {tablename}.cw_eid = TMP.cw_eid
                        """
                    )
                    self._tmp_data_cleanup(tmp_tablename, f"update_{etype}", uuid)
    ...
    def flush_entities_to_update(self):
        """Flush the entities to update data from in-memory structures to a temporary table."""
        for etype, data in self._data_entities_to_update.items():
            if not data:
                # There is no data for these etype for this flush round.
                continue
            attrs = self._initialized[f"update_{etype}"]
            buf = pgstore._create_copyfrom_buffer(data, attrs)
            tablename = f"cw_{etype.lower()}_to_update"
            tmp_tablename = f"{tablename}_{self.uuid}"
            columns = ["cw_%s" % attr for attr in attrs]
            cursor = self._cnx.cnxset.cu
            cursor.copy_from(buf, tmp_tablename, null="NULL", columns=columns)
            # Clear data cache
            self._data_entities_to_update[etype] = []
