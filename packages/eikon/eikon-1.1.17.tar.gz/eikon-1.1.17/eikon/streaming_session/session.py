...
class Session(object):
    ...
    async def http_request_async(self, url: str, method=None, headers={},
        ...
        if self._dp_proxy_app_version:
            headers.update({"app-version": self._dp_proxy_app_version})

        if self._user_uuid:
            cookies = {"user-uuid": self._user_uuid}
        ...
        # in httpx 0.20.0 argument name was changed
        if version_parse(httpx.__version__) < version_parse("0.20.0"):
            redirects_keyword = "allow_redirects"
        else:
            redirects_keyword = "follow_redirects"

        kwargs[redirects_keyword] = True
        ...
        try:
            ...
            self.log(
                1,
                f'Send request with headers '
                f'{_request_response.request.headers.raw} and cookies {cookies}'
            )
