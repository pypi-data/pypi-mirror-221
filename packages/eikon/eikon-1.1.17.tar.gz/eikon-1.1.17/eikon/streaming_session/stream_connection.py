...
class StreamConnection(threading.Thread):
    ...
    def register_streaming_session(cls, streaming_session):
        with cls.__register_lock:
            if not streaming_session:
                raise EikonError(
                    'Error', 'Try to register unavailable streaming session')
    ...
    def unregister_streaming_session(cls, streaming_session):
        with cls.__register_lock:
            if not streaming_session:
                raise EikonError(
                    'Error', 'Try to unregister unavailable streaming session')
            if streaming_session.streaming_session_id is None:
                raise EikonError(
                    'Error', 'Try to unregister unavailable streaming session id')
            ...
            cls.__all_streaming_session.pop(
    ...
    def __del__(self):
        self.log(1, 'StreamConnection {} is releasing'.format(
            self._streaming_session_id))
        if self._websocket:
            try:
                if self._websocket.keep_running:
                    # Close web socket
                    self.log(1, "Close websocket client {}".format(
                        self._streaming_session_id))
                ...
                self.log(
                    1, f'Exception on close websocket attempt for main stream {self._streaming_session_id}: {e!r}')
                pass
        if self._streaming_session_id in StreamConnection.__all_streaming_session:
            self.log(1, "Unregister streaming session {}".format(
                self._streaming_session_id))
    ...
    def run(self):
        ...
        while not self.is_closing:
            self.log(
                1, f"Streaming session {self._streaming_session_id} waits for start event")
            ...
            if not self.is_closing:
                ...
                self.log(
                    1, f"Streaming session {self._streaming_session_id} received start event, then open websocket.")

                self._websocket = websocket.WebSocketApp(
                    self._streaming_config.url,
                    header=["User-Agent: Python"] + self._streaming_config._headers,
                    cookie=cookie,
                ...
                self.log(
                    1,
                    f"Websocket connection open with headers "
                    f"{self._websocket.header} and cookie {self._websocket.cookie}",
                )
                ...
                self._websocket = None
                self.log(1, "Websocket for streaming session {} was closed".format(
                    self._streaming_session_id))
        self.log(1, "Streaming session {} will be closed".format(
            self._streaming_session_id))
    ...
    def _send(self, msg):
        try:
            ...
            self._logger.log(
                logging.ERROR, "WebSocketConnectionClosedException: {}".format(e))
    ...
    def _process_message(self, message_json):
        ...
        if _id == self._ws_login_id:
            self.log(Session.TRACE,
                     f"Receive message for login {_id}: {message_json}")
        else:
            self.log(Session.TRACE,
                     f"Receive message for stream {_id}: {message_json}")
    ...
    def _process_login_response(self, response):
        ...
        if id != self._ws_login_id:
            self.log(
                logging.DEBUG, f'Received login response for id {id} different than login id {self._ws_login_id}')
        else:
            self.log(logging.DEBUG,
                     f'Received login response for login id {id}')
    ...
    def _process_status_message(self, status):
        ...
        _stream = str(status["State"]["Stream"]
                      ) if "State" in status and "Stream" in status["State"] else None
        ...
        if mp_subscription:
            ...
            self.log(
                logging.WARNING, "Receive status message for unknown subscription id {}".format(_id))
    ...
    def _process_refresh_message(self, refresh):
        ...
        _complete = str(refresh["Complete"]).lower(
        ) == "true" if refresh.get("Complete") else True

        # EventCode value determines further interest
        _stream = str(refresh["State"]["Stream"]) if refresh.get(
            "State") and refresh["State"].get("Stream") else None
        ...
        if mp_subscription:
            ...
            # A completion occurs when "Complete" == true or the "Complete" field is absent
            complete = True if refresh.get(
                "Complete") is None else bool(refresh["Complete"])
            if complete:
                mp_subscription._on_complete()
        else:
            self.log(
                logging.WARNING, "Receive refresh message for unknown subscription {}".format(_id))
    ...
    def _process_update_message(self, update):
        ...
        if mp_subscription:
            ...
            self.log(
                logging.WARNING, "Receive update message for unknown subscription {}".format(_id))
    ...
    def _process_error_message(self, error):
        ...
        if mp_subscription:
            ...
            self.log(
                logging.WARNING, "Receive error message for session {} : {}".format(_id, error))
            self._on_event_cb(self._streaming_session_id,
                              Session.EventCode.StreamDisconnected,
                              error)
        else:
            self.log(logging.WARNING, "Receive error message for unknown subscription {} : {}".format(
                _id, error))
