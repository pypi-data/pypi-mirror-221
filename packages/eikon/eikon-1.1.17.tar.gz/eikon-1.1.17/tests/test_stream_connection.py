...
def test_websocket_callback_ws_open():
    ...
    omm_stream_connetion = eikon.streaming_session.stream_connection.StreamConnection(
        name,
        session,
        stream_connection_name,
        connection_config,
    ...
    try:
        #   version 0.54 - 0.57
        omm_stream_connetion._ws_open()
        #   version 0.58
        omm_stream_connetion._ws_open("ws")
    except Exception as e:
...
def test_websocket_callback_ws_error():
    ...
    try:
        #   version 0.54 - 0.57
        omm_stream_connetion._ws_error(mock_data)
        #   version 0.58
        omm_stream_connetion._ws_error("ws", mock_data)
    except Exception as e:
...
def test_websocket_callback_ws_message():
    ...
    try:
        #   version 0.54 - 0.57
        omm_stream_connetion._ws_message(json.dumps(mock_data))
        #   version 0.58
        omm_stream_connetion._ws_message("ws", json.dumps(mock_data))
    except Exception as e:
        assert False, "NO EXCEPTION EXPECTED BUT GOT {e}"
...
def test_websocket_callback_ws_close():
    ...
    try:
        #   version 0.54 - 0.57
        omm_stream_connetion._ws_close(mock_data)
        #   version 0.58
        omm_stream_connetion._ws_close("ws", mock_data)
    except Exception as e:
        assert False, "NO EXCEPTION EXPECTED BUT GOT {e}"
