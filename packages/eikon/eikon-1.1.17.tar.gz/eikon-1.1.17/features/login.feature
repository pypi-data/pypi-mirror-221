Feature: Eikon login test
	...
	Scenario: Application key not set
		Given the daemon is running
		And application key is not set
		When I send JSON request
		Then an exception EikonError is raised with text: "Can not process metadata for request: $AAAREST_ValidateAppId.baseAppId - non-empty is required"}"
...
#		Then an exception EikonError is raised with text: "Application ID is not valid"
	...
	#This needs investigation, as we can not control proxy up or down from the eikon lib.
