...
def get_symbology(symbol, from_symbol_type='RIC', to_symbol_type=None, raw_output=False, debug=False, best_match=True):
    """
    ...
    if type(best_match) is not bool:
        error_msg = f"best_match {best_match} value has not bool type"
        raise ValueError(error_msg)

    payload = {'symbols': symbol, 'from': from_symbol_type, 'to': to_symbol_type, 'bestMatchOnly': best_match}
