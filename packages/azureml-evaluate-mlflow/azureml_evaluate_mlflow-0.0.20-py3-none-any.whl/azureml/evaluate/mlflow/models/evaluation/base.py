...
class EvaluationDataset:
    ...
    def __eq__(self, other):
        ...
        return (
                is_features_data_equal  # noqa: E126
...
def list_evaluators():
    ...
    from azureml.evaluate.mlflow.models.evaluation.evaluator_registry import _model_evaluation_registry
...
def log_dataset_tag(client, run_id, model, dataset):
    if hasattr(log_dataset_tag, '_executed_once'):
        return
    model_uuid = model.metadata.model_uuid
    dataset._log_dataset_tag(client, run_id, model_uuid)
    log_dataset_tag._executed_once = True
...
def _evaluate(
    ...
    for evaluator_name in evaluator_name_list:
        ...
        try:
            ...
            continue
        model = evaluator.load_model(raw_model, model_type)

        log_dataset_tag(client=client, run_id=run_id, model=model, dataset=dataset)
