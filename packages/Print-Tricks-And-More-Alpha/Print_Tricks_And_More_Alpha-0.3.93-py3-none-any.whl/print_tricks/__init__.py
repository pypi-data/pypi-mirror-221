############################################################################
######################## Print Tricks & More - version 0.3.93 working ######
############################################################################
...
class pt:
    ...
    new_trace_level = -3  ## I've moved the new_trace_level out of _simpleTrace and to here, so that I can debug my own print_tricks statements using ... ###db() now. 
...
class UTI:
        ...
        try:
            while True:
                ...
                if "UTI._simpleTrace(" in code:
                    ...
                    # print('found _simpleTrace in code, increasing new trace level to: ', pt.new_trace_level)
    ...
    def _traceProcessing(filePath, lineNo, code, argsLen):
        ...
        if ";" in code:
            ...
            # ... ###db(ptCount)
            code, cur_count = UTI._track_pt_on_one_line(code, ptCount)
            # ... ###db(code)
        check_line = lineNo
        count_a = code.count("(")
        count_b = code.count(")")
...
class NAM:
    ...
    # replace_these_words = [(f'{inst_nam}.{word}(') for word in func_keys]
