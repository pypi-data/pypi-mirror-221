...
contains
  ...
  subroutine adjust(potential)
    ...
    do isp = 1,nsp
       do jsp = 1,nsp
          ...
          a_cut(isp,jsp) =   w * SQRT(rcutsq)
          b_cut(isp,jsp) = - w * rcutsq - u
       end do
    end do
  ...
  subroutine smooth(isp,jsp,rsq,uij,wij,hij)
    ...
    uij = uij + a_cut(isp,jsp) * sqrt(rsq) + b_cut(isp,jsp)
    wij = wij - a_cut(isp,jsp) / sqrt(rsq)
    hij = hij
