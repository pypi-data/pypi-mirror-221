...
@app.command('convert')
def cli_make_new_app(input: Annotated[Path, typer.Argument(help="Base app file.")],
    ...
    try:
        cv = ConvertService(workspace, input, Path(
...
@app.command('bake')
def cli_convert_onnx_to_engine(input: Annotated[Path, typer.Argument(help="ONNX model input file.")],
                               output: Annotated[Path, typer.Option('-O', help='TensorRT engine output path.')],
                               precision: Annotated[Precision, typer.Option('-P', '--precision', case_sensitive=False,
                                                                            help='Precision of Model. One of the int8, fp16 and fp32')],
                               batch: Annotated[int, typer.Option('-B', '--batch',
                                                                  help='Batch size of model')],
                               input_shapes: Annotated[str, typer.Option('-S', '--shapes',
                                                                         help='Input shapes of model. Example: 3x640x640')],
                               queit: Annotated[bool, typer.Option(help="Do not print anything.")] = False):
    """
    ...
    bs = BakeService(workspace, input, output,
                     options["dx_target_version"], options['gpu_num'])
    ...
    try:
        bs.convert_model(not queit, precision, input_shapes, batch)
    except WaffleBoxException as e:
        print(e)
        raise typer.Exit(-1)
    except Exception as e:
