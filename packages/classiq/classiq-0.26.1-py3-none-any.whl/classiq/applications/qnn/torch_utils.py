...
def einsum_weigths(grad_output: Tensor, grad_weights: Tensor) -> Tensor:
    if len(grad_weights.shape) == 2:
        return torch.einsum("i...,ip...->p", grad_output, grad_weights)
    return torch.einsum("ip...,ipl...->pl", grad_output, grad_weights)


def einsum_inputs(grad_output: Tensor, grad_inputs: Tensor) -> Tensor:
