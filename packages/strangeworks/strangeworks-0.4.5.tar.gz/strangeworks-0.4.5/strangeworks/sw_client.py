...
class SWClient:
    ...
    def execute_post(
        ...
        """Execute a job request."""
        _resource = self.get_resource_for_product(product_slug)
        _url = (
            urljoin(_resource.proxy_url(), endpoint)
            if endpoint
            else _resource.proxy_url()
        )
        return self.rest_client.post(url=_url, json=payload)
    ...
    def execute_get(self, product_slug: str, endpoint: Optional[str] = None):
        ...
        return self.rest_client.get(url=_url)
