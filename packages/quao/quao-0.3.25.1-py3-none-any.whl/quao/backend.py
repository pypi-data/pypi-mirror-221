...
class Backend:
    ...
    def __pre_execute_job(self, circuit):
        ...
        pool.submit(self.__export_circuit, circuit).result()
    ...
    def __export_circuit(self, circuit):
        """
          ...
          Args:
              ...
              @param circuit:
        """
        logger.debug("export url {}".format(self.circuit_export_url))

        if self.circuit_export_url is None or len(self.circuit_export_url) < 1:
            return

        logger.debug("Export circuit thread: {}".format(threading.current_thread().name))
