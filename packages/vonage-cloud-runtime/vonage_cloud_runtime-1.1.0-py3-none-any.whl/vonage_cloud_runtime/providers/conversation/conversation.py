...
class Conversation(IConversation):
    ...
    async def onConversationEvent(self,callback: str,events: List[str] = None):
        ...
        if events is not None:
            for i in range(0,events.__len__()):
                filters.append(Filter("type","contains",[events[i]]))
            
        
        rp = RequestParams()
        rp.method = REQUEST_VERB.POST
        rp.data = ConversationPayloadWithCallback(self.session.wrapCallback(callback,filters),self.session.config.apiApplicationId)
