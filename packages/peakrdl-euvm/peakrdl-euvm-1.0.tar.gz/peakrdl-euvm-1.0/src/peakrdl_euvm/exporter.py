    ...
    def export(self, node: Node, path: str, **kwargs):
        ...
        if isinstance(node, AddrmapNode) and node.get_property('bridge'):
            node.env.msg.warning(
                "eUVM RAL generator does not have proper support for bridge addmaps yet. The 'bridge' property will be ignored.",
        ...
        if export_as_package:
            ...
            template = self.jj_env.get_template("top_pkg.d")
        else:
            context['include_guard'] = self._get_include_guard(path)
            template = self.jj_env.get_template("top_include.d")
