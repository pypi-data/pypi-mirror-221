...
def mesh_to_proto(mesh: Trimesh) -> Geometry:
    ...
    return Geometry(
        mesh=Mesh(vertices=mesh.vertices.reshape(-1), faces=mesh.faces.reshape(-1))
    )
...
def proto_to_mesh(proto: Geometry) -> Trimesh:
    ...
    if proto.HasField("mesh"):
        # reshape vertices into array
        vertices = np.array(proto.mesh.vertices, dtype=np.float64).reshape((-1, 3))
        # transform the vertices if requested
        if transform is not None:
            vertices = transform_points(vertices, matrix=transform)
        return Trimesh(
            vertices=vertices,
            faces=np.array(proto.mesh.faces, dtype=np.int64).reshape((-1, 3)),
        )
