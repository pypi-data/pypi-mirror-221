"""
Usage: odoo-manager build [--no-cache] [--no-depends] [--no-setup] [--pull]
                          [--user=<user>] [--password=<password>]
                          [-v | --verbose] [-h | --help]

Options:
  -h, --help                Show this screen
  --no-cache                Use `docker compose build --no-cache ...`
  --no-depends              (not implemented...)
  --no-setup                (not implemented...)
  --password=<password>     (not implemented...)
  --pull                    Use `docker compose build --pull ...`
  --user=<user>             (not implemented...)
  -v, --verbose             Output additional information, where possible

Run `docker compose build` and download Odoo source code, as well some other
minor steps to ensure the repository is ready for use.

See `docker compose build --help` for details about its related options being
used here.
...
class Build(BaseCommand):
    ...
    def run(self):
        ...
        shell.out("      odoo version:      {}".format(odoo_version), color="light_grey")
        shell.out("      using enterprise?: {}".format(use_enterprise), color="light_grey")
        shell.out("      repo depth:        {}".format(depth), color="light_grey")
        shell.out("", color="light_grey")
        ...
        if use_enterprise:
            if not os.path.isdir(paths.base(".container/enterprise/account_reports")):
                ...
                git_clone(
                    self.ctx,
                    "git@github-bluestingray:odoo/enterprise",
                    odoo_version,
                    "odoo/enterprise",
