"""
Usage: odoo-manager format (html|python|xml) [--source=<directory_or_file>]
                           [-v | --verbose] [-h | --help]
...
Auto-format the given file using 'black' (python) or 'xmllint' (html, xml).
Specify the type of content being formatted, as well as the source file path.
...
class Format(BaseCommand):
    ...
    def run_html(self):
        ...
        if not source:
            shell.out("A source file must be provided.", color="magenta", is_error=True)
        if os.path.isdir(source):
            shell.out("Formatting directories is not currently supported.", color="magenta", is_error=True)
        ...
        try:
            ...
            try:
                ...
                shell.out(
                    "Source file failed to format even with encoding and wrapper div", color="magenta", is_error=True
                )
                raise error
