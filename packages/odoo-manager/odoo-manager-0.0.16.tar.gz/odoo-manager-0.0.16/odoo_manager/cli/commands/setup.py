"""
Usage: odoo-manager setup (dependencies|mapping|version) [-y | --yes]
                          [--only=<modules>] [--no-update]
                          [-v | --verbose] [-h | --help]

Options:
  -h, --help                Show this screen
  --no-update               Only install new modules (instead of updating existing)
  --only=<modules>          Only set up certain modules (instead of everything in the '__manifest__.py' file)
                            Example: --only=example1,example2
  -v, --verbose             Output additional information, where possible
  -y, --yes                 Overwrite '_lib/' contents without prompting for manual approval
...
class Setup(BaseCommand):
    ...
    def run_dependencies(self):
        ...
        if "depends" in configuration:
            ...
            if problem_modules:
                shell.out("The following modules had some problem ...", color="light_red")
                shell.out("  " + str.join("\n  ", problem_modules), color="light_grey")

            if effected_modules:
                shell.out("")
                shell.out("Adding the following modules to _lib...", color="light_blue")
                shell.out("\n---\n", color="light_blue")
                shell.out("  " + str.join("\n  ", effected_modules), color="light_grey")
                shell.out("\n---\n", color="light_blue")
