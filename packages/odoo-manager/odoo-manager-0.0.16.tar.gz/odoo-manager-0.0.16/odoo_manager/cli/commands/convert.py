"""
Usage: odoo-manager convert [--module=<module_name>] [--append] [--install]
                            [--source=<file>] [--dest=<file>]
                            [--from=<file_type>] [--to=<file_type>]
                            [--with-format] [-v | --verbose]

Options:
  --append                  Convert, then overwrite the content in place
  --dest=<file>             The file path where the result should be saved
  --from=<file_type>        The input file type, such as "md"
  -h, --help                Show this screen
  --install                 Try to ensure pandoc is installed
  --module=<module_name>    The name of the module being converted; this will automate other options if they are left blank
  --source=<file>           The file path containing the content to be converted
  --to=<file_type>          The output file type, such as "html"
  -v, --verbose             Output additional information, where possible
  --with-format             Automatically `odoo-manager format...` after converting
...
other types of supported formats.
  - Python package docs: https://pypi.org/project/pypandoc
  - System package docs: https://pandoc.org/MANUAL.html
...
class Convert(BaseCommand):
    ...
    def run(self):
        ...
        if dest and with_format:
            if to_type not in Format({}).supported_commands + Format({}).supported_extensions:
                shell.out("Formatting '{}' files is not supported".format(to_type), color="magenta", is_error=True)
                exit(1)
    ...
    def _get_destination_file(self, to_type):
        ...
        if dest and not os.path.isfile(paths.base(dest)):
            shell.out("The destination path ({}) does not appear to exist...".format(dest), color="magenta", is_error=True)
            exit(1)
        ...
        if not dest and not to_type:
            shell.out("You must provide a destination path or a file type to convert to", color="magenta", is_error=True)
            exit(1)
    ...
    def _get_source_file(self):
        ...
        if not source or not os.path.isfile(paths.base(source)):
            shell.out("The source path does not appear to exist...", color="magenta", is_error=True)
