...
async def create_user_message(prompt):
    return ChatCompletionRequestMessage(role="user", content=prompt)

async def create_system_message(context):
...
async def get_chat_response(chat_completion_request):
...
async def upsert_vectors_to_pinecone(user_embedding, openai_embedding, prompt, text, namespace):
...
async def chat_pipeline(prompt: str, namespace: str="default", retrieval:bool=True, context:Optional[str]=None) -> str:
    try:
...
async def chainspect(text:str, context:Optional[str]=None, retrieval=False, iterations:int=8):
    texts = []
    for i in range(iterations):
        text = await chat_pipeline(prompt=text, context=context, retrieval=retrieval)
        texts.append(text)
