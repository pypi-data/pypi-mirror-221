...
from .client import APIClient, APIConfig, HTTPException
...
pinecone = make_client(base_url=os.getenv("PINECONE_BASE_URL"), headers={"api-key": os.getenv("PINECONE_API_KEY")})
openai = make_client(base_url=os.getenv("OPENAI_BASE_URL"), headers={"Authorization": f"Bearer {os.getenv('OPENAI_API_KEY')}"})
...
@handle_errors
@process_time
async def get_previous_statements(user_embedding, namespace):
...
@handle_errors
@process_time
async def get_chat_response(chat_completion_request:CreateChatCompletionRequest)->str:
    text = await create_chat_completion(chat_completion_request)
...
@handle_errors
@process_time
async def get_embeddings(text:str)->Vector:
...
@handle_errors
@process_time
async def upsert_vectors_to_pinecone(user_embedding:Vector, openai_embedding:Vector, prompt:str, text:str, namespace:str)->None:
...
async def chat_completion(prompt: str, namespace: str="default", retrieval:bool=True, context:Optional[str]=None) -> str:
    ...
    if isinstance(context, str):
        ...
        chat_completion_request = await create_chat_request([user_message])
    text = await get_chat_response(chat_completion_request)
    if retrieval:
        user_embedding = await get_embeddings(prompt)
        openai_embedding = await get_embeddings(text)
        await upsert_vectors_to_pinecone(user_embedding, openai_embedding, prompt, text, namespace)
    return text
...
@process_time
@handle_errors
async def lang_chain(text:str, iterations:int=8):
    texts = []
    for i in range(iterations):
        text = await chat_completion(prompt=text, retrieval=False)
        texts.append(text)
