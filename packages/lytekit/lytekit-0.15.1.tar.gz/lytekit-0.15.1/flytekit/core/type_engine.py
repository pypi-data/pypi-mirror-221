        ...
        if dataclasses.is_dataclass(type_):
            if dcj_utils._issubclass_safe(type_, mixin):
                options["field_many"] = bool(
                    dcj_core._is_supported_generic(field.type) and dcj_utils._is_collection(field.type)
                )
...
class DataclassTransformer(TypeTransformer[object]):
        ...
        if not issubclass(t, DataClassJsonMixin):
            return LiteralType(
                record_type=RecordType(
                    [RecordFieldType(f.name, TypeEngine.to_literal_type(f.type)) for f in dataclasses.fields(t)]
                )
            )
        ...
        try:
            ...
            class FlyteJSONSchema(JSONSchema):
                def get_required(self, obj):
                    return sorted([field.data_key or field.name for field in obj.fields.values()])
    ...
    def to_literal(self, ctx: FlyteContext, python_val: T, python_type: Type[T], expected: LiteralType) -> Literal:
        ...
        if not issubclass(type(python_val), DataClassJsonMixin):
            fields = [
                RecordField(
                    f.name,
                    TypeEngine.to_literal(ctx, getattr(python_val, f.name), f.type, TypeEngine.to_literal_type(f.type)),
                )
    ...
    def _flyte_type_superhack(self, v: T, field_type: Type[T], schema, defs):
        ...
        if inspect.isclass(field_type) and (
            issubclass(field_type, FlyteFile)
            ...
            self._dataclass_superhack(v, field_type, defs[schema["$ref"][len("#/definitions/") :]], defs)
                ...
                union_type=UnionType(
                    variants=[
                        LiteralType.from_flyte_idl(
                            _ParseDict(x["properties"]["type"]["const"], _types_pb2.LiteralType())
                        )
                        for x in schema
                    ]
                )
            ...
            lit, pytype, idx = tr._to_literal_ext(FlyteContext.current_context(), v, field_type, expected)

            assert lit.scalar is not None
            assert lit.scalar.union is not None

            return {
                "value": self._flyte_type_superhack(v, pytype, schema[idx]["properties"]["value"], defs),
                "type": _MessageToDict(lit.scalar.union.stored_type.to_flyte_idl()),
            }
        elif get_origin(field_type) is list:
            value_type = get_args(field_type)[0]
            return [self._flyte_type_superhack(x, value_type, schema["items"], defs) for x in v]
    ...
    def _dataclass_superhack(self, python_val: T, python_type: Type[T], schema, defs):
        for f in dataclasses.fields(python_type):
            ...
            field_type = f.type
            setattr(python_val, f.name, self._flyte_type_superhack(v, field_type, schema["properties"][f.name], defs))
    ...
    def _deserialize_superhack(self, python_val, expected_python_type: Type):
        ...
        if dataclasses.is_dataclass(expected_python_type) and isinstance(python_val, dict):
        ...
        if get_origin(expected_python_type) is typing.Union:
            stored_type = LiteralType.from_flyte_idl(_ParseDict(python_val["type"], _types_pb2.LiteralType()))
            ...
            raise TypeTransformerFailedError(f"could not find a suitable instantiation for {expected_python_type}")
    ...
    def _dataclass_deserialize_superhack(self, python_val: T, expected_python_type: Type):
        for f in dataclasses.fields(expected_python_type):
            ...
            if hasattr(f.type, "__origin__") and f.type.__origin__ is list:
                value = [self._deserialize_superhack(v, f.type.__args__[0]) for v in value]
            elif hasattr(f.type, "__origin__") and f.type.__origin__ is dict:
                value = {k: self._deserialize_superhack(v, f.type.__args__[1]) for k, v in value.items()}
            else:
    ...
    def to_python_value(self, ctx: FlyteContext, lv: Literal, expected_python_type: Type[T]) -> T:
        ...
        if not issubclass(expected_python_type, DataClassJsonMixin):
            ...
            for f in lv.record.fields:
                ...
                fields[f.key] = TypeEngine.to_python_value(ctx, f.value, field_types[f.key])
            ...
            try:
                ...
                raise TypeTransformerFailedError(f"{lv} failed to convert to dataclass {expected_python_type}")
        ...
        try:
            ...
            raise TypeTransformerFailedError(f"Did not match JSON schema for dataclass {expected_python_type}") from e

        dc = cast(DataClassJsonMixin, expected_python_type).from_dict(data)
        return self._fix_dataclass_int(
            expected_python_type, self._dataclass_deserialize_superhack(dc, expected_python_type)
        )
...
class UnionTransformer(TypeTransformer[T]):
    ...
    def _to_literal_ext(
        ...
        if found_res:
            ...
            return Literal(scalar=Scalar(union=Union(value=res, stored_type=res_type))), res_pytype, res_idx
...
class EnumTransformer(TypeTransformer[enum.Enum]):
    ...
    def get_literal_type(self, t: Type[T]) -> LiteralType:
        if get_origin(t) is Annotated:
            raise ValueError(
                "Flytekit does not currently have support                     for FlyteAnnotations applied to enums."
                f" {t} cannot be                     parsed."
            )
