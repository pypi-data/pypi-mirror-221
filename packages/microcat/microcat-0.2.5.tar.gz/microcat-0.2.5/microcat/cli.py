...
def single_init(
    ...
    # Create a MicrocatConfig object using the provided working directory
        ...
        if os.path.exists(workdir):
            logger.warning(f"WARNING: The working directory '{workdir}' already exists.")
...
def config(
    ...
    if cellranger_ref is not None and not edited:
        ...
        single_config["host"]["cellranger"]["reference"] = cellranger_ref
    ...
    if starsolo_ref is not None and not edited:
        ...
        single_config = parse_yaml(single_config_file)
        single_config["host"]["starsolo"]["reference"] = starsolo_ref
        update_config(single_config_file, single_config_file, single_config)
    ...
    if krak2_ref is not None and not edited:
        # Spatial and single both have starsolo refernence 
        single_config = parse_yaml(single_config_file)
        single_config["classifier"]["kraken2uniq"]["kraken2_database"] = krak2_ref
        update_config(single_config_file, single_config_file, single_config)

        spatial_config = parse_yaml(spatial_config_file)
        spatial_config["classifier"]["kraken2uniq"]["kraken2_database"] = krak2_ref
        update_config(spatial_config_file, spatial_config_file, spatial_config)

        bulk_config = parse_yaml(bulk_config_file)
        bulk_config["classifier"]["kraken2uniq"]["kraken2_database"] = krak2_ref
    ...
    if krakuniq_ref is not None and not edited:
        # Spatial and single both have starsolo refernence 
        single_config = parse_yaml(single_config_file)
        single_config["classifier"]["krakenuniq"]["krakenuniq_database"] = krakuniq_ref
        update_config(single_config_file, single_config_file, single_config)

        spatial_config = parse_yaml(spatial_config_file)
        spatial_config["classifier"]["krakenuniq"]["krakenuniq_database"] = krakuniq_ref
        update_config(spatial_config_file, spatial_config_file, spatial_config)

        bulk_config = parse_yaml(bulk_config_file)
        bulk_config["classifier"]["krakenuniq"]["krakenuniq_database"] = krakuniq_ref
        update_config(bulk_config_file, bulk_config_file, bulk_config)
...
def run_remote(
    ...
    if "--touch" in unknown:
        ...
        if cluster_engine == "generic":
            profile_conf =  parse_yaml(
                                                os.path.join(profile_path, "config.yaml")
                                                )
            submmit_conf = parse_yaml(
                                                os.path.join(profile_path, "key_mapping.yaml")
                                                )
