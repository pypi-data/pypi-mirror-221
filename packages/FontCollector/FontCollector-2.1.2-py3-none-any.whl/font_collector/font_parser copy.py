...
class FontParser:
    ...
    @staticmethod
    def get_var_font_italic_bold_property(
        font: TTFont, axis_value_tables
    ) -> Tuple[bool, int]:
        """
        Parameters:
        ...
        # GDI seems to have default value when an coordinates (wght or ital) doesn't exist.
        # In brief, it doesn't take the from OS/2 or the head table.
        weight = 400
        is_italic = False

        for axis_value in axis_value_tables:

            if axis_value.Format == 4:
                is_italic, weight = FontParser.get_var_font_italic_bold_property(
                    font, axis_value.AxisValueRecord
                )
            else:
                axis_tag = (
                    font["STAT"]
