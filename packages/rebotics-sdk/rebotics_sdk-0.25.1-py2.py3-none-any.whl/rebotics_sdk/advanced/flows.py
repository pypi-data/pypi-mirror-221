...
class PresignedURLFileUploader:
    ...
    def upload(self, file, **kwargs) -> Optional[NoReturn]:
        response = remote_loaders.upload(self.destination, file, progress_bar=True, **kwargs)
...
class RCDBImportFlow(PresignedURLFileUploader):
    def __init__(self, rcdb_interface, retailer, retailer_model, extension):
        ...
        response = self.rcdb.data_import(retailer, retailer_model, extension)
        self.rcdb_id = response['id']
        super().__init__(destination=response['destination'])
