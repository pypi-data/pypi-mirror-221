...
from moto import mock_glue, mock_s3, mock_ssm, mock_sts
from mypy_boto3_s3 import S3Client, S3ServiceResource
from mypy_boto3_ssm import SSMClient
from mypy_boto3_ecr import ECRClient
import snowcross.aws as aws
from tests import (
    TEST_FILE_PATH,
    TEST_FILE_NAME,
    TEST_BUCKET_NAME,
...
def test_environment_storage_bucket(fixture_sts_client):
    ...
    assert (
        aws.environment_storage_bucket(environment=environment, name=TEST_BUCKET_NAME)
...
def test_resolve_parameters(fixture_ssm_client: SSMClient):
    ...
    res = aws.resolve_parameters(parameters_to_resolve)
...
def test_resolve_parameters_non_ssm(fixture_ssm_client: SSMClient):
    ...
    res = aws.resolve_parameters(non_ssm_parameters)
...
def test_resolve_parameters_empty(fixture_ssm_client: SSMClient):
    empty_dict = {}
    res = aws.resolve_parameters(empty_dict)
...
def test_resolve_parameters_invalid_param(fixture_ssm_client: SSMClient, caplog):
    ...
    res = aws.resolve_parameters(parameters)
...
def test_get_storage_object(fixture_file):
    res = json.load(aws.get_storage_object(bucket=TEST_BUCKET_NAME, key=TEST_FILE_NAME))
...
def test_get_storage_object_not_exists(fixture_file, caplog):
    ...
    aws.get_storage_object(bucket=TEST_BUCKET_NAME, key=invalid_key)
    assert f"Object {invalid_key} not found - returning None" in caplog.text
...
def test_upload_storage_object(fixture_s3_bucket, fixture_s3_client: S3Client):
    aws.upload_storage_object(TEST_FILE_PATH, TEST_BUCKET_NAME, TEST_FILE_NAME)
...
def test_download_storage_object(fixture_file, tmp_path):
    ...
    aws.download_storage_object(bucket=TEST_BUCKET_NAME, key=TEST_FILE_NAME, filename=download_path)
...
def test_list_storage_objects(fixture_s3_client: S3Client):
    ...
    assert (
        len(list(aws.list_storage_objects(TEST_BUCKET_NAME, prefix_1))) == 2
        and len(list(aws.list_storage_objects(TEST_BUCKET_NAME, prefix_2))) == 1
    )


# should parse an s3 scheme into objects.
def test_parse_storage_url_s3_scheme():
    obj = aws.parse_storage_url(f"s3://{TEST_BUCKET_NAME}/prefix/schema/table/format/file.parquet")
...
def test_parse_storage_url_s3a_scheme():
    obj = aws.parse_storage_url(f"s3a://{TEST_BUCKET_NAME}/prefix/schema/table/format/file.parquet")
...
def test_parse_storage_url_none():
    obj = aws.parse_storage_url(None)
...
def test_latest_image_version(fixture_ecr_image, fixture_ecr_client: ECRClient):
    latest_image_version = aws.latest_image_version(
