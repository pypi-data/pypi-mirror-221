...
class QualityAssessments:
    ...
    def comparison_with_previous_data(df, name_of_df, cols_to_check=['impressions','likes'], perc_increase_threshold=20,
                                    perc_decrease_threshold=0.5, check_cols_set=True, raise_exceptions=True, manual_override=False):
            ...
            The historic column totals are stored in a datatable for reference and the function will check the current totals with the most recent previous totals and 
            raise an exception if the totals have changed by more than the specified thresholds.
            ...
            add a new row to the historic db with the new values. This manual override can only be done once in a row to stop someone forgetting they put manual override on and leaving it running in the script
            which would mean the function would never pick up any errors.

            Args:
                df (pd.DataFrame): The dataframe to be checked
                name_of_df (str): The name of the dataframe to be used for logging purposes
                cols_to_check (Optional[list], optional): The columns to check for changes in totals. Defaults to ['impressions','likes'].
                perc_increase_threshold (int, optional): The percentage increase threshold for the totals. Defaults to 20.
                perc_decrease_threshold (float, optional): The percentage decrease threshold for the totals. Defaults to 0.5.
                check_cols_set (bool, optional): If True, checks if the columns in the dataframe have changed from the previous time. Defaults to True.
                raise_exceptions (bool, optional): If True, raises an exception if the totals have changed by more than the specified thresholds. Defaults to True.
                manual_override (bool, optional): If True, adds a new row to the historic db with the different but now deemed correct new values. Defaults to False.
            
            Raises:
                Exception: If raise_exceptions = True and the totals have changed by more than the specified thresholds.
            
            Returns:
                error_message (str): String error message describing which columns have changed or which totals have changed by more than the specified thresholds. This message can then be sent
                                    to a slack function
            """
            ...
            for key, value in old_dict.items():
                ...
                elif key == 'Date' or key == 'manual_override':
                    continue
            ...
            if not manual_override:
                if error_occured:
                    ...
                    logger.logger.info('ERROR' + error_message) # If error messages has been added to then log it
                ...
                if old_dict['manual_override'] == False:
                    logger.logger.info('Manual Override set to True, adding new row to historic db with different values now deemed to be correct') 
