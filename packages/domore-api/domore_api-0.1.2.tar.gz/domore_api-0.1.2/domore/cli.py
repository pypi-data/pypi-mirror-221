...
def init_parser_run(subparsers):
    sub_parser_run = subparsers.add_parser(
        "run", help="Make DoMore testcases and run with pytest."
    )
...
def main_dorun_alias():
    ...
    if len(sys.argv) == 2:
        if sys.argv[1] in ["-V", "--version"]:
            ...
            if not pytest_case_file.endswith(".py"):
            ...
            sys.argv.insert(2, "-s")
            sys.argv.insert(3, "-v")
            sys.argv.append("--capture=tee-sys")
...
def main_har2case_alias():
    ...
    sys.argv.insert(3, ".js|.css|.png|.jpg|.jpeg|.ico")
    main()
