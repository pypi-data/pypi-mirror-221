""" Convert HAR (HTTP Archive) to YAML/JSON testcase for DoMore.
...
from domore.ext.har2case.core import HarParser
from domore.utils import ga_client
...
def init_har2case_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "har2case",
        help="Convert HAR(HTTP Archive) to YAML/JSON testcases for DoMore.",
    )
...
def init_har2pytest_parser(subparsers):
    """
    HAR converter: parse command line options and run commands.
    """
    parser = subparsers.add_parser(
        "har2pytest",
        help="Convert HAR(HTTP Archive) to pytest testcases for DoMore.",
    )
    parser.add_argument("har_source_file", nargs="?", help="Specify HAR source file")
    parser.add_argument(
        "--filter",
...
def main_har2case_return_case_file(args):
    har_source_file = args.har_source_file

    output_file_type = "pytest"

    ga_client.track_event("ConvertTests", f"har2case {output_file_type}")
    return HarParser(har_source_file, args.filter, args.exclude, args.profile).gen_testcase(output_file_type)
