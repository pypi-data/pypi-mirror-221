...
class ProjectBase(ProjectString):
    ...
    def write_api(self, row, _update=True):
        fun_name = self.get_fun_name(row.path)
    ...
    def write_testcase(self, fun_name, _update=False):
        ...
        if os.path.exists(fun_path.path):
            ...
            ord_str_list = re.findall(r'\n((?:    # *@pytest.mark.skip\(.*?\) *\n)?'
                                      ...
                                      r'    def test_(%s(?:__(?:.+?))?)'
                                      r'(?:_(\d+))?\(.+\):[\w\W]+?\n+)'
                                      r'(?:\n    @allure.step\(|\n    def|$)?' % fun_name, file_str)
            if ord_str_list:
                ord_list = {i[1]: i[0] for i in ord_str_list}
                ord_num = {j[1]: (int(j[2]) if j[2] else i + 1) for i, j in enumerate(ord_str_list)}
                i = len(ord_str_list)
                    ...
                    if case_name in ord_list.keys():
                        if _update:
                            old_str = re.findall(r'\n((?:    # *@pytest.mark.skip\(.*?\) *\n)?'
                                                 r'(?:    @allure.step\(.*?\) *\n)?'
                                                 r'    def test_%s(_(\d+))?\(.+\):[\w\W]+?\n+)'
                                                 r'(?:\n    @allure.step\(|\n    def|$)?' % case_name, file_str)
                            fun_str = fun_str.replace(f"def test_{case_name}(",
                                                      f"def test_{case_name}{'_%03d' % ord_num.get(case_name)}(")
