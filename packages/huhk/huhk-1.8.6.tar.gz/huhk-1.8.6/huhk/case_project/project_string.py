    ...
    def get_api_testcase_str(self, fun_name):
        ...
        if data_list:
            if set(api.input) & set(self.page_and_size) or str(api.method).lower() == "get":
                ...
                for n in data_list:
                    name, fun_str = get_str(f"入参 {n} 为空", n, f"{n}=None")
                    out[name] = fun_str
                if len(data_list) > 1:
                    name, fun_str = get_str(f"所有入参都为空", "null", f"_all_is_None=True")
                    out[name] = fun_str
        return out
