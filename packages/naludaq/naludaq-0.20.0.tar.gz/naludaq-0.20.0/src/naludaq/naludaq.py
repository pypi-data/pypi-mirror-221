...
class NaluDaq:
    ...
    def change_trigger_value(self, channel, value):
        ...
        """
        self.board.trigger_values[channel] = value

    def update_triggers(self):
        """Send all the trigger settings to the board."""
        get_trigger_controller(self.board).write_triggers()
    ...
    def stop_acquisition(self):
        ...
        if self.board.using_new_backend:
            if self.board.server_connected:
...
class UpacDaq(NaluDaq):
    ...
    def capture_event(self):
        ...
        """
        new_buffer_size = len(self.daq.output_buffer) + 1
        self.is_capturing = True
