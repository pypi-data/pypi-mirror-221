...
class GrowInfo():
    ...
    def get_grow_info_data(self, arg_grow_info_dict):
        try: 
            self.total_level = arg_grow_info_dict['total']['level']
            for i in range(4):
...
class Equipment():
    ...
    def get_equipment_data(self, arg_equipment_dict):
        try:
            self.item_name = arg_equipment_dict['itemName'] # 이름
        ...
        try:
            self.grow_info.get_grow_info_data(arg_equipment_dict["growInfo"]) # 105제 성장 장비 정보
        except:
            pass
