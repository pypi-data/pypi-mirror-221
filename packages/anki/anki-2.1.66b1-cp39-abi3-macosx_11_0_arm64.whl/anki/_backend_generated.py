...
class RustBackendGenerated:
 ...
 if the backup encountered an error."""
    ...
    def set_deck_collapsed_raw(self, message: bytes) -> bytes:
        return self._run_command(5, 11, message)
    ...
    def set_deck_collapsed(self, *, deck_id: int, collapsed: bool, scope: anki.decks_pb2.SetDeckCollapsedRequest.Scope.V) -> anki.collection_pb2.OpChanges:
        ...
        raw_bytes = self._run_command(5, 11, message.SerializeToString())
        output = anki.collection_pb2.OpChanges()
        output.ParseFromString(raw_bytes)
        return output

    def get_deck_legacy_raw(self, message: bytes) -> bytes:
        return self._run_command(5, 12, message)

    def get_deck_legacy(self, did: int) -> bytes:
        message = anki.decks_pb2.DeckId(did=did)
        raw_bytes = self._run_command(5, 12, message.SerializeToString())
        output = anki.generic_pb2.Json()
        output.ParseFromString(raw_bytes)
        return output.json

    def get_deck_names_raw(self, message: bytes) -> bytes:
        return self._run_command(5, 13, message)
    ...
    def reparent_decks(self, *, deck_ids: Sequence[int], new_parent: int) -> anki.collection_pb2.OpChangesWithCount:
        ...
        raw_bytes = self._run_command(5, 17, message.SerializeToString())
        output = anki.collection_pb2.OpChangesWithCount()
        output.ParseFromString(raw_bytes)
        return output

    def rename_deck_raw(self, message: bytes) -> bytes:
        return self._run_command(5, 18, message)
    ...
    def bury_or_suspend_cards_raw(self, message: bytes) -> bytes:
        return self._run_command(9, 11, message)
    ...
    def set_due_date(self, *, card_ids: Sequence[int], days: str, config_key: anki.config_pb2.OptionalStringConfigKey) -> anki.collection_pb2.OpChanges:
        ...
        raw_bytes = self._run_command(9, 16, message.SerializeToString())
        output = anki.collection_pb2.OpChanges()
        output.ParseFromString(raw_bytes)
        return output

    def sort_cards_raw(self, message: bytes) -> bytes:
        return self._run_command(9, 17, message)
    ...
    def sort_cards(self, *, card_ids: Sequence[int], starting_from: int, step_size: int, randomize: bool, shift_existing: bool) -> anki.collection_pb2.OpChangesWithCount:
        ...
        raw_bytes = self._run_command(9, 17, message.SerializeToString())
        output = anki.collection_pb2.OpChangesWithCount()
        output.ParseFromString(raw_bytes)
        return output

    def sort_deck_raw(self, message: bytes) -> bytes:
        return self._run_command(9, 18, message)
    ...
    def sort_deck(self, *, deck_id: int, randomize: bool) -> anki.collection_pb2.OpChangesWithCount:
        ...
        raw_bytes = self._run_command(9, 18, message.SerializeToString())
        output = anki.collection_pb2.OpChangesWithCount()
        output.ParseFromString(raw_bytes)
        return output

    def get_scheduling_states_raw(self, message: bytes) -> bytes:
        return self._run_command(9, 19, message)

    def get_scheduling_states(self, cid: int) -> anki.scheduler_pb2.SchedulingStates:
        message = anki.cards_pb2.CardId(cid=cid)
        raw_bytes = self._run_command(9, 19, message.SerializeToString())
 ...
 from the reviewer. The backend method will throw an error."""
        return self._run_command(9, 20, message)

    def get_scheduling_states_with_context(self) -> anki.scheduler_pb2.SchedulingStatesWithContext:
        """This should be implemented by the frontend, and should return the values
 from the reviewer. The backend method will throw an error."""
        message = anki.generic_pb2.Empty()
        raw_bytes = self._run_command(9, 20, message.SerializeToString())
        output = anki.scheduler_pb2.SchedulingStatesWithContext()
        output.ParseFromString(raw_bytes)
        return output
 ...
 data in the reviewer. The backend method will throw an error."""
    ...
    def add_or_update_notetype(self, *, json: bytes, preserve_usn_and_mtime: bool, skip_checks: bool) -> int:
        ...
        raw_bytes = self._run_command(15, 4, message.SerializeToString())
        output = anki.notetypes_pb2.NotetypeId()
        output.ParseFromString(raw_bytes)
        return output.ntid

    def get_stock_notetype_legacy_raw(self, message: bytes) -> bytes:
        return self._run_command(15, 5, message)
    ...
    def get_aux_template_config_key(self, *, notetype_id: int, card_ordinal: int, key: str) -> str:
        ...
        raw_bytes = self._run_command(15, 13, message.SerializeToString())
        output = anki.generic_pb2.String()
        output.ParseFromString(raw_bytes)
        return output.val

    def get_change_notetype_info_raw(self, message: bytes) -> bytes:
        return self._run_command(15, 14, message)
    ...
    def get_change_notetype_info(self, *, old_notetype_id: int, new_notetype_id: int) -> anki.notetypes_pb2.ChangeNotetypeInfo:
        ...
        raw_bytes = self._run_command(15, 14, message.SerializeToString())
        output = anki.notetypes_pb2.ChangeNotetypeInfo()
        output.ParseFromString(raw_bytes)
        return output

    def change_notetype_raw(self, message: bytes) -> bytes:
        return self._run_command(15, 15, message)
    ...
    def change_notetype(self, *, note_ids: Sequence[int], new_fields: Sequence[int], new_templates: Sequence[int], old_notetype_id: int, new_notetype_id: int, current_schema: int, old_notetype_name: str) -> anki.collection_pb2.OpChanges:
        ...
        raw_bytes = self._run_command(15, 15, message.SerializeToString())
        output = anki.collection_pb2.OpChanges()
        output.ParseFromString(raw_bytes)
        return output

    def get_field_names_raw(self, message: bytes) -> bytes:
        return self._run_command(15, 16, message)

    def get_field_names(self, ntid: int) -> Sequence[str]:
        message = anki.notetypes_pb2.NotetypeId(ntid=ntid)
        raw_bytes = self._run_command(15, 16, message.SerializeToString())
        output = anki.generic_pb2.StringList()
        output.ParseFromString(raw_bytes)
        return output.vals

    def restore_notetype_to_stock_raw(self, message: bytes) -> bytes:
        return self._run_command(15, 17, message)
    ...
    def restore_notetype_to_stock(self, message: anki.notetypes_pb2.RestoreNotetypeToStockRequest) -> anki.collection_pb2.OpChanges:
        ...
        raw_bytes = self._run_command(15, 17, message.SerializeToString())
        output = anki.collection_pb2.OpChanges()
        output.ParseFromString(raw_bytes)
        return output
    ...
    def defaults_for_adding_raw(self, message: bytes) -> bytes:
        return self._run_command(17, 2, message)
    ...
    def defaults_for_adding(self, home_deck_of_current_review_card: int) -> anki.notes_pb2.DeckAndNotetype:
        ...
        raw_bytes = self._run_command(17, 2, message.SerializeToString())
        output = anki.notes_pb2.DeckAndNotetype()
        output.ParseFromString(raw_bytes)
        return output

    def default_deck_for_notetype_raw(self, message: bytes) -> bytes:
        return self._run_command(17, 3, message)

    def default_deck_for_notetype(self, ntid: int) -> int:
        message = anki.notetypes_pb2.NotetypeId(ntid=ntid)
        raw_bytes = self._run_command(17, 3, message.SerializeToString())
        output = anki.decks_pb2.DeckId()
        output.ParseFromString(raw_bytes)
        return output.did

    def update_notes_raw(self, message: bytes) -> bytes:
        return self._run_command(17, 4, message)
    ...
    def update_notes(self, *, notes: Sequence[anki.notes_pb2.Note], skip_undo_entry: bool) -> anki.collection_pb2.OpChanges:
        ...
        raw_bytes = self._run_command(17, 4, message.SerializeToString())
        output = anki.collection_pb2.OpChanges()
        output.ParseFromString(raw_bytes)
        return output

    def get_note_raw(self, message: bytes) -> bytes:
        return self._run_command(17, 5, message)

    def get_note(self, nid: int) -> anki.notes_pb2.Note:
        message = anki.notes_pb2.NoteId(nid=nid)
        raw_bytes = self._run_command(17, 5, message.SerializeToString())
        output = anki.notes_pb2.Note()
        output.ParseFromString(raw_bytes)
        return output

    def remove_notes_raw(self, message: bytes) -> bytes:
        return self._run_command(17, 6, message)
    ...
    def update_deck_configs(self, *, target_deck_id: int, configs: Sequence[anki.deck_config_pb2.DeckConfig], removed_config_ids: Sequence[int], apply_to_children: bool, card_state_customizer: str, limits: anki.deck_config_pb2.DeckConfigsForUpdate.CurrentDeck.Limits, new_cards_ignore_review_limit: bool) -> anki.collection_pb2.OpChanges:
        message = anki.deck_config_pb2.UpdateDeckConfigsRequest(target_deck_id=target_deck_id, configs=configs, removed_config_ids=removed_config_ids, apply_to_children=apply_to_children, card_state_customizer=card_state_customizer, limits=limits, new_cards_ignore_review_limit=new_cards_ignore_review_limit)
    ...
    def add_image_occlusion_note(self, *, image_path: str, occlusions: str, header: str, back_extra: str, tags: Sequence[str], notetype_id: int) -> anki.collection_pb2.OpChanges:
    ...
    def join_search_nodes_raw(self, message: bytes) -> bytes:
        return self._run_command(31, 3, message)
    ...
    def join_search_nodes(self, *, joiner: anki.search_pb2.SearchNode.Group.Joiner.V, existing_node: anki.search_pb2.SearchNode, additional_node: anki.search_pb2.SearchNode) -> str:
        ...
        raw_bytes = self._run_command(31, 3, message.SerializeToString())
        output = anki.generic_pb2.String()
        output.ParseFromString(raw_bytes)
        return output.val

    def replace_search_node_raw(self, message: bytes) -> bytes:
        return self._run_command(31, 4, message)
    ...
    def replace_search_node(self, *, existing_node: anki.search_pb2.SearchNode, replacement_node: anki.search_pb2.SearchNode) -> str:
        ...
        raw_bytes = self._run_command(31, 4, message.SerializeToString())
        output = anki.generic_pb2.String()
        output.ParseFromString(raw_bytes)
        return output.val

    def find_and_replace_raw(self, message: bytes) -> bytes:
        return self._run_command(31, 5, message)
    ...
    def set_active_browser_columns(self, vals: Sequence[str]) -> anki.generic_pb2.Empty:
        ...
        raw_bytes = self._run_command(31, 8, message.SerializeToString())
        output = anki.generic_pb2.Empty()
        output.ParseFromString(raw_bytes)
        return output
