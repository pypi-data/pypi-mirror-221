...
def get_code_path_minio():
    ...
    if code_path is None:
        raise ValueError(
            "Failed to get the environment variable, please ensure that the CODE_PATH environment variable has been set."
        )
    return code_path
...
def get_data_path_minio():
    ...
    if data_path is None:
        raise ValueError(
            "Failed to get the environment variable, please ensure that the DATA_PATH environment variable has been set."
        )
    return data_path
...
def get_pretrain_model_path_minio():
    ...
    if pretrain_model_path is None:
        raise ValueError(
            "Failed to get the environment variable, please make sure the PRETRAIN_MODEL_PATH environment variable has been set."
        )
