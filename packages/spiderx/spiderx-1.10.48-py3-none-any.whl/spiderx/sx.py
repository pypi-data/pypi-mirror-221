...
def CMD_SUBPROCESS_CALL(cmd:Union[str,list], 类型=1,timeout=None)->int:
    '''返回0 运行成功 1没有这个命令 '''
    if 类型 == 1:
        # 不允许创建窗口
        return subprocess.call(cmd, creationflags=subprocess.CREATE_NO_WINDOW,timeout=timeout)
    elif 类型 == 2:
        # 隐藏窗口   有输出
        si = subprocess.STARTUPINFO()
        si.dwFlags = subprocess.STARTF_USESHOWWINDOW
        return subprocess.call(cmd, startupinfo=si,timeout=timeout)
    elif 类型 == 3:
        # 不允许创建窗口 隐藏窗口  无输出
        si = subprocess.STARTUPINFO()
        si.dwFlags = subprocess.STARTF_USESHOWWINDOW
        return subprocess.call(cmd,startupinfo=si,creationflags=subprocess.CREATE_NO_WINDOW,timeout=timeout)
    else:
        # 不允许子窗口
        return subprocess.call(cmd, creationflags=subprocess.DETACHED_PROCESS)
def 设置_临时环境变量(k, v):
    os.environ[k] = v
...
def 解析二维码链接_pyzbar(文件路径)->bytes:
    ...
    if 文件路径 and os.path.exists(文件路径):
        ...
        if barcodes:
            ...
            return ''
def 解析二维码链接_wechat(文件路径)->list:
    # pip install opencv-python==4.5.2.54 opencv-contrib-python==4.5.2.54
    from cv2 import wechat_qrcode_WeChatQRCode,imread
    ...
    for i in range(try_num):
        if CMD_SYSTEM(cmd)==0:
            ...
            if not os.path.exists(文件路径+'/') and os.path.exists(save_path):
                return True
    return False
def 获取_网络图片(图片网址: str, headers=None,params=None, proxies=None, cookies=None, verify=False,allow_redirects=True, pil=True, show=False, try_num=3):
    '''获取网络图片("http://...")'''
    proxies = proxies if proxies else default_proxies
    if headers and isinstance(headers,str):headers=dict_From_HeadersStr(headers)
    if cookies and isinstance(cookies,str):cookies=dict_From_CookieStr(cookies)
