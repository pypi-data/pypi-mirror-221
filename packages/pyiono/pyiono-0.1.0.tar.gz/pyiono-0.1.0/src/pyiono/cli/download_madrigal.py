...
### this code was adaped from globalDownload.py to meet our pressing needs for netCDF4 format.
...
$Id: globalDownload.py 6936 2019-07-10 15:52:45Z brideout $
"""
...
def download_madrigal(session, download_path = ''):
    
    # create a dictionary for the months
    m2n = {m:i+1 for i, m in enumerate(['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'])}

    # since they provide two files for the specificed date and for the day before, and 
    # you need the files for vgos session which may cover two days,
    # save only the date for the following date of vgos session
    date1 = datetime.date(2000+int(session[:2]),m2n[session[2:5]],int(session[5:7])) 
    date2 = date1 + datetime.timedelta(days=1)

    fileList = ['gps'+date1.strftime('%y')+date1.strftime('%m')+date1.strftime('%d')+';', 
                'gps'+date2.strftime('%y')+date2.strftime('%m')+date2.strftime('%d')+';']
        
    fileList = ''.join(sorted(set(fileList)))[:-1]
    date1 = fileList[5:7]+'/'+fileList[7:9]+'/'+str(2000+int(fileList[3:5]))
    date2 = fileList[-4:-2]+'/'+fileList[-2:]+'/'+str(2000+int(fileList[-6:-4]))
    ...
    if download_path:
        ...
        dir = os.getcwd()
    if not os.path.exists(dir + 'Data/Madrigal/'):
        os.makedirs(dir + 'Data/Madrigal/')
    outputDir = dir + 'Data/Madrigal/'
    
    # set the arguments
    url = 'http://cedar.openmadrigal.org' # http://madrigal.haystack.mit.edu
    ...
    for i in range(numFiles):
        ...
        basename = basename[:-5] + '.nc'
            
        saveName = os.path.join(saveDir, basename)

        time.sleep(0.5)
        #if not os.path.isfile(saveName) and basename[:-8] in fileNames:
        if basename[:-8] in fileNames:
            try:     
