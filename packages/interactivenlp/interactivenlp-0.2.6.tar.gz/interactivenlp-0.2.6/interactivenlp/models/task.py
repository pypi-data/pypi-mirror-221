...
def get_interactive_function(task: InteractiveTaskType) -> InteractiveFunctionType:
    ...
    if task.task_type == "highlight":
        def highlight_func(rss: InteractiveRssType, form: Optional[dict] = None) -> InteractiveFunctionResultType:
            paragraphs = rss.paragraphs
            ...
            return InteractiveFunctionResultType(processed_paragraphs, None)

        res_func = highlight_func

    elif task.task_type == "generation":
        def generation_func(rss: InteractiveRssType, form: Optional[dict] = None) -> InteractiveFunctionResultType:
            paragraphs = list(map(lambda p: Block(
                BlockLevelEnum.Paragraph, BlockTypeEnum.Normal, p.content, None), rss.paragraphs))
            total_text = '\n'.join(map(lambda p: p.content, rss.paragraphs))
            ...
            return InteractiveFunctionResultType(paragraphs, global_block)

        res_func = generation_func

    elif task.task_type == "insert":
        def insert_func(rss: InteractiveRssType, form: Optional[dict] = None) -> InteractiveFunctionResultType:
            old_paragraphs = rss.paragraphs
            new_paragraphs = []
                    ...
                    if insert_res["position"] == index:
                        new_paragraphs.append(
                            Block(BlockLevelEnum.Paragraph, BlockTypeEnum.Quote, insert_res["content"], None,
                                  BlockStatusEnum.New))
            return InteractiveFunctionResultType(new_paragraphs, None)
