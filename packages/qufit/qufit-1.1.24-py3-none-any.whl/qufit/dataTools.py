    ...
    for i,qi in enumerate(range(dim)):
        ...
        if qi not in target:
            ...
            #     axes[0].set_title(f'center={center0,center1}')
                ...
                offstd, onstd = stdnum[count]*np.abs(para0.x[2]), stdnum[count]*np.abs(para1.x[2])
                ...
                cLst_m = [(center[i],stdnum[count]*np.std(s_st[i,:,qi])) for i in range(n_clusters)]
                num0Lst, num1Lst = p[:2]
                ...
                for n_state in range(n_clusters):
                    s = s_st[n_state,:,qi]
                    theta = np.arange(0, 2*np.pi, 0.01)
                    roff = np.real(center[n_state]) + stdnum[count]*np.std(s) * np.cos(theta)
                    ioff = np.imag(center[n_state]) + stdnum[count]*np.std(s) * np.sin(theta)
