...
''')
        if change.is_change() or (lcode != rcode): # generated from line 165, col 1
            _v = VFFSL(SL,"details_changed",True) # '$details_changed' on line 166, col 1
            if _v is not None: write(_filter(_v, rawExpr='$details_changed'))
            write('''
''')
        else: # generated from line 167, col 1
            _v = VFFSL(SL,"details_unchanged",True) # '$details_unchanged' on line 168, col 1
            if _v is not None: write(_filter(_v, rawExpr='$details_unchanged'))
            write('''
''')
        write('''</div>
</div>
