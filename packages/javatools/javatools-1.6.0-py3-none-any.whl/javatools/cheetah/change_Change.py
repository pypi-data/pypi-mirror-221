...
''')
        _v = VFFSL(SL,"escape",False)(VFFSL(SL,"change.label",True)) # '$escape($change.label)' on line 26, col 1
        if _v is not None: write(_filter(_v, rawExpr='$escape($change.label)'))
        write('''
</div>
''')
        write('''
...
''')
            for child in VFN(VFFSL(SL,"change",True),"collect",False)(): # generated from line 38, col 1
                _v = VFFSL(SL,"render_change",False)(child) # '$render_change(child)' on line 39, col 1
