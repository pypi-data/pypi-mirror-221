...
class AsyncDatabase(AsyncDatabaseNoCache):
    """Async interface for Repl.it Database, with a dict as a memcache."""
    ...
    async def get_raw(self, key: str) -> str:
        """Get the value of an item from the database.
        This method is marked `async` despite not being asynchronous for compatibility with the regular `replit` package.
        Args:
            key (str): The key to retreive
        Raises:
            KeyError: Key is not set
    ...
    async def list(self, prefix: str) -> Tuple[str, ...]:
        ...
        Returns:
            Tuple[str]: The keys found.
        """
        tuple(filter(lambda key: key.startswith(prefix), self.cache.keys()))
...
class DatabaseNoCache(abc.MutableMapping):
...
class Database(DatabaseNoCache):
    """Dictionary-like interface for Repl.it Database, with a memcache.
    This interface will coerce all values everything to and from JSON. If you
    don't want this, use AsyncDatabase instead.
    """
