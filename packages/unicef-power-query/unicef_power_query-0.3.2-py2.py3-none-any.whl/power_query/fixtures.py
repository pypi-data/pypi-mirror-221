...
from power_query.models import (
    Dataset,
    Formatter,
    Parametrizer,
    Query,
...
class UserFactory(DjangoModelFactory):
    ...
    email = factory.LazyAttribute(
        lambda o: f"{o.first_name.lower()}.{o.last_name.lower()}_{time.time_ns()}@unicef.com"
    )
    username = factory.LazyAttribute(
        lambda o: f"{o.first_name}{o.last_name}_{time.time_ns()}"
    )
...
class QueryFactory(DjangoModelFactory):
    ...
    target = factory.Iterator(
        ContentType.objects.filter(app_label="auth", model="permission")
    )
...
class ReportFactory(DjangoModelFactory):
    ...
    owner = factory.SubFactory(
        UserFactory, is_superuser=True, is_staff=True, password="123"
    )
