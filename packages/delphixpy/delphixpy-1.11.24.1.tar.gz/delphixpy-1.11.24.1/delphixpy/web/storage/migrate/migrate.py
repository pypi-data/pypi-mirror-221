...
def get(engine):
    """
    Retrieve the specified DoseMigrationStatus object.

    :param engine: The Delphix Engine
    :type engine: :py:class:`delphixpy.delphix_engine.DelphixEngine`
    :rtype: :py:class:`delphixpy.web.vo.DoseMigrationStatus`
    """
    url = "/resources/json/delphix/storage/migrate"
    response = engine.get(url)
    result = response_validator.validate(response, engine)
    raw_result = getattr(engine, 'raw_result', False)
    return response_validator.parse_result(result, undef_enabled=True, return_types=['DoseMigrationStatus'], returns_list=False, raw_result=raw_result)
