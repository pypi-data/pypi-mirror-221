...
def predict(input_data, 
    ...
    if predict_movie:
        ...
        nib.save(nib.Nifti1Image(movie_prediction,
                 ...
                 out_filename)
        print(f'Saved {Path(input_data)} as {out_filename}')
...
def _CLI_interface():
    ...
    >>> python img2fmri.py [-h] --input input_dir_or_movie [--output output_dir]
                                [--rois each roi here] [--sigma sigma_val] [--center_crop 0_or_1]
...
def _get_args():
    ...
    parser.add_argument('--predict_movie', metavar='predict_movie', type=int, default=0,
