...
class RelationSwitchFacet(facetbase.VocabularyFacet):
    ...
    * `rqlpaths`, a list of 3-tuples (label, rqlpath, targetvar)
    ...
    needs_update = False  # explain to standard facet mechanism that it should not try to update this facet itself.
    ...
    def get_widget(self):
        ...
        selected = self.get_selected()  # noqa: F841
    ...
    def vocabulary(self):
        values = []
        mainvar = self.filtered_variable  # noqa: F841
    ...
    def possible_switches(self):
        ...
        switches = set()
        args = self.cw_rset.args if self.cw_rset else None  # noqa: F841
    ...
    def rqlpath_switch_rql_queries(self):
        select = self.select
        mainvar = self.filtered_variable
        etype = select.solutions[0][mainvar.name]  # noqa: F841
        if self.rqlpaths:
