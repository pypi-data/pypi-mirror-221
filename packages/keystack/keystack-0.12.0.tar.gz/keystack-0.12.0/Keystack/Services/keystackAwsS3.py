...
Usage:
   1> python3 keystack_awsS3.py > /dev/null &
   2> Run at startup: Add in crontab: @reboot <user> python /Keystack/Services/keystack_awsS3.py
   3> Subprocess #1
"""
...
class AwsS3Upload():
    ...
    def pushToS3(self, sourceFile, bucketName):
        ...
        """
        match = re.search('(GROUP.*)', sourceFile)
        if match is None:
...
while True:
    ...
    # /opt/KeystackSystem/ServicesStagingArea/AwsS3Uploads/PLAYBOOK=Samples-pythonSample_04-13-2023-14:29:34:960688.json
    for keystackMessageToUpload in glob(f'{vars.awsS3StagingFolder}/*json'):       
        ...
        try:
            ...
            # /path/KeystackTests/Results/GROUP=Default/PLAYBOOK=<playbook>/04-13-2023-12:09:47:259727_hgee8
            ...
            awsS3ServiceObj.writeToServiceLogFile(msgType='failed', msg=f'aws login credentials: {traceback.format_exc(None,errMsg)}',
                                                  playbookName=playbookName, sessionId=sessionId, logFile=logFile, fileLock=fileLock)
            awsS3ServiceObj.writeToServiceLogFile(msgType='failed', msg=f'aws login credentials: {traceback.format_exc(None,errMsg)}',
        ...
        for eachFileOrFolderToUpload in keystackTestResultPathContent['artifactsPath']:      
            ...
            newThreadObj = AwsS3Upload(eachFileOrFolderToUpload, playbookName, sessionId, logFile,
                                       loginCredentials['accessKey'], loginCredentials['secretKey'], 
                                       loginCredentials['region'], loginCredentials['S3BucketName'],)
    ...
    if goToWork:
        for eachThread in threadList:
            if isDebugEnabled:
                ...
                regexpMatch = re.search(f'PATH=.+json_PLAYBOOK=(.+)_([0-9]+-[0-9]+-[0-9]+-[0-9]+:[0-9]+:[0-9]+:[0-9]+.+)', eachThread.name)
        ...
        while True:
            ...
            for eachThread in threadList:
                regexpMatch = re.search(f'PATH=(.+json)_PLAYBOOK=(.+)_([0-9]+-[0-9]+-[0-9]+-[0-9]+:[0-9]+:[0-9]+:[0-9]+.+)', eachThread.name)
