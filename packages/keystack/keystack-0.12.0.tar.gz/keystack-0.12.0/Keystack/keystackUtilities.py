import os, re, json, yaml, datetime, subprocess, time, platform, traceback
...
def devDebug(msg, mode='w'):
    """
    Write to /path/KeystackSystem/Logs/devDebugLogs.txt
    
    from keystackUtilities import devDebug
    devDebug(f'write debug message')
    ...
    if os.path.exists(GlobalVars.debugLogFilePath) == False:
        with open(GlobalVars.debugLogFilePath, mode='w') as fileObj:
            fileObj.write(f'{msg}\n')
            
        chownChmodFolder(GlobalVars.debugLogFilePath, GlobalVars.user, GlobalVars.userGroup)
     
    with open(GlobalVars.debugLogFilePath, mode='a+') as fileObj:
        fileObj.write(f'{msg}\n\n')
...
def execSubprocess(args, cwd=None, stdout=True):
    """
    ...
    if stdout:
        print(f'utilities.execSubprocess: {" ".join(args)}')
        for line in result.decode('utf-8').split('\n'):
            if line:
                if type(line) is bytes:
                    line = line.decode('utf-8')
                
                print('utilities.execSubprocess: ', line)
...
def execSubprocess2(command, shell=True, cwd=None, showStdout=True):
    """
...
def execSubprocessInShellMode(command, cwd=None, showStdout=True):
    """
    ...
    if showStdout:
        for line in result.decode('utf-8').split('\n'):
            if line:
                ...
                print('utilities.execSubprocessInShellMode -> ', line)
    ...
    if isFileExists(yamlFile, raiseException=True):
        ...
        while True:
            try:
                ...
                if counter == retry:
                    print(f'utilities: readYaml failed: {errMsg}')
                    raise(errMsg)
                else:
                    counter += 1
                    time.sleep(1)
    ...
    while True:
        try: 
            with open(yamlFile, mode) as fileObj:           
                yaml.dump(contents, fileObj, default_flow_style=False)
...
def writeToFile(filename, msg, mode='a+', threadLock=None, printToStdout=True):
    """
    Log message to file.
...
def makeFolder(targetPath, permission=0o774, stdout=True):
    ...
    try: 
        ...
        raise Exception(f'utilities.py.makeFolder(): Failed: {targetPath} {permission}')
    
    if stdout:
        print(f'utilities.py.makeFolder(): {targetPath} {permission}')
        
def mkdir(directory, user=None, permissionLevel='770', stdout=True):
    """
    Create a directory
    """
    if os.path.exists(directory) == False:
        ...
        if result == False:
            raise Exception('utilities.mkdir: Failed to create new directory: {}\n{}You need to be root user.'.format(directory,stdout))

        if permissionLevel is not None:
            result, stdout = execSubprocess(['sudo', '-u', user, 'chmod', '-R', permissionLevel, directory], stdout=stdout)
            if result == False:
                raise Exception('utilities.mkdir: Failed to chmod {} on directory: {}.\nYou need to be root user'.format(
                    permissionLevel, directory))

    return True

def mkdir2(directory, stdout=True):
    """
    Create a directory
    """
    if os.path.exists(directory) == False:
        if stdout:
            print(f'\nutilities.mkdir2: {directory}')
            
        result, stdout = execSubprocess(['mkdir', '-p', directory], stdout=stdout)
        if result == False:
            raise Exception('utilities.mkdir2: Failed to create new directory: {}\n{}'.format(directory, stdout))
...
def createNewFile(filename, permissionLevel='770'):
    ...
    try:
        ...
        raise Exception('utililities.createNewFile: Failed to create empty file: {}.'.format(filename))

    result, stdout = execSubprocess(['chmod', permissionLevel, filename])
    if result == False:
        raise Exception('utilities.createNewFile: Failed to chmod {} for file: {}.\n{}'.format(
            permissionLevel, filename, stdout))

def removeFile(pathFile):
    if os.path.isfile(pathFile):
        result, stdout = execSubprocess(['rm', pathFile])
        if result == False:
            raise Exception('utilities.mkdir: Failed to remove file: {}\n{}'.format(pathFile, stdout))

    return True

def removeFolder(path):
    result, stdout = execSubprocess(['rm', '-rf', path])
    if result == False:
        raise Exception(f'utilities.removeFolder: Failed to remove folder: {path}\n{stdout}')
...
def renameFolder(path, newPath):
    ...
    if result == False:
        raise Exception (f'utilities.renameFolder: Failed to rename folder: {path} -> {newPath}\n{stdout}')
...
def isFileExists(filename, raiseException=False):
    ...
    if os.path.exists(filename) == False:
        if raiseException == True:
            raise Exception('utilities.isFileExists: No such file: {0}'.format(filename))
...
def getDictIndexFromList(listOfDicts, key, value):
    ...
    """
    for index,element in enumerate(listOfDicts):
        for index,values in element.items():
            # index:1 values:{'env': 'jadyn', 'result': 'Failed'}
            # index:4 values:{'env': 'audra', 'result': 'Aborted'}
            for property,values2 in values.items():
...
def removePastFoldersBasedOnFolderTimestamp(folder, removeDaysOlderThan=2): 
    ...
    Example:
        timestamp folder = /path/KeystackTests/Results/PLAYBOOK=pythonSample/06-23-2022-12:06:22:953462_<sessionId>
    """     
    today = datetime.datetime.now()

    for root in glob(f'{folder}/*'):
        # /opt/KeystackTests/Results/GROUP=Default/PLAYBOOK=pythonSample/06-23-2022-12:06:22:953462_pauseOnError3
        ...
        if int(daysRecorded) >= int(removeDaysOlderThan):
            try:
                if os.path.exists(f'{root}/overallSummary.json'):
                    ...
                    # Don't remove result folders that are still actively running
                    if overallSummary['status'] == 'Running':
                        continue
...
def removePastFilesAndFolders(folder, removeDaysOlderThan=2):
    """
    Remove old files/folders based on date/time created.
    The analyzing date/time is unix time 
    """            
    today = datetime.datetime.now()

    for root, dirs, files in os.walk(folder):
        unixTimestamp = os.path.getmtime(root)
        ...
        if int(daysRecorded) >= int(removeDaysOlderThan):
            try:
                ...
                #raise(f'removePastFilesFromFolder: {logFile}\n{errMsg}')
                print(f'removePastFilesFromFolder error: {root}\n{errMsg}')
                pass
...
def removeDockerImage(version, sudo=False):
    ...
    for line in output.split('\n'):
        ...
        if match:
            dockerTag = match.group(1)
            dockerId = match.group(2)
    ...
    while True:
        ...
        if line:
            ...
            if match:
                ...
                print(f'Stopping Docker Container: {containerProcessId} {tag}')
                if sudo:
                    execSubprocess2(command=f'sudo docker container stop {containerProcessId}')
                else:
                    execSubprocess2(command=f'docker container stop {containerProcessId}')
                    
                if removeContainer:
                    # Must remove the current same Keystack Docker container and image because
                    # images could not be overwritten.
                    if sudo:
                        execSubprocess2(command=f'sudo docker container rm {containerProcessId}')
                    else:
                        execSubprocess2(command=f'docker container rm {containerProcessId}')
