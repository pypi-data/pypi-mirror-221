...
class GetModules(APIView):
    ...
    def post(self, request):
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            moduleList = glob(f'{GlobalVars.keystackTestRootPath}/Modules/*') 
            ...
            for index, eachModule in enumerate(moduleList):
                ...
                html += f'<a class="nav-link" href="/getModuleFolderFiles/{moduleName}" data-value="{moduleName}">'
                html += f'<span>{moduleName}</span>'
                html += '</a>'           
                html += '</li>'
