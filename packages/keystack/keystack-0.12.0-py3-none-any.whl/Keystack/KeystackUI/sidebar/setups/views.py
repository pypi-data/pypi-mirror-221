...
from baseLibs import getSetups, getGroupSessions
#from systemLogging import SystemLogsAssistant
#from keystackUtilities import mkdir2, writeToFile, readYaml, readJson, writeToJson, getTimestamp, chownChmodFolder, execSubprocessInShellMode
from topbar.settings.accountMgmt.verifyLogin import authenticateLogin, verifyUserRole
#from EnvMgmt import ManageEnv
# /Keystack/KeystackUI/sidebar/setups
#currentDir = os.path.abspath(os.path.dirname(__file__))
#sys.path.insert(0, currentDir.replace('/KeystackUI/sidebar/setups', ''))
#from db import DB
...
class Setups(View):
    ...
    def get(self, request):
        ...
        envGroupToView = request.GET.get('group')
        ...
        #                'selectedEnvGroupToView': envGroupToView,
    ...
    def delete(self, request):
        ...
        try:
            for env in deleteEnvs:
                ...
                envName = env.split(f'{GlobalVars.keystackTestRootPath}/Envs/')[-1].split('.')[0]
                os.remove(env)
        ...
        return JsonResponse({'errorMsg': errorMsg, 'status': status}, status=statusCode)
    '''
