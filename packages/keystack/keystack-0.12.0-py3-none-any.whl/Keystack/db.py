...
class DB:
    ...
    load_dotenv(keystackSystemSettingsFile)
    localHostIp = os.environ.get('keystack_localHostIp', 'localhost')
    dbLogFile = f"{keystackSystemPath}/Logs/mongodb.log"
    dbIp       = os.environ.get('keystack_mongoDbIp', 'localhost')
    dbIpPort   = os.environ.get('keystack_dbIpPort', 27017)
    dbName     = 'Keystack'
...
class ManageDB(object):
    ...
    def executeMongod(self, debugMode=False):
        ...
        #command += f"--bind_ip_all --port {DB.dbIpPort} "
...
class ConnectMongoDB:
    ...
    @retry           
    def getOneDocument(self, collectionName, fields):
        """
    ...
    @retry                
    def getDocuments(self, collectionName, fields, includeFields=None, sortBy=None):
        """
        Get documents based on the fields.
        ...
        Example:
            ...
            userData.count(): 0=No document found. 1=document found
        """
        collection = self.dbName[collectionName]
    ...
    def updateDocument(self, collectionName, queryFields, updateFields, removeKey=False, multi=False, appendToList=False, 
        ...
        Examples:
           ...
           # Append to list
           DB.topologies.updateDocument(queryFields={'TopologyName': topologyName},
                                        updateFields={'Devices.Domains.{}'.format(selectedDomain): device},
                                        appendToList=True)
           ...
           DB.topologies.updateDocument(queryFields={'TopologyName': topologyName},
                                        updateFields={'Devices.Domains.{}'.format(domain): device}, removeFromList=True)
        ...
        try:
            if appendToList:
                result = collection.update(queryFields, {'$push': updateFields}, multi=multi)
                
                # No result.acknowledged. Just return the result
                return result
            ...
            if appendListToList:
                ...
                result = collection.update(queryFields, {'$push': updateFields}, upsert=True, multi=True)
                if result.acknowledged:
                    return result
                else:
            ...
            if removeFromList:
                result = collection.update(queryFields, {'$pull': updateFields}, multi=multi)
                return result
            
            if removeKey:
                result = collection.update(queryFields, {'$unset': updateFields}, multi=multi)
                return result
                          
            result = collection.update(queryFields, {'$set': updateFields}, multi=multi)
            # Returns True|False
