...
from keystackUtilities import readYaml, readJson, writeToJson, getTimestamp, execSubprocessInShellMode
...
class vars:    
    ...
    jiraServiceLogFile  = f'{keystackServiceLogsFolder}/jira'
    ...
    logsServiceLogFile  = f'{keystackServiceLogsFolder}/keystackUILogs'
    

class KeystackServices:
    def __init__(self, typeOfService=None, isFromKeystackUI=False):
        """ 
        typeOfService: options: keystackAwsS3 | keystackJira | keysatckLogs
        ...
        # Ex: /opt/KeystackTests
        keystackSystemSettingsFile = f'{keystackObj["keystackSystemPath"]}/keystackSystemSettings.env'
        if os.path.exists(keystackSystemSettingsFile):
            load_dotenv(keystackSystemSettingsFile)
        else:
            raise Exception(f'Serviceware.py exception: Could not locate file: {keystackSystemSettingsFile}')   
        
        if typeOfService:
            self.setService(typeOfService)

        if isFromKeystackUI:
            vars.pythonPath = os.environ['keystack_dockerPythonPath']
        else:
            vars.pythonPath = os.environ['keystack_pythonPath']
                
    def setService(self, service):       
        if service == 'keystackAwsS3':
            # vars.awsS3ServiceLogFile = f'{keystackServiceLogsFolder}/awsS3'
    ...
    def isServiceRunning(self, service, showStdout=False):
        """
        service: keystackAwsS3, keystackLogs
        """
        if showStdout:
    ...
    def stopServices(self):
        """ 
        A function to stop multiple services
        """
