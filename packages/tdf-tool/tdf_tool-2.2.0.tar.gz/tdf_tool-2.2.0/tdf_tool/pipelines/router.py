...
class Router:
    ...
    def start(self):
        """
        tl router start：会以交互式进行路由操作，对指定的模块执行路由生成和路由注册逻辑
        """
    ...
    def module(self, name: str):
        """
        tl router module [module_name]：对指定的模块执行路由生成和路由注册逻辑
        """
        ShellDir.dirInvalidate()
        businessModuleList = self.__router_businessModuleList()
...
class _{2}State extends State<{2}> {{
    ...
    def __judgeHasBuildRunnerPackage(self, packageLibPath: str) -> bool:
        ...
        for item in packageJsonConfig.packages:
            if item.packageName == 'build_runner':
                hasBuildRunner = True
            if item.packageName == 'tdf_router_anno':
                hasRouterAnno = True
