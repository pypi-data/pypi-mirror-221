    ...
    def service(self):
        ...
        if command == 'start':
            if service.is_running():
                ...
                if len(sys.argv) > 3:
                    ext_cmd = sys.argv[3]
                    if ext_cmd in ["prod","product"]:
                        isProduct = True
                service.start(isProduct)
        elif command == 'stop':
            ...
            if service.is_running():
                runingTask = store.getCurrentTaskUUID()
            ...
            if len(receiveWidget) > 0:
                runningStr += f'\n\nrecv {env} widget is [{receiveWidget} ]'
    ...
    def widget(self):
        ...
        if command == 'init':
            ...
            if mecord_service.check_is_running():
                print('Service is already running, please shutdown with [mecord service stop], and continue command')
                return
            ...
            for it in map:
                ...
                if is_block:
                    ...
                    before_args = ""
                ss = f"{it}{end_args}"
                print(f'{before_args}{ss.ljust(maxJust + 4)} {path}\033[0m')
        elif command == 'add':
