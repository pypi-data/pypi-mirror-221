...
def _do_eigen_json_request(requesturl,**params):
    ...
    if settings._azure_tenant_id_ and settings._azure_client_id_ and settings._azure_auth_enabled_:
    ...
    try:
        if "Sign in to your account" in ret:
            if not settings._azure_auth_enabled_ or not (settings._azure_tenant_id_ and settings._azure_client_id_):
