...
from HPO.utils.model import NetworkTS
...
def _compute(hyperparameter,budget = 1, in_model = None , train_path = None,  test_path = None, cuda_device = None,plot_queue = None, model_id = None, binary = False ):
  ...
  hpo = {'channels': 64, 'lr': 0.000250170869707739693, 'p': 0.0, 'epochs': 100}
  ...
  dataset_train = Train_TEPS(augmentations = augmentations, samples_per_class = 100,device = cuda_device,one_hot = False,binary = binary,samples_per_epoch = 1)
  ...
  for fold in range(budget):
      ...
      model = NetworkTS(dataset_train.get_n_features(),hpo["channels"],num_classes= dataset_train.get_n_classes() , layers = hpo["layers"], auxiliary = False,drop_prob = hpo["p"], genotype = gen, binary = binary)
