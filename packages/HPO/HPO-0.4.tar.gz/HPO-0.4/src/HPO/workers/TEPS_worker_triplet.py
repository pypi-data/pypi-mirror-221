...
from HPO.utils.train import train_model_triplet
...
def _compute(hyperparameter,budget = 1, in_model = None , train_path = None,  test_path = None, cuda_device = None,plot_queue = None, model_id = None, binary = True):
  ...
  hpo = {'channels': 64, 'lr': 0.0250170869707739693, 'p': 0.0, 'epochs': 50, 'layers': 3}
  ...
  augmentations = [jitter,crop, window_warp,cut_out]

  dataset_train = Train_TEPS(samples_per_class = 500,device = cuda_device,one_hot = False,binary = binary,samples_per_epoch = 1, sub_set_classes = [0,3])
  dataset_val = Test_TEPS(binary = binary,samples_per_class = 200,device = cuda_device,samples_per_epoch = 1,one_hot = False)
  ...
  for fold in range(budget):
      ...
      for run in range(30):
        model = NetworkMain(dataset_train.get_n_features(),hpo["channels"],num_classes= 3, layers = hpo["layers"], auxiliary = False,drop_prob = hpo["p"], genotype = gen, binary = False)
        model = model.cuda(device = cuda_device)
        """
        ### Train the model
        """
        _ = train_model_triplet(model , hpo, trainloader, hpo["epochs"], batch_size , cuda_device, graph = plot_queue, binary = False) 
      ##with aug
...
if __name__ == "__main__":
  import csv
  hpo = {'channels': 32, 'lr': 0.0025170869707739693, 'p': 0.0, 'epochs': 100, 'layers': 3}
