...
def score():
    ...
    logging.info("Linear regression MAE: %f\t RMSE: %f", lin_reg[1], lin_reg[0])

    # decision tree rmse
    print("\nDecision tree RMSE:", tree_reg)

    logging.info("Decision tree RMSE: %f", tree_reg)
    ...
    for mean_score, params in zip(new_cvres["mean_test_score"],
                                new_cvres["params"]):
        print(np.sqrt(-mean_score), params)
    ...
    logging.info("Final model RMSE: %f", final_rmse)
