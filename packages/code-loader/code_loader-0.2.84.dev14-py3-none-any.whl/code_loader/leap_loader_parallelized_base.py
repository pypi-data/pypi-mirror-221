...
class LeapLoaderParallelizedBase(ABC):
    def __init__(self, code_path: str, code_entry_name: str,
                 n_workers: Optional[int] = 2, max_ready_results_in_queue: int = 128) -> None:
    ...
    @lru_cache()
    def start(self) -> None:
        self._inputs_waiting_to_be_process = Queue(5000)
        self._ready_processed_results = Queue(self.max_ready_results_in_queue)
