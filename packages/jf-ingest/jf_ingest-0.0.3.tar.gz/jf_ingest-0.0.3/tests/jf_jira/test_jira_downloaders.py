...
def get_connection(mocker: requests_mock.Mocker):
    ...
    mocker.register_uri('GET', 'https://test-co.atlassian.net/rest/api/2/serverInfo', text=f'{MOCK_SERVER_INFO_RESP}')
...
class TestJiraDownload(TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        with open(f"{os.path.dirname(__file__)}/fixtures/jira_fields_response.json", "r") as file:
            cls.mock_field_response = file.read()
        with requests_mock.Mocker() as m:
            m.register_uri('GET', 'https://test-co.atlassian.net/rest/api/2/field', text=f'{cls.mock_field_response}')
    ...
    def test_get_fields_once(self):
        ...
        with requests_mock.Mocker() as m:
            m.register_uri('GET', 'https://test-co.atlassian.net/rest/api/2/field', text=self.mock_field_response)
            fields = download_fields(jira_connection=self.jira_connection)
