...
class System:
    ...
    def runSim(self, xyz, timeCond, initVal, dynamicBehaviour = True):
        """
        ...
        print ("running from Nikola")
        self.compose()
        print ("Objects created")
        self.initVar(initVal)
        print ("Objects initialized")
        print ("Simulation running...", end='')
        
        # request SIMULATION from framework method
        arr_time,arr_values,arr_deriv,arr_outVal = simulate(xyz, self.Fxyz, timeCond, dynamicBehaviour)
...
def simulate(xyz, Fxyz, timeCond, dynBhvr = True):
    """
    ...
    alive = 10.0/100  # percent to monitor on console
    while t < tStop: # ends at time tStop
        ...
        if dynBhvr:
            # TODO: check if correct and complete !!!
            deriv=Fxyz(t, xyz)[1] # calculates derivatives (causes) with old values!
        # print('>> Framework.simulate: Runge Kutta')
        if dynBhvr:
            xyz = run_kut4(xyz, Fxyz, t, h) # new values calculated with Runge-Kutta "integration"
