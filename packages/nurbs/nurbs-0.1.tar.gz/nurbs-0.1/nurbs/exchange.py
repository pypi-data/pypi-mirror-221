...
from . import compatibility, elements, linalg, operations
...
def export_cfg(obj, file_name):
    ...
    Libconfig format is also used by the `nurbs command-line application <https://github.com/orbingol/nurbs-cli>`_
...
def export_yaml(obj, file_name):
    ...
    YAML format is also used by the `nurbs command-line application <https://github.com/orbingol/nurbs-cli>`_
    as a way to input shape data from the command line.
...
def export_json(obj, file_name):
    ...
    JSON format is also used by the `nurbs command-line application <https://github.com/orbingol/nurbs-cli>`_
    as a way to input shape data from the command line.
...
def import_3dm(file_name, **kwargs):
    ...
    .. deprecated:: 5.2.2
        ...
        ``rw3dm`` Python module is replaced by ``on2json``. It can be used to convert .3dm files to nurbs JSON format.
...
def export_3dm(obj, file_name, **kwargs):
    ...
    .. deprecated:: 5.2.2
        ...
        ``rw3dm`` Python module is replaced by ``json2on``. It can be used to convert nurbs JSON format to .3dm files.
        Please refer to https://github.com/orbingol/rw3dm for more details.
