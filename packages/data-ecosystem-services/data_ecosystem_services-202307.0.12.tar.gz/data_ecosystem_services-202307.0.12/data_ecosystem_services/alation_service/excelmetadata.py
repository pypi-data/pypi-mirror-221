...
class ExcelMetadata:
    ...
    @classmethod
    def generate_excel_file_data(cls, alation_schema_id, config, schema_file_path):
        """
        ...
        with tracer.start_as_current_span("generate_excel_file_data"):
            ...
            try:
                ...
                df_table_list = table.get_tables_for_schema_for_excel(
                    config, alation_datasource_id, alation_schema_id)

                return df_schema, df_table_list, manifest_excel_file
    ...
    @classmethod
    def generate_excel_file_from_data(cls, df_schema, df_tables_list, manifest_excel_file, df_status, df_steward, df_access_level, df_language, df_update_frequency):
        """
        ...
        with tracer.start_as_current_span("generate_excel_file_from_data"):
            ...
            # Define the text format with specified font, size, color and bottom border
            ...
            # Write 'Table Updates' in cell D2
            ws_table_list.write('D2', 'Table Updates', header_text_format)
            ...
            for i in range(len(max_length) - (header_col_init - 2)):
                ws_table_list.write(1, i + header_col_init,
                                    ' ', header_text_format)

            # Write Instruction
            ws_table_list.write(
                'D3', 'Instructions: Items in grey are Read-Only.  Update the fields in blue.')
            ws_table_list.write(
                'D4', 'Dropdowns should contain picklist validation.')
            ...
            # Hide column B
            ws_table_list.set_column('B:B', None, None, {'hidden': True})

            # Hide column C
            ws_table_list.set_column('C:C', None, None, {'hidden': True})
