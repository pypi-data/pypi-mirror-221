...
class DDHSession():
	def __init__(self, name, app, token, resource, api_host, verify=None, refresh='auto'):
		self.token = token
		self.name = name
		self.verify = verify
	...
	def __repr__(self):
		return 'DDH2Session(resource="{}", api_host="{}", user_name="{}", expires="{}")'.format(self.resource,
											  								 self.api_host,
...
def create_session(name='default_ddh_session', params=None, autoCopy=True, verify=None):
	...
	if "access_token" in token:
		return DDHSession(name, app, token, resource, api_host, verify)
	else:
