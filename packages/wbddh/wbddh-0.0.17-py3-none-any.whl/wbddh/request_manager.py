...
def get(endpoint, params=None, headers=None, session=None):
    ...
    if session:
        ...
        return requests.get(get_request_url(endpoint, session) + special_query_string, params=regular_params, verify=session.verify, headers=session.get_headers(headers))
    else:
...
def try_get(endpoint, params=None, headers=None, session=None, num_try=3, interval=300):
    ...
    while True:
        try:
            ...
            if session:
                session.check_tokens()
                return requests.get(get_request_url(endpoint, session), params=params, verify=session.verify, headers=session.get_headers(headers))
            else:
                return requests.get(get_request_url(endpoint), params=params)
...
def post(endpoint, params=None, json=None, headers=None, session=None):
    ...
    if session:
        session.check_tokens()
        return requests.post(get_request_url(endpoint, session), params=params, json=json, verify=session.verify, headers=session.get_headers(headers))
    else:
...
def post_file(endpoint, files=None, headers=None, session=None):
    ...
    if session:
        session.check_tokens()
        return requests.post(get_request_url(endpoint, session), files=files, verify=session.verify, headers=session.get_headers(headers))
    else:
        raise DDHSessionException("DDH POST request requires a session")


def get_request_url(endpoint, session=None):
    return '/'.join([get_api_host(session), endpoint.strip()])
