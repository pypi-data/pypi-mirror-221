...
def xorBlocks(out_bytes: bytes, in1: bytes, in2: bytes) -> bytes:
    if (not len(in1) == len(in2)) and (not len(in2) == len(out_bytes)):
        raise ValueError('Length must all match')
...
def Transform(bc, tweak: bytes, inputData: bytes, direction: int) -> bytes:
    ...
    if not len(P) % 16 == 0:
        raise ValueError('Input Data must be a multiple of 16 long')
    m = int(len(P) / 16)
    if m == 0 or m > 128:
        raise ValueError('EME operates on 1 to 128 block-cipher blocks, you passed ' + m)
    c = bytearray([0] * len(P))
