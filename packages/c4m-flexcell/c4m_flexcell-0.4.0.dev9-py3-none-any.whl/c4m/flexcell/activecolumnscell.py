...
class _Element:
    ...
    def __init__(self, *, cell: "ActiveColumnsCell", name: str) -> None:
        ...
        asyncio.get_event_loop().run_until_complete(self.__init_queue())

    async def __init_queue(self):
        q = asyncio.Queue()
        self._placed_queue: asyncio.Queue = q
        # Put single item on queue that will be removed when the element is placed
        await q.put(True)
    ...
    async def _wait4placement(self):
        ...
        await self._placed_queue.join()
        # self.log("_wait4placement(): leave")

    def _set_placed(self) -> None:
        # self.log("_Element._set_placed()")
        if self.is_placed:
            raise RuntimeError(
                f"Internal error: _set_placed() called twice on elem '{self.name}'"
            )

        self._is_placed = True

        async def clear_queue():
            assert await self._placed_queue.get()
            self._placed_queue.task_done()
        ev = asyncio.get_event_loop()
        aw = clear_queue()
        if not ev.is_running():
            ev.run_until_complete(aw)
        else:
            ev.create_task(aw)
...
class _LayoutElement(_Element):
    ...
    async def __init_queue(self):
        x_q = asyncio.Queue()
        self._x_queue: asyncio.Queue = x_q
        # Put single item on queue that will be removed when the element is placed
        await x_q.put(True)

        y_q = asyncio.Queue()
        self._y_queue: asyncio.Queue = y_q
        # Put single item on queue that will be removed when the element is placed
        await y_q.put(True)

    async def _wait4x(self) -> None:
        # self.log("_wait4x(): enter")
        await self._x_queue.join()
        # self.log("_wait4x(): leave")

    async def _wait4y(self) -> None:
        # self.log("_wait4y(): enter")
        await self._y_queue.join()
        # self.log("_wait4y(): leave")
    ...
    def _get_x(self) -> float:
        if self.__x is None:
            raise RuntimeError(f"Internal error: non-existing x accessed on elem '{self._name}'")
        return self.__x
    def _set_x(self, x: float) -> None:
        # self.log(f"setting x to {x}")
        if self.__x is not None:
            raise RuntimeError(
                f"Internal error: element '{self.name}' already has x value"
            )
        self.__x = x
        self._try_place()

        async def clear_queue():
            assert await self._x_queue.get()
            self._x_queue.task_done()
        ev = asyncio.get_event_loop()
        aw = clear_queue()
        if not ev.is_running():
            ev.run_until_complete(aw)
    ...
    def _has_y(self) -> bool:
        return self.__y is not None
    def _get_y(self) -> float:
        if self.__y is None:
            raise RuntimeError(f"Internal error: non-existing y accessed on elem '{self._name}'")
        return self.__y
    def _set_y(self,y: float) -> None:
        # self.log(f"setting y to {y}")
        if self.__y is not None:
            raise RuntimeError(
                f"Internal error: element '{self.name}' already has y value"
            )
        self.__y = y
        self._try_place()

        async def clear_queue():
            assert await self._y_queue.get()
            self._y_queue.task_done()
        ev = asyncio.get_event_loop()
        aw = clear_queue()
        if not ev.is_running():
            ev.run_until_complete(aw)
    ...
    def _try_place(self) -> bool:
        if (
            ...
            self.__placed = self.cell.layouter.place(
                self.__unplaced, origin=_geo.Point(x=self.__x, y=self.__y)
            )
            self._set_placed()
...
class _ElementsRow(_ElementsElement[_netelem_type], Generic[_netelem_type]):
    ...
    def _set_y(self, y: float):
        if self.__y is not None:
            raise RuntimeError(
                f"Internal error: y set twice for {self.__class__.__name__} '{self.name}'"
            )
...
class _ElementsColumn(_ElementsElement[_netelem_type], Generic[_netelem_type]):
    ...
    def _set_x(self, x: float):
        if self.__x is not None:
            raise RuntimeError(
                f"Internal error: x set twice for {self.__class__.__name__} '{self.name}'"
            )
...
class _SD(_NetElement):
    ...
    def __init__(self, *,
        ...
        _implant: _prm.Implant, _well: Optional[_prm.Well], _well_net: Optional[_ckt.CircuitNetT],
        _bottom_enclosure: str, _bottom_implant_enclosure: _prp.Enclosure,
        _top_enclosure: str,
    ) -> None:
    ...
    # Overrule external _y value to be middle of the metal1
    @property
    def _has_y(self) -> bool:
        return (self._active_bottom is not None) and (self._active_top is not None)
    @property
    def _y(self) -> float:
        if not ((self._active_bottom is not None) and (self._active_top is not None)):
            self.log("No _y")
            raise RuntimeError(f"Internal error: non-existing y accessed on elem '{self._name}'")
        return 0.5*(self._active_bottom + self._active_top)
...
class _ActiveColumn(_ElementsColumn[ActiveColumnElementT]):
    ...
    def _draw_wire(self) -> None:
        ...
        if nsd is not None:
            ...
            if left is None:
                left = self._x - 0.5*ac_canvas._actcont_width
            ...
            if right is None:
                right = self._x + 0.5*ac_canvas._actcont_width
...
class _M1Column(_ElementsColumn[M1ColumnElementT]):
    def __init__(self, *,
        ...
        self._left = cast_MultiElemT(left)
...
class _Constraints:
    ...
    async def _do_layout(self) -> bool:
        ...
        tasks.append(asyncio.create_task(self._place_polypads_m1cols()))
        ...
        # All tasks should be complete now and all columns and rows should have been placed
        if not all(t.done() for t in tasks):
            raise RuntimeError("Internal error: not all tasks completed")
    ...
    async def _place_polypads_m1rows(self) -> None:
        # self.log("_place_polypads(): enter")
        cell = self.cell
        canvas = cell.canvas
        ac_canvas = cell.ac_canvas
        ...
        for elem in filter(
            ...
            chain(*(mrow.__iter_elems__() for mrow in self.m1rows)),
        ):
            # self.log(f"_place_polypads(): computing x for {elem.name}")
            ...
            for l in elem._left:
                if isinstance(l, PolyContactT):
                    if not l._has_x:
                        raise RuntimeError(
                            f"Deadlock: {elem.name} needs to wait for x of {l.name}"
                        )
                    left_xs.append(
                        elem._x
                    ...
                    await l._wait4x()
                    left_xs.append(l._x + ac_canvas._min_m1col_pitch)
                else:
                    raise NotImplementedError(f"Internal error: type '{type(elem)}'")
            elem._x = max(left_xs)
            # self.log(f"_place_polypads(): computed x {elem._x} for {elem.name}")
        # self.log("_place_polypads(): leave")

    async def _place_polypads_m1cols(self) -> None:
        # self.log("_place_polypads(): enter")
        for m1col in chain(*self.m1columns):
            # self.log(f"_place_polypads(): computing x for {m1col.name}")
            await m1col._wait4x()
            x = m1col._x
            for elem in filter(
                lambda e: isinstance(e, PolyContactT) and not e._has_x,
                ...
                elem._x = x
            # self.log(f"_place_polypads(): computed x {m1col._x} for {m1col.name}")
        ...
        for multim1col in self.m1columns:
            ...
            for col in multim1col:
                for elem in col._left:
                    await elem._wait4x()
                    x = max(x, elem._x + ac_canvas._min_m1col_pitch)
            ...
            for elem in multim1col.__iter_elems__():
                if isinstance(elem, PolyContactT):
                    ...
                    x = max((x, *left_xs)) # pyright: ignore
                elif isinstance(elem, (SignalNSDT, SignalPSDT)):
                    assert elem.with_contact, "Internal error: unchecked error"
                    await elem._wait4x()

                    if (prev_sd is not None) and abs(cast(float, prev_sd._x) - elem._x) > _geo.epsilon:
                        raise ValueError(
...
class _ActiveColumnsCanvas:
    def __init__(self, *, fab: "_scfab.StdCellFactory"):
        ...
        if (pwell is None) or (pwell not in active.well):
            ...
            self._min_nact_well_enclosure = active.min_substrate_enclosure.max()
        else:
            ...
            self._min_nact_well_enclosure = enc.max()
        ...
        if (nwell is None) or (nwell not in active.well):
            ...
            self._min_pact_well_enclosure = enc.max()
