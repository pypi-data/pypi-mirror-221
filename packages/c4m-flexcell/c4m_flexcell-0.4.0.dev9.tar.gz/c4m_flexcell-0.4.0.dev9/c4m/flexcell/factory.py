...
class _Nand(_acc.ActiveColumnsCell):
    ...
    def build_generator(self) -> _acc.ConstraintsT:
        ...
        actcols: List[_acc.ActiveColumnT] = []
        polyrows_even: List[_acc.PolyRowT] = []
        polyrows_odd: List[_acc.PolyRowT] = []
        ...
        for n in range(self.inputs):
            ...
            poly_pad = self.polypad(
                name=f"{net_name}_polypad", net=net,
                left=(() if prev_transcol is None else prev_transcol),
            )
            ...
            if n%2 == 0:
                polyrows_even.append(polyrow)
            else:
                polyrows_odd.append(polyrow)
        ...
        return self.constraints(
            activecolumns=actcols, polyrows=(
                self.multipolyrow(name="polyrow_even", rows=polyrows_even),
                self.multipolyrow(name="polyrow_odd", rows=polyrows_odd),
            ),
...
class _And(_acc.ActiveColumnsCell):
    ...
    def build_generator(self) -> _acc.ConstraintsT:
        ...
        nq_polypad = self.polypad(name="nq_polypad", net=nq, left=actcols[-3])
        polyrow = self.polyrow(
...
class _And21Nor(_acc.ActiveColumnsCell):
    ...
    def build_generator(self) -> _acc.ConstraintsT:
        ...
        for n in range(3):
            nsd = nsds[n]
            psd = psds[n]
            actcols.append(self.activecolumn(name=f"sds[{n}]", connect=False, elements=(nsd, psd)))
            ...
            if n == 0:
                polypad = self.polypad(name=f"i{n}_polypad", net=i)
            else:
                polypad = self.polypad(name=f"i{n}_polypad", net=i, left=actcols[-3])
            polyrows.append(self.polyrow(name=f"i{n}_polyrow", elements=(polypad, polyknot)))

            m1col = self.m1pin(
                name=f"i{n}_pin", elements=polypad,
                bottom=nq_nsd, top=nq_psd,
            )
            m1cols.append(m1col.multim1column)
        nsd = nsds[3]
        psd = psds[3]
        ...
        return self.constraints(
            activecolumns=actcols,
            polyrows=(
                self.multipolyrow(name="mpolyrow0", rows=(polyrows[0], polyrows[2])),
                self.multipolyrow(name="mpolyrow1", rows=polyrows[1]),
            ),
            m1rows=m1rows,
            m1columns=m1cols,
        )
...
class _Mux2(_acc.ActiveColumnsCell):
    def build_generator(self) -> _acc.ConstraintsT:
        ...
        cmd_n_npasspad = self.polypad(name="cmd_n_npasspad", net=_cmd_n, left=i0_trans)
        ...
        polyrows = (
            cmd_n_npasspolyrow.multipolyrow,
            self.multipolyrow(
                name="polyrow1", rows=(i0_nmospolyrow, i1_polyrow),
            ),
            self.multipolyrow(
...
class _Xor2(_acc.ActiveColumnsCell):
    ...
    def build_generator(self) -> _acc.ConstraintsT:
        ...
        if not self.inverted:
            ...
            i1_m1row = self.m1row(
                name="i1_m1row", elements=(i1_nmos0pad, i1_nmos1pad),
            )
            ...
            i1_pin = self.m1pin(name="i1_pin", elements=(i1_nmos1pad, i1_pmospad))
        else:
            i1_nmosrow = self.polyrow(
                name="i1_nmosrow", elements=(i1_nmos0, i1_nmospad, i1_nmos1),
            )
            i1_pmos1pad = self.polypad(name="i1_pmos", net=i1, left=i1_pass1)
            i1_pmos1row = self.polyrow(name="i1_pmos1row", elements=(i1_pmos1pad, i1_pmos1))
            i1_m1row = self.m1row(
                name="i1_m1row", elements=(i1_pmos0pad, i1_pmos1pad),
            )
            i1_pin = self.m1pin(name="i1_pin", elements=(i1_nmospad, i1_pmos1pad))
        ...
        return self.constraints(
            ...
            polyrows=(
                ...
                self.multipolyrow(name="pmos_multirow", rows=(
                    i0_pmosrow,
                    i1_n_pmosrow if not self.inverted else i1_pmos0row,
...
class _DFF(_acc.ActiveColumnsCell):
    ...
    def build_generator(self) -> _acc.ConstraintsT:
        ...
        u_pmospad = self.polypad(name="u_nmospad", net=_u, left=i_trans)
        ...
        # clk_n - pt. 2
        clk_n_nmos1pad = self.polypad(name="clk_n_nmos1pad", net=_clk_n, left=u_trans)
        ...
        clk_buf_pmos0pad = self.polypad(
            name="clk_buf_pmos0pad", net=_clk_buf, left=u_trans,
        )
        ...
        dff_m_nmospad = self.polypad(name="dff_m_nmospad", net=_dff_m, left=y_trans)
        dff_m_pmospad = self.polypad(name="dff_m_pmospad", net=_dff_m, left=y_trans)
        ...
        return self.constraints(
            ...
            polyrows=(
                self.multipolyrow(name="polyrow0", rows=(
                    clk_nmosrow, i_nmosrow, clk_n_nmos1row, dff_m_nmospolyrow,
                    clk_n_nmos2polyrow,
                )),
                self.multipolyrow(name="polyrow1", rows=(
                    u_nmosrow, q_nmospolyrow, y_nmospolyrow,
                )),
                self.multipolyrow(name="polyrow2", rows=(
                    clk_buf_polyrow, dff_s_polyrow,
                )),
                self.multipolyrow(name="polyrow3", rows=(
                    clk_n_polyrow, q_pmospolyrow,
                )),
                self.multipolyrow(name="polyrow4", rows=(
                    clk_pmosrow, i_pmosrow, clk_buf_pmos0polyrow, y_pmospolyrow,
                    clk_buf_pmos1polyrow,
                )),
                self.multipolyrow(name="polyrow5", rows=(
                    u_pmosrow, dff_m_pmospolyrow,
                )),
            ),
            m1rows=(
...
class _DFFnR(_acc.ActiveColumnsCell):
    ...
    def build_generator(self) -> _acc.ConstraintsT:
        ...
        y_m1col1 = self.m1column(
            name="y_m1col1", elements=(y_m1knot1, y_m1knot2, y_psd0),
        )
        y_m1row = self.m1row(
            name="y_m1row", elements=(y_m1knot2, y_psd1.contact),
        )
        ...
        return self.constraints(
            ...
            polyrows=(
                ...
                self.multipolyrow(name="polyrow1", rows=(
                    u_nmosrow, y_nmospolyrow, nrst_nmos0polyrow, nrst_nmos1polyrow,
                    q_nmospolyrow,
                )),
                self.multipolyrow(name="polyrow2", rows=(
                    clk_buf_polyrow, dff_s_polyrow,
                ...
                self.multipolyrow(name="polyrow4", rows=(
                    clk_pmosrow, i_pmosrow, clk_buf_pmos0polyrow, y_pmospolyrow,
                    nrst_pmospolyrow, clk_buf_pmos1polyrow, q_pmospolyrow,
                )),
                self.multipolyrow(name="polyrow5", rows=(
                    u_pmosrow, dff_m_pmospolyrow,
                )),
            ...
            m1rows=(
                ...
                self.multim1row(
                    name="psdcontm1row", rows=(y_m1row, dff_s_m1row),
                ),
                self.multim1row(name="psdmrow", rows=(
                    u_psdrow, dff_m_psdrow, net5_psdrow,
                )),
                # q_psdrow need separate multirow as they have different y
                self.multim1row(name="psdmrowmax", rows=q_psdrow),
            ),
            m1columns=(
                ...
                clk_buf_pmos1m1col.multim1column, nrst_m1col1.multim1column,
