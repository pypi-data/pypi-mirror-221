...
def writeCfgProcPiv(data,nomeFile):
    ...
    with open(nomeFile,'w', encoding="utf8") as f:
        ...
        #    % Windows dimensions ***************************************
        f.write(f'[{foList(vect[0],"<4d")}], 			Height of the windows (rows) - insert the sequence of numbers separated by a blank character (1)\n')
        f.write(f'[{foList(vect[2],"<4d")}], 			Width of the windows (columns) (2)\n')
        f.write(f'[{foList(vect[1],"<4d")}], 			Grid distance along the height direction (y) (3)\n')
        f.write(f'[{foList(vect[3],"<4d")}], 			Grid distance along the width direction (x) (4)\n')
...
def data2PIV(data):
    ...
    PIV.Inp.FlagValid=0
    PIV.Inp.FlagAttivaValSN=4
    if PRO.FlagNogTest: 
        PIV.Inp.FlagValidNog=1
    else:
        PIV.Inp.FlagValidNog=0
        if PRO.FlagMedTest:
    ...
    #PIV.Inp.FlagCorrHart=PRO.FlagCorrHart
    ...
    if PRO.FlagAdaptative:
        PIV.Inp.SemiDimCalcVel=-PRO.NItAdaptative
        PIV.Inp.NIterazioni=PRO.NIterazioni+PRO.NItAdaptative
    else:   
