...
class Mongo(Database):
    ...
    async def update_admin(self, obj: root.DesktopObject, collection: str, default_content: Optional[IO] = None) -> UpdateResult | None:
        """
        ...
        coll = self.get_collection(collection)
        try:
            result = await coll.replace_one({'_id': ObjectId(obj.id)}, obj.to_dict())
            if result and default_content is not None:
                fs = self.__new_gridfs_bucket(collection)
