...
W1_TO_WWMF = [
    ...
    (24, "Orages possibles", 90, "Orages"),
    (25, "Orages probables", 90, "Orages"),
    ...
    (32, "Averses localement orageuses", 70, "Averses"),
...
def fromW1ToWWMF(
    codeW1: Union[int, Iterable, xr.DataArray], w1_to_wwmf: Optional[list] = None
) -> Union[int, Iterable, xr.DataArray]:
    if w1_to_wwmf is None:
        w1_to_wwmf = W1_TO_WWMF

    if isinstance(codeW1, xr.DataArray):
        result = xr.apply_ufunc(lambda x: np.vectorize(fromW1ToWWMF)(x), codeW1)
        result.attrs["units"] = "wwmf"
        return result
    elif isinstance(codeW1, Iterable):
        return [fromW1ToWWMF(code, w1_to_wwmf) for code in codeW1]
    elif np.isnan(codeW1):
        return np.nan
    return convert_wwmf(codeW1, w1_to_wwmf, "W1")


def fromWWMFToW1(code_list: Union[Iterable, int]) -> list:
    """
    Enable to convert a list of WwMF code to a list of W1 code

    Args:
        code_list (Union[Iterable, int]): The input WwMF codes
    Returns:
        [list]:  W1 codes
    """
    if not isinstance(code_list, Iterable):
        code_list = [code_list]
    W1_code = []
    for x in code_list:
        if x not in DF_WWMF_TO_W1["Code WWMF"].values:
            LOGGER.error(f"We did not found the translation for this code {x}")
        W1_code = (
            W1_code
            + DF_WWMF_TO_W1.loc[DF_WWMF_TO_W1["Code WWMF"] == x, "Code W1"].to_list()[0]
        )
