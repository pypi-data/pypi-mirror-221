...
class ModelBuilder:
    ...
    def add_group(self, node, group):
        node.input_names = group.inports
        node.output_names = group.outports
        self.groups[node] = group

    def add_reference_submodel(self, node, submodel):
        node.input_names = submodel.inports
        node.output_names = submodel.outports
    ...
    def add_link(self, src, dst, id=None):
        ...
        if dst in self.links:
            ...
            raise TypeError(
                ...
                "Hint: An input port can only be connected to one output port."
            )
    ...
    def remove_link(self, id):
        ...
        """
        for dst_port in self.links:
            if self.links[dst_port].id == id:
                del self.links[dst_port]
                return
        raise ValueError(f"Link {id} not found in model")
