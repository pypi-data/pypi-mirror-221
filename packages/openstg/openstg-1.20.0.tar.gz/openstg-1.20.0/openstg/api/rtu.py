...
class AllLSParametersExtendedAsync(ReadOnlyResource):
    def get(self, ls_name):
        info = get_required_info(request)
        from primestgplus.service import RTUService
        try:
            s = RTUService(info['request_id'], get_url_from_ls(ls_name), sync=False, source=info['source'])
            rep = s.get_all_ls_parameters_extended(info['date_from'], info['date_to'])
            return rep, {'request_id': info['request_id']}
        except IndexError as e:
            return {'error': e.message}
...
class AllLSParametersAsync(ReadOnlyResource):
    ...
    def get(self, ls_name):
        ...
        try:
            s = RTUService(info['request_id'], get_url_from_ls(ls_name), sync=False, source=info['source'])
            rep = s.get_all_ls_parameters(info['date_from'], info['date_to'])
            return rep, {'request_id': info['request_id']}
        except IndexError as e:
            return {'error': e.message}
...
class AllLSVoltageCurrentProfileAsync(ReadOnlyResource):
    ...
    def get(self, ls_name):
        ...
        try:
            s = RTUService(info['request_id'], get_url_from_ls(ls_name), sync=False, source=info['source'])
            rep = s.get_all_ls_voltage_current_profile(info['date_from'], info['date_to'])
            return rep, {'request_id': info['request_id']}
        except IndexError as e:
            return {'error': e.message}
...
rtu_resources = [
    ...
    (AllLSParametersExtendedAsync, '/<string:ls_name>/get-ls-parameters-extended/async'),
    # PRIME code: S52
    ...
    (AllLSParametersAsync, '/<string:ls_name>/get-ls-parameters/async'),
    # PRIME code: S62
    ...
    (AllLSVoltageCurrentProfileAsync, '/<string:ls_name>/get-ls-voltage-current-profile/async'),
]