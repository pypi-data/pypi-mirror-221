...
class TestDatasetPool():
    ...
    def test_normalise(self):
        data2 = data.normalise(object_name='Bacteria', col='SpineLength', normalise_by='SpineWidth', new_col='norm_col')
        test_norm = data.Bacteria.SpineLength / data.Bacteria.SpineWidth
        assert data2.Bacteria.norm_col.equals(test_norm)
    ...
    def test_add_from_child(self):
        data2 = data.add_from_child(object_name='Bacteria', child='CFP_spots', col='DistCC_oc2',
                                    multiple='min', rename='CFP_mCherry_min_dist')
