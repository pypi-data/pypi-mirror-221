...
def list(config):
    ...
    sources = []
    headers = ["ID", "Name", "Type", "Reference", "Sensitive", "Visibility" ,"Description", "Validity"]
...
def validate_source(config):
    ...
    for i, source in enumerate(config.config["sources"]):
        ...
        if "description" not in source:
            logger.warning(f"Source [{source['name']}] has no description, defaulting to empty string")
            config.config["sources"][i]["description"] = ""

        # check for reference
        if "reference" not in source:
            logger.warning(f"Source [{source['name']}] has no reference status, defaulting to False")
            config.config["sources"][i]["reference"] = False

        # check for Sensitive
        if "sensitive" not in source:
            logger.warning(f"Source [{source['name']}] has no sensitive status, defaulting to False")
            config.config["sources"][i]["sensitive"] = False

        # check for visibility
        if "visibility" not in source:
            logger.warning(f"Source [{source['name']}] has no visibility status, defaulting to private")
                    ...
                    if (
                        ...
                        if source["description"] == response_data["description"]:
                            if response_data["versions"] != []:
                                if source_version(source) == response_data["versions"][0]["sem_ver_id"]:
                                        logger.info(
