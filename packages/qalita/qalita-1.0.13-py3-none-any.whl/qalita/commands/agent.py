...
from qalita.cli import pass_config, logger, get_version
...
def login(config):
    ...
    if r.status_code == 200:
        ...
        logger.info("Make sure you are using compatible versions for the platform and the cli,\n\t\t\t\t\t\t\t\t\t > check compatibility matrix on the documentation <")
    authenticate()
            ...
            while True:
                ...
                if check_job.status_code == 200:
                    ...
                    for job in jobs:
                        ...
                        job_run(source_id, source_version, pack_id, pack_version, job_id=job["id"])
                    time.sleep(1)
                else:
...
def job_run(config, source_id, source_version, pack_id, pack_version,job_id=None):
    ...
    if source_version is None:
        ...
        if response_source.status_code == 200:
            ...
            if versions:
                latest_version = max(versions, key=lambda v: semver.parse_version_info(v['sem_ver_id']))
                source_version = latest_version['sem_ver_id']

        logger.info(f"Source version not specified, Latest source version is {source_version}")
    ...
    logs_id = post_run(os.path.join(temp_folder_name,pack_folder),f'{datetime_string}_{random_seed}',pack_id,pack_version,source_id,source_version)
...
def post_run(config,run_path,name,pack_id,pack_version,source_id,source_version):
    ...
    if os.path.exists(run_path+"/logs.txt"):
        ...
        if r.status_code == 200:
            ...
            logger.error(
                "\tFailed to push logs, logs already exist"
            )
            sys.exit(1)
        else:
            logger.error(
    ...
    if os.path.exists(run_path+"/metrics.json"):
        ...
        if r.status_code == 200:
            ...
            logger.error(
                "\tFailed to push Metrics, logs already exist"
            )
            sys.exit(1)
        else:
            logger.error(
    ...
    if os.path.exists(run_path+"/recommendations.json"):
        ...
        if r.status_code == 200:
            ...
            logger.error(
                "\tFailed to push recommendations, logs already exist"
            )
            sys.exit(1)
        else:
            logger.error(
...
def joblist(config):
    ...
    tab_jobs = []
    agent_conf = config.load_agent_config()
    headers = ["ID", "Name","Status","Source ID","Source Version","Pack ID","Pack Version","Start","End"]

    r = send_request(request=f"{agent_conf['context']['local']['url']}/agents/{agent_conf['context']['remote']['id']}",
                         mode="get")
    if r.status_code == 200:
        jobs = r.json()
        ...
        logger.error(f"Error cannot fetch job list, make sure you are logged in with > qalita agent login : {r.status_code} - {r.reason}")
        return
