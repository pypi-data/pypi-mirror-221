...
def get_tick_price_multi_df(order_book_ids, start_date, end_date, fields, market):
    ...
    history_latest_date = 0 if history_df is None else date_to_int8(get_current_trading_date(
        history_df.index.levels[1].max()))
    today = today_int()
    next_trading_date = date_to_int8(get_next_trading_date(today, market=market))
    if start_date > next_trading_date or end_date < today:
        return history_df
    ...
    for ins in ins_list:
        if ins.de_listed_date != '0000-00-00' and ins.de_listed_date < today_str:
            continue
        cur_live_date = live_date if (
                ins.type == 'Future' or (ins.type == 'Option' and ins.exchange not in ('XSHG', 'XSHE'))
        ) else today
        try:
            if history_df is not None and date_to_int8(get_current_trading_date(
                    history_df.loc[ins.order_book_id].index.max())) == cur_live_date:
                continue
        except KeyError:
            pass
        try:
