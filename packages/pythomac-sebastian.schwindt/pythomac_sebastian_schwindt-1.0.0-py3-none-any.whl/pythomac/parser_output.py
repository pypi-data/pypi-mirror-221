"""
Adapted from HOMETEL/scripts/python3/postel/parser_output.py under the GNU GENERAL PUBLIC LICENSE Version 3,
    29 June 2007

@author: TELEMAC-MASCARET Consortium (original)
@brief
    Functions to read Telemac result file

Adapted from HOMETEL.scripts.python.postel by Sebastian Schwindt (2023). Changelog:
    * translated np.float into float
    * reformatted bad line breaks
...
class OutputFileData():
    ...
    def get_time_profile(self):
        """
        @brief : Returns the time profile in iteration and in seconds, read from the TELEMAC-* output file
        ...
        return ('Iteration #', np.array(itr)), ('Time (s)', np.array(time, dtype=float))
    ...
    def get_volume_profile(self):
        """
        Returns the time series of Values, read from the TELEMAC-2D output file volumes, fluxes, errors, etc.
        Assumptions:
                ...
                for i in range(liqnumber):
                    ...
                    while not proc:
                        ...
                        if iline >= len(self.output):
                            raise TelemacException('... Could not parse FLUXES FOR BOUNDARY {}'.format(str(i+1)))
                        proc = re.match(form_volfluxes, self.output[iline])
    ...
    def get_value_history_output(self, vrs):
        ...
        @param vrs (string): 'voltotal' (total volume), 'volfluxes' (boundary fluxes), or 'volerror' (error in volume)
    ...
    def get_sediment_mass_profile(self):
        """
        Return the total and lost mass of sediment for each class for each listing iteration

        @returns (dict) Keys are the id of the sediment and each contains a dictionary with eky total and lost
        """
        ...
        for iclass in mass:
            for ttype in ['total', 'lost', 'cumu_lost', 're_ial']:
                if ttype in ['total', 're_ial']:
                    ...
                    array = np.array(mass[iclass][ttype][:-1], dtype=float)
                else:
                    array = np.array(mass[iclass][ttype], dtype=float)
    ...
    def get_user_defined_output(self, user_form, line_num=False):
        ...
        @param user_form (string) user gives a regular expression to find in the file.
        @param line_num (boolean) return or not the line number

        @return (list, list) or (list) file line (and line number if line_num==True)
        """
        iline = 0
        user_value = []
        num_line = []
    ...
    def get_tracer_mass_profile(self):
        """
        Return the total and lost mass of suspended tracer for each class for each listing iteration

        @returns (dict) Keys are the id of the tracer and each contains a dictionary with eky total and lost
        """
